...
public class BatchPayBatchCommitUtil {
	private static Log logger = LogFactory
			.getLog(BatchPayBatchCommitUtil.class);
	 ...
	 */
	static boolean confirmBatch(String bizType, TvFilepackagerefDto idto,
		...
		try {
			...
			if("000077100005".equals(ITFECommonConstant.SRC_NODE))
			{
				movedataSql = "INSERT INTO TV_PAYOUTFINANCE_MAIN"
					...
					+ " FROM TV_PAYOUTFINANCE A WHERE S_BOOKORGCODE= ? "
					+ " AND S_FILENAME= ? AND (S_STATUS IS NULL OR S_STATUS= ? )"
				...
				movedataSql = "INSERT INTO TV_PAYOUTFINANCE_MAIN"
					...
					+ " FROM ("
					+ "SELECT MAX(I_VOUSRLNO) AS I_VOUSRLNO,S_BOOKORGCODE,S_TRECODE, S_RCVRECKBNKNO,S_PAYERACCT,MAX(S_PAYERNAME) S_PAYERNAME,MAX(S_PAYERADDR) S_PAYERADDR,"
					+ "D_ACCEPT,S_PACKAGENO,MAX(S_ADDWORD) S_ADDWORD,S_BUDGETTYPE,S_BDGORGCODE,S_FUNCSBTCODE,sum(F_AMT) F_AMT,S_FILENAME"
					+ " FROM TV_PAYOUTFINANCE A WHERE S_BOOKORGCODE= ? "
					+ " AND S_FILENAME= ? AND (S_STATUS IS NULL OR S_STATUS= ? )"
					+ " GROUP BY  S_BOOKORGCODE,S_TRECODE, S_RCVRECKBNKNO,S_PAYERACCT,"
			...
			if("000077100005".equals(ITFECommonConstant.SRC_NODE))
			{
				movedataSql = "INSERT INTO TV_PAYOUTFINANCE_SUB"
					...
					+ " F_AMT,CURRENT TIMESTAMP FROM TV_PAYOUTFINANCE A WHERE S_BOOKORGCODE= ? AND S_FILENAME= ? AND (S_STATUS IS NULL OR S_STATUS= ? ) ";
			}else {
				movedataSql = "INSERT INTO TV_PAYOUTFINANCE_SUB"
					+ "(I_VOUSRLNO,S_SEQNO,S_TRANO,S_PAYEEACCT,S_PAYEENAME,S_PAYEEADDR,S_PAYEEOPNBNKNO,N_SUBAMT,TS_SYSUPDATE)"
					+ " SELECT "
			...
			sqlExec = DatabaseFacade.getDb().getSqlExecutorFactory()
					.getSQLExecutor();
			movedataSql = "UPDATE TV_PAYOUTFINANCE SET S_STATUS=? WHERE S_BOOKORGCODE= ? AND S_FILENAME= ? ";
			// 核算主体代码
...
//				movedataSql = "delete from TV_FILEPACKAGEREF where S_ORGCODE= '"+loginfo.getSorgcode()+"' AND S_FILENAME= '"+fileName+"' and S_OPERATIONTYPECODE='"+bizType+"'";
...
//					+"from TV_PAYOUTFINANCE_MAIN main WHERE S_BOOKORGCODE= '"+loginfo.getSorgcode()+"' AND S_FILENAME= '"+fileName+"' and S_STATUS='"+DealCodeConstants.DEALCODE_ITFE_DEALING+"'";
//				sqlExec.runQuery(movedataSql);
//				sqlExec.commitTrans();
