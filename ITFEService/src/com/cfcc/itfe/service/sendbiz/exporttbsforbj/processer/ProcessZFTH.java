...
/**TBS集中支付退回回执处理器**/
public class ProcessZFTH extends BasicTBSFileProcesser{
	...
	public MulitTableDto process(String fullFileName) {
		...
		try {
			...
			for(int i=0; i<fileContent.size(); i++) {
				...
				dto.setTableName(TvPayreckBankBackDto.tableName());
				dto.setVtcode(MsgConstant.VOUCHER_NO_2302);
				...
				if(StringUtils.isBlank(famt)) {
					...
					dto.setFamt(new BigDecimal(famt.trim()).negate());
				}
			...
			throw new RuntimeException("tbs回执文件处理异常(ZFTH)",e);
		}
	}
}
