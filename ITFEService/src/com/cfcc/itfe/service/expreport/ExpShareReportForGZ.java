...
public class ExpShareReportForGZ extends AbstractExpReport {
...
//		queryDto.setSfinorgcode(idto.getSfinorgcode());
		//只查询共享级次为零o的记录，否则会出现重复
		queryDto.setSbudgetlevelcode(StateConstant.shareBudgetLevel);
		...
		String filename ="" ;
            ...
            resList.addAll(CommonFacade.getODB().findRsByDtoForWhere(queryDto,
					sqlWhere));
         // 获取合并后的报表
			List <TrIncomedayrptDto> mergelist = mergeTaxOrgBillForShareRpt(idto, sbookorgcode,rptType);
          ...
          //判断是否款合计
			...
			if (resList.size() > 0) {
				...
				for (int i = 0; i < resList.size(); i++) {
					...
					filebuf.append(splitSign);
					/**
					 * 针对珠海需求，修改成如果是珠海，则所属国库代码为本身 by hua 2013-6-18
					 */
					if("1913".equals(strecode.trim().substring(0, 4))) {
						filebuf.append(_dto.getStrecode());
					}else {
