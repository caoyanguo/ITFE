...
public class ExpShareReportForGZ_OLD extends AbstractExpReport {
...
//		queryDto.setSfinorgcode(idto.getSfinorgcode());
		...
		String filename ="" ;
            ...
            String sql = "select sum(N_MONEYDAY) as nmoneyday,sum(N_MONEYTENDAY) as nmoneytenday,sum(N_MONEYMONTH)  as nmoneymonth,SUM(N_MONEYQUARTER) as nmoneyquarter, sum(N_MONEYYEAR) as nmoneyyear," +
            		" S_TRECODE,S_TAXORGCODE,S_RPTDATE,S_BUDGETTYPE,S_BUDGETSUBCODE,S_BELONGFLAG,S_TRIMFLAG  FROM TR_INCOMEDAYRPT where S_RPTDATE =? AND S_BILLKIND=? AND S_TRIMFLAG =? AND S_BUDGETTYPE =? " +
            		sqlWhere +" GROUP BY S_TRECODE,S_TAXORGCODE,S_RPTDATE,S_BUDGETTYPE,S_BUDGETSUBCODE,S_BELONGFLAG,S_TRIMFLAG";
             
            sqlexec.addParam(srptdate);
            sqlexec.addParam(rptType);
            sqlexec.addParam(strimflag);
            sqlexec.addParam(bugtype);
            List <TrIncomedayrptDto> sqlList = (List<TrIncomedayrptDto>) sqlexec.runQueryCloseCon(sql,TrIncomedayrptDto.class).getDtoCollection();
            if (sqlList.size()>0) {
            	 resList.addAll(sqlList);
			}
			if (resList.size() > 0) {
				// 得到相对文件名称
				...
				StringBuffer filebuf = new StringBuffer(
						"skgkdm,mdgkdm,ssgkdm,jgdm,kmdm,zwrq,yszl,rlj,ylj,nlj\r\n");
				...
				for (int i = 0; i < resList.size(); i++) {
					...
					if(MsgConstant.RULE_SIGN_SELF.equals(sbelong)&& null!=_dto.getSbudgetlevelcode() && _dto.getSbudgetlevelcode().compareTo(tredtolevel)>0){
						continue;
					}
