package com.cfcc.itfe.service.para.tsinfoconnorgacc;
...
public class TsInfoconnorgaccService extends AbstractTsInfoconnorgaccService {
	private static Log log = LogFactory.getLog(TsInfoconnorgaccService.class);	
    ...
    public IDto addInfo(IDto dtoInfo) throws ITFEBizException {
    	try {
			...
			if (e.getSqlState().equals("23505")) {
				throw new ITFEBizException("字段核算主体代码，国库主体代码，付款人帐户已存在，不能重复录入！", e);
			}
    ...
    public void modInfo(IDto dtoInfo) throws ITFEBizException {
    	try {
			DatabaseFacade.getDb().update((TsInfoconnorgaccDto)dtoInfo);
		} catch (JAFDatabaseException e) {
			log.error(e);
			if (e.getSqlState().equals("23505")) {
				throw new ITFEBizException("字段核算主体代码，国库主体代码，付款人帐户已存在，不能重复录入！", e);
