 ...
 *
 */
public class VoucherTimerSendVouChkBill implements Callable{
	private static Log logger = LogFactory.getLog(VoucherTimerSendVouChkBill.class);
	private static final String SubVoucherType = "'"
					...
					+MsgConstant.VOUCHER_NO_2302+"'" ;//凭证类型5106 5108 5207 5209 3208 2301 2302
	private List voucherList;//生成的凭证
	
	public Object onCall(MuleEventContext eventContext)  {
		logger.debug("======================== 凭证库定时生成发送每日对账单任务开始========================");
		Voucher voucher=(Voucher) ContextFactory.getApplicationContext().getBean(
				MsgConstant.VOUCHER);
		String voucherReturnDate=TimeFacade.getCurrentStringTime();//凭证回单/退回日期： 取系统当前日期
		try {
			/*
			 * 根据凭证回单/退回日期 
			 * 查找已处理完成的凭证：状态为已回单、已退回
			 */
			List list=findVoucherDto(voucherReturnDate);
			if(list != null && list.size() >0){
				for(TvVoucherinfoDto dto:(List<TvVoucherinfoDto>)list){
					//校验对账凭证是否已生成
					try{
						List voucherList=CommonFacade.getODB().findRsByDto(dto);
					...
					//生成每日凭证
					 new CreateVoucherForReportService().createVoucherAndSend(dto.getSvtcode(), dto.getSattach(), 
							 dto.getStrecode(), voucherReturnDate, dto.getSorgcode());
					 //发送凭证
					 if(voucherList!=null&&voucherList.size()>0){
						 List lists = new ArrayList();
						 lists.add(voucherList);
						 voucher.voucherSendReturnSuccess(lists);
					 }						 
				}			
			}		
		}catch(Exception e){
		...
		logger.debug("======================== 凭证库定时生成发送每日对账单任务关闭 ========================");
		return null;	
	}
	 ...
	 */
	public List findVoucherDto(String voucherReturnDate) throws JAFDatabaseException, ValidateException {		
		SQLExecutor sqlExecutor = DatabaseFacade.getDb().getSqlExecutorFactory().getSQLExecutor();
		sqlExecutor.clearParams();
		SQLResults rs = sqlExecutor.runQueryCloseCon(getSql(voucherReturnDate),TvVoucherinfoDto.class);
	 ...
	 * @return
	 */
	private String getSql(String voucherReturnDate){
		return " SELECT CASE S_VTCODE WHEN '2301' THEN '3502' WHEN '2302' THEN '3502' ELSE '3501' END AS S_VTCODE,"+
			   " S_VTCODE AS S_ATTACH,S_TRECODE, S_ORGCODE, S_CONFIRUSERCODE AS S_CHECKDATE FROM TV_VOUCHERINFO WHERE ( S_STATUS = " +
			   " '"+DealCodeConstants.VOUCHER_SUCCESS+"' or S_STATUS = '"+DealCodeConstants.VOUCHER_FAIL+"' ) "+
			   " AND S_VTCODE IN ("+SubVoucherType+") AND S_CONFIRUSERCODE='"+voucherReturnDate+"'"+
		       " GROUP BY S_VTCODE ,S_TRECODE , S_ORGCODE, S_CONFIRUSERCODE";
	}
				
	/**
	 * 生成每日凭证服务类 CreateVoucherForReportService
	 * @author hejianrong
	 * @time  20-06-20
	 *
	 */
	class CreateVoucherForReportService {
		private Log log = LogFactory.getLog(CreateVoucherForReportService.class);
		...
		public Integer createVoucherAndSend(String VoucherType,
			...
			try {
				...
				if(result.getRowCount()<=0)
					return count;
				voucherList=new ArrayList<TvVoucherinfoDto>();
