 ...
 * @time 12-02-21 15:15:51 子系统: RecBiz 模块:centralizePayInto 组件:CentralizePayInto
 */
@SuppressWarnings("unchecked")
public class CentralizePayIntoBean extends AbstractCentralizePayIntoBean
		implements IPageDataProvider {

	private static Log log = LogFactory.getLog(CentralizePayIntoBean.class);
	...
	private DirectPaySubBean subBean = null;
	...
	public CentralizePayIntoBean() {
		...
		searchdto = new TbsTvDirectpayplanMainDto();
		setSubBean(new DirectPaySubBean(commonDataAccessService, searchdto));
	...
	{
		try {
			filelist = uploadConfirmService.getDirectGrantFileList(filemainpath, "010");
		} catch (Throwable e) {
			log.error(e);
			if(e.toString().contains("HttpInvokeServletException")||e.toString().contains("HttpInvokerException")){
    ...
    public String serviceFileAdd(Object o){
		...
		try
		{
			...
			if (checklist != null && checklist.size() > 0) {
				...
				MulitTableDto bizDto = fileResolveCommonService.loadFile(servicefilelist, BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN,MsgConstant.ZHIJIE_DAORU, null);
				...
				// 记接收日志
				commonDataAccessService.saveTvrecvlog(DeleteServerFileUtil.wipeFileOut(servicefilelist, bizDto.getErrNameList()),BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN);
			...
			if (problemStr.toString().trim().length() > 0) {
				...
				if (servicefilelist != null && servicefilelist.size() > 0) {
					...
					noteInfo = "共加载了" + checklist.size() + "个文件，其中有" + errCount+ "个错误文件，部分信息如下【详细错误信息请查看" + errName + "】：\r\n";
				}
    ...
    public String pldownload(Object o){
		...
		if (checklist != null && checklist.size() > 0) {
			...
			try {
				...
				MessageDialog.openMessageDialog(null, "批量直接支付数据下载成功！\n路径:"+clientpath );
			} catch (Exception e) {
				log.error("批量直接支付数据下载成功", e);
				MessageDialog.openErrorDialog(null, e);
			}
		}
		return super.pldownload(o);
		...
		try {
			...
			for (int i = 0; i < filepath.size(); i++) {
				...
				interfacetype = getFileObjByFileNameBiz(
						BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN, tmpfilename);
			...
			if (fileList != null && fileList.size() > 0 && resultType != null
				...
				MulitTableDto bizDto = fileResolveCommonService.loadFile(
						fileList, BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN,
						resultType, null);
				errCount = bizDto.getErrorCount() + wrongFileSum;
				if (null != bizDto.getErrorList()
						&& bizDto.getErrorList().size() > 0) {
	...
	public String submit(Object o) {
		try {
			if (commonDataAccessService.judgeDirectSubmit(
					BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN, loginfo
							.getSorgcode())) {
				int result = uploadConfirmService
						.directSubmit(BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN);
			...
			try {
				...
				for (int i = 0; i < selectedfilelist.size(); i++) {
					...
					int result = uploadConfirmService.batchDelete(
							BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN, dto);
		...
		if (org.eclipse.jface.dialogs.MessageDialog.openQuestion(this.editor
			...
			try {
				...
				for (int i = 0; i < selecteddatalist.size(); i++) {
					TbsTvDirectpayplanMainDto dto = (TbsTvDirectpayplanMainDto) selecteddatalist
							.get(i);
	...
	// BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN, sumamt);
	...
	public String zbsearch(Object o) {
		...
		try {
			...
			showdatalist = uploadConfirmService.eachQuery(
					BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN, searchdto);
			// 统计笔数金额
			List<TbsTvDirectpayplanMainDto> mlist = (List<TbsTvDirectpayplanMainDto>) showdatalist;
			totalmoney = new BigDecimal(0.00);
			totalnum = 0;
			for (TbsTvDirectpayplanMainDto mdto : mlist) {
	...
	/**
	 * Direction: 双击显示明细 ename: doubleClickEvent 引用方法: viewers: 子明细记录显示
	 * messages:
	 */
	public String doubleClickEvent(Object o) {
		if (null != o) {
			TbsTvDirectpayplanMainDto main = (TbsTvDirectpayplanMainDto) o;
	...
	private String getFileObjByFileNameBiz(String biztype, String filename)
		...
		if (tmpfilename_new.indexOf(".txt") > 0) {
			...
			if (tmpfilename.length() == 13 || tmpfilename.length() == 15) {
				...
				interficetype = MsgConstant.ZHIJIE_DAORU; // tbs接口
				if (BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN.equals(fileobj
						.getSbiztype())) {
				} else {
	...
	public void setSumamt(BigDecimal sumamt) {
		...
		if (sumamt == null || sumamt.compareTo(new BigDecimal(0)) == 0) {
			...
			refdto
					.setSoperationtypecode(BizTypeConstant.BIZ_TYPE_DIRECT_PAY_PLAN);
			try {
				...
				if (showfilelist != null && showfilelist.size() > 0) {
					...
					MessageDialog.openMessageDialog(null, "直接支付额度不存在未销号数据!");
				}
			} catch (ITFEBizException e) {
				log.error(e);
				MessageDialog.openErrorDialog(null, e);
	...
	public String selectEvent(Object o) {
		List<TbsTvDirectpayplanMainDto> mlist = (List<TbsTvDirectpayplanMainDto>) selecteddatalist;
		totalmoney = new BigDecimal(0.00);
		totalnum = 0;
		for (TbsTvDirectpayplanMainDto mdto : mlist) {
			totalmoney = totalmoney.add(mdto.getFamt());
	...
	public DirectPaySubBean getSubBean() {
		return subBean;
	}

	public void setSubBean(DirectPaySubBean subBean) {
	...
	public void setFilemainpath(String filemainpath) {
		if(filemainpath.indexOf("/itfe/czftp/")>=0||filemainpath.indexOf("/itfe/nnczftp/")>=0)
			CentralizePayIntoBean.filemainpath = filemainpath;
		else
			CentralizePayIntoBean.filemainpath="/itfe/czftp/";
	}
