package com.cfcc.itfe.client.recbiz.commercialbankpayment;
 ...
 * codecomment: 
 * @author db2admin
 * @time   13-01-23 11:13:30
 * 子系统: RecBiz
 * 模块:commercialbankDirectPayment
 * 组件:CommercialbankDirectPayment
 */
public class CommercialbankPaymentBean extends AbstractCommercialbankPaymentBean implements IPageDataProvider {

    private static Log log = LogFactory.getLog(CommercialbankPaymentBean.class);
	...
	private PayreckBankSubBean subBean = null;
	private List<String> biztypeList=new ArrayList<String>();

	public CommercialbankPaymentBean() {
      super();
      ...
      setSubBean(new PayreckBankSubBean(commonDataAccessService, searchdto));
    ...
    public String dateload(Object o){
		...
		try {
			...
			if (filepath.size() > 1000) {
				MessageDialog.openMessageDialog(null, "所选加载文件不能大于100个！");
				return null;
			}
    ...
    public String submit(Object o){
    	String flag=BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY;
    
    	try {
			if (commonDataAccessService.judgeDirectSubmit(flag, loginfo.getSorgcode())) {
				int result = uploadConfirmService.directSubmit(flag);
    ...
    public String plsubmit(Object o){
    	...
    	try {
			 ...
			 * 判断能否批量销号
			 */
			if (commonDataAccessService.judgeBatchConfirm(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY, loginfo.getSorgcode())) {
...
//			searchdto.setSstate(StateConstant.CONFIRMSTATE_NO);
	...
	private String getFileObjByFileNameBiz(String filename)
		...
		if (tmpfilename_new.indexOf(".txt") > 0) {
			...
			if (tmpfilename.length() == 13 || tmpfilename.length() == 15) {
				...
				interficetype = MsgConstant.APPLYPAY_DAORU; // tbs接口
				if (BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY.equals(fileobj
						.getSbiztype())||BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY.equals(fileobj
								.getSbiztype())) {
				} else {
	...
	public void setSumamt(BigDecimal sumamt) {
		...
		this.sumamt = sumamt;
		String flag=BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY+BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY;
		if (sumamt == null || sumamt.compareTo(new BigDecimal(0)) == 0) {
			...
			String sqlWhere="";
			if(flag.equals(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY+BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY)){
				sqlWhere=" and ( S_operationtypecode= '"+BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY+"' or S_operationtypecode='"+BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY+"')";
			}else if(flag.equals(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY)){
				sqlWhere=" and S_operationtypecode= '"+BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY+"'";
			}else if(flag.equals(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY)){
				sqlWhere=" and S_operationtypecode= '"+BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY+"'";
			}
...
//			refdto.setSoperationtypecode(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY);
			try {
				...
				if (showfilelist != null && showfilelist.size() > 0) {
					...
					MessageDialog.openMessageDialog(null, "划款申请不存在未销号数据!");
				}
			} catch (ITFEBizException e) {
				log.error(e);
				MessageDialog.openErrorDialog(null, e);
	...
	public String getBiztype(){
    	...
    	if(null!=list&&list.size()>0){
    		if(list.contains(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY)){
        		flag.append(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY);
        	}
        	if(list.contains(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY)){
        		flag.append(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY);
        	}
    	}else{
    		flag.append(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_DIRECT_PAY)
    		.append(BizTypeConstant.BIZ_TYPE_PAYRECK_BANK_GRANT_PAY);
    	}
	...
	public PayreckBankSubBean getSubBean() {
		return subBean;
	}

	public void setSubBean(PayreckBankSubBean subBean) {
