package com.cfcc.itfe.client.sendbiz.directpayreturnreceipt;
...
import com.cfcc.itfe.client.sendbiz.bizcertsend.ActiveXCompositeDirectPayReturnOcx;
 ...
 * 
 * @author zhangliang
 * @time 14-10-31 17:56:55 子系统: SendBiz 模块:directPayReturnReceipt
 *       组件:DirectPayReturnReceipt
 */
@SuppressWarnings("unchecked")
public class DirectPayReturnReceiptBean extends
		AbstractDirectPayReturnReceiptBean implements IPageDataProvider {

	private static Log log = LogFactory
			.getLog(DirectPayReturnReceiptBean.class);
	...
	public DirectPayReturnReceiptBean() {
		super();
	...
	public String voucherGenerator(Object o) {
		...
		if (dto.getScheckdate() == null || "".equals(dto.getScheckdate())) {
			MessageDialog.openMessageDialog(null, "清算日期不可为空！");
			return "";
		}
		if (StringUtils.isBlank(dto.getSvtcode())) {
			dto.setSvtcode(MsgConstant.VOUCHER_NO_2301);
			dto.setShold1("1");
		}
		int count = 0;
		try {
	...
	public String voucherView(Object o) {
		...
		try {
			ActiveXCompositeDirectPayReturnOcx.init(0);
	...
	private String voucherIsRepeat(TvVoucherinfoDto voucherDto)
			throws ITFEBizException {
		if (MsgConstant.VOUCHER_NO_5201.equals(voucherDto.getSvtcode())) {
			voucherDto.setSvtcode(MsgConstant.VOUCHER_NO_2301);
			voucherDto.setShold1("2");// 单笔批量标示2批量
		}
		List<TvVoucherinfoDto> list = getVoucherList(voucherDto);
		if (list == null || list.size() == 0) {
			return "";
	...
	public PageResponse retrieve(PageRequest pageRequest) {
		...
		wheresql.append(" 1=1 AND S_PAYBANKCODE IS  NULL  ");
					...
					if (vDto.getStrecode().equals(strecode)) {
						...
						if (seq != null && !seq.equals("")) {
							...
							if (seqList != null && seqList.size() > 0) {
								...
								ActiveXCompositeDirectPayReturnOcx
						...
						if (stamp.equals(MsgConstant.VOUCHERSAMP_OFFICIAL)) {
							if (!voucherLoadService.getOfficialStamp().equals(
								...
								sinList.add(ActiveXCompositeDirectPayReturnOcx
	...
	public static void setLog(Log log) {
		DirectPayReturnReceiptBean.log = log;
	}
	...
	public void setVoucherType(String voucherType) {
		...
		this.dto
				.setShold1(voucherType.indexOf(MsgConstant.VOUCHER_NO_2301) >= 0 ? voucherType
						.substring(4)
						: null);
	...
	public List getSubVoucherList() {
		if (subVoucherList == null || subVoucherList.size() <= 0) {
			...
			Mapper map = new Mapper(MsgConstant.VOUCHER_NO_2301 + "1",
					"直接支付清算-单笔(2301)");
			Mapper map1 = new Mapper(MsgConstant.VOUCHER_NO_2301 + "4",
					"直接支付清算-批量(2301)");
			Mapper map3 = new Mapper(MsgConstant.VOUCHER_NO_2302,
					"直接支付清算-退回(2302)");
