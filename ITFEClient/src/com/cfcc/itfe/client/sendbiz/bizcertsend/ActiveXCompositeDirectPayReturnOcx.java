...
import com.cfcc.itfe.client.sendbiz.directpayreturnreceipt.DirectPayReturnReceiptBean;
...
public class ActiveXCompositeDirectPayReturnOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeDirectPayReturnOcx.class);
	// ole控件对象
	...
	public static String modelId = "f72d7ae6-83bc-4e3a-bb8c-5ee562a420a3";
	/**
	 * 生成签章报文（读取UKEY印章）
	 * 
	 * @param dto
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
				voucherXML = ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=0;
		count = ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}		
		String printCounts= "";
		printCounts=((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);	
		...
		if (i == 0) {
			try {
				i = AddVoucherXmls(((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeDirectPayReturnOcx(Composite parent) {
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeDirectPayReturnOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeDirectPayReturnOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeDirectPayReturnOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	return v.getInt();
	}
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getDto(dto);
		...
		String printCount="";
		printCount = ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).queryVoucherPrintCount(dto);
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				MVCUtils.openEditor("直接支付清算回单");
				MVCUtils.openComposite("直接支付清算回单", "维护界面");
				((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getCheckList().clear();
				((AbstractMetaDataEditorPart) ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getModelHolder()).fireModelChanged(ModelChangedEvent.REFRESH_ALL_EVENT);
				ls_AllCount = "";
				listCount = 0;
				nowCount.setText("" + (listCount + 1));
			} catch (Exception ex) {
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				String stamptCounts= ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					String returnmsg = ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
					MessageDialog.openMessageDialog(null, returnmsg);
				}else if(stamptCounts.equals("1")){
					String demo = ((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((DirectPayReturnReceiptBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeDirectPayReturnOcx.stampName = stampName;
	}
	...
	public static void setModelId(String modelId) {
		ActiveXCompositeDirectPayReturnOcx.modelId = modelId;
	}
}