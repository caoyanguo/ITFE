...
import com.cfcc.itfe.client.recbiz.payoutreport.PayoutReportBean;
...
public class ActiveXCompositePayoutReportOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositePayoutReportOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((PayoutReportBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((PayoutReportBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=((PayoutReportBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((PayoutReportBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}		
		String printCounts= ((PayoutReportBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((PayoutReportBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositePayoutReportOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("284bc456-3343-4ce2-ba93-065944908213");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositePayoutReportOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositePayoutReportOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositePayoutReportOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((PayoutReportBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((PayoutReportBean) context.getModelHolder().getModel()).getDto(dto);
		...
		String printCount=((PayoutReportBean) context.getModelHolder().getModel()).queryVoucherPrintCount(dto);
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_262", "支出报表凭证", "授权打印凭证", msg)){
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("支出报表业务处理");
				MVCUtils.openComposite("支出报表业务处理", "维护界面");
				((PayoutReportBean) context.getModelHolder().getModel()).getCheckList().clear();
				((AbstractMetaDataEditorPart) ((PayoutReportBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((PayoutReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((PayoutReportBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					MessageDialog.openMessageDialog(null, ((PayoutReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
	...
	public static void setStampName(String stampName) {
		ActiveXCompositePayoutReportOcx.stampName = stampName;
	}
}