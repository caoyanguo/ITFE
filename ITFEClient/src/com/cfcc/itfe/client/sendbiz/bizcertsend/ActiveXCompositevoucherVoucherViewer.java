...
import com.cfcc.itfe.client.dataquery.businessdetaillist.BusinessDetailListBean;
import com.cfcc.itfe.client.dialog.AdminConfirmDialogFacade;
import com.cfcc.itfe.client.dialog.DisplayCursor;
import com.cfcc.itfe.client.recbiz.vouchercheckaccountquery.VoucherCheckAccountQueryBean;
...
public class ActiveXCompositevoucherVoucherViewer extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositevoucherVoucherViewer.class);
	// ole控件对象
	...
	public static void init(int num) {		
		voucherDtoList = ((BusinessDetailListBean) context.getModelHolder().getModel()).getVoucherViewersList();
		...
		int count=((BusinessDetailListBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		String printCounts= ((BusinessDetailListBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
	 ...
	 * @param parent
	 */
	public ActiveXCompositevoucherVoucherViewer(Composite parent) {
...
//		btnPrintAll.addSelectionListener(new BtnPrintAll());		
		...
		context = DataBindingContext
				.createInstance("4e6fed82-183d-4d8a-99af-74201e2e5e9d");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositevoucherVoucherViewer.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositevoucherVoucherViewer.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositevoucherVoucherViewer.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((VoucherCheckAccountQueryBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((VoucherCheckAccountQueryBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((VoucherCheckAccountQueryBean) context.getModelHolder()
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_257", "接收每日对账", "授权打印凭证", msg)){
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("无纸化业务清单打印");
				MVCUtils.openComposite("无纸化业务清单打印", "原始凭证打印统计");
//				((BusinessDetailListBean) context.getModelHolder().getModel())
//						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((BusinessDetailListBean) context
