...
import com.cfcc.itfe.client.recbiz.basedata.BasedataBean;
...
public class ActiveXCompositeBaseDataOcx extends Composite {

	private static Log logger = LogFactory
			.getLog(ActiveXCompositeBaseDataOcx.class);
	// ole控件对象
	...
	private ITFELoginInfo loginInfo = (ITFELoginInfo) ApplicationActionBarAdvisor
			.getDefault().getLoginInfo();
	// 报表显示区域
	ReportComposite report;
	public static String modelId = "3c829b1c-d3dd-4ce7-9270-4921caf21b34";
	 ...
	 */
	public static String getVoucherStamp(TvVoucherinfoDto vDto, String certID,
			String stampPosition, String stampID) {
		String voucherStampXML = "";
		String stamp = "";
		String err = null;
		String voucherXML = "";
		try {
			voucherXML = ((BasedataBean) context.getModelHolder().getModel())
					.getVoucherXMl(vDto);
			;
			if (!vDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3513)
					&& !vDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3512)
					&& !vDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3511)) {
		...
		voucherStampXML = getVoucherStamp(certID, vDto.getSadmdivcode(),
				Integer.parseInt(vDto.getSstyear()), vDto.getSvtcode(), stamp,
				voucherXML);
	...
	public static String getVoucherStamp(String certID, String admDivCode,
		...
		Variant v = execActivexMethod(OcxConstant.GETVOUCHERSTAMP,
	...
	public static void init(int num) {
		...
		voucherDtoList = ((BasedataBean) context.getModelHolder().getModel())
				.getCheckList();
		...
		count = ((BasedataBean) context.getModelHolder().getModel())
		...
		retList = ((BasedataBean) context.getModelHolder().getModel())
				.getStampEnums(infoDto);

		for (Mapper map : retList) {
			comboStampType.add(map.getDisplayValue().toString());
		}
		String printCounts = "";
		printCounts = ((BasedataBean) context.getModelHolder().getModel())
				.queryVoucherPrintCount(infoDto);
		printCount
				.setText(printCounts.equals("-1") == true ? "0" : printCounts);
		int i = -1;
		err = null;
		...
		if (i == 0) {
			try {
				i = AddVoucherXmls(((BasedataBean) context.getModelHolder()
						.getModel()).getVoucherXMl(infoDto));
			} catch (Exception e) {
				logger.error(e);
				Exception e1 = new Exception("组装凭证报文出现异常！", e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeBaseDataOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		_frame.setBounds(6, 60, 1324, 750);
		...
		ReportComposite report = new ReportComposite(composite,
				JasperConstants.ALL);
		report.setBounds(6, 60, 1324, 750);
		// 设置背景颜色，和ActiveX控件一致
		...
		new Thread() {
			public void run() {
				...
				Variant v1 = execActivexMethod(OcxConstant.SETURL,
						new Object[] { voucherUrl
								+ OcxConstant.EVOUCHERSERVICEURL });
				if (v1.getInt() != 0) {
					logger.error("初始化电子凭证库服务地址失败!");
				}
				Variant v2 = execActivexMethod(
						OcxConstant.SETEstampURL,
						new Object[] { stampUrl
								+ (loginInfo.getPublicparam().contains(
										",ocxnewinterface=true,") ? OcxConstant.ESTAMPSERVICEURL
										.replace("realware", "estamp")
										: OcxConstant.ESTAMPSERVICEURL) });
				if (v2.getInt() != 0) {
	...
	public static int initStampOcx(String certId, String rgCode, long year,
		...
		// 调用ocx的初始化方法
		Variant v = execActivexMethod(OcxConstant.INITOCX, new Object[] {
				certId, rgCode, new Long(year), voucherType, stepCode,
				new Long(operateType), new Long(defaultPage),
	...
	public static String GetLastErr() {
		Variant v = execActivexMethod(OcxConstant.GetLastErr, new Object[] {});
		return v.getString();
	}
	...
	public void setUrl(String url) {
		...
		// 调用ocx的初始化方法
		execActivexMethod(OcxConstant.SETURL, new Object[] { url });
	}
	...
	public boolean addVoucher(String voucherNO, String xmlBody) {
		// 调用ocx添加凭证的方法
		Variant v = execActivexMethod(OcxConstant.ADDVOUCHER, new Object[] {
				voucherNO, xmlBody });
		// 0表示成功，-1表示失败
		if (v.getLong() == 0) {
			return true;
	...
	public static int addVoucher(String voucherNO) {
		// 调用ocx添加凭证的方法
		Variant v = execActivexMethod(OcxConstant.ADDVOUCHERFROMSERVER,
				new Object[] { voucherNO });
		// 0表示成功，-1表示失败
		return v.getInt();
	}
	...
	public boolean clearVouchers() {
		// 调用ocx添加凭证的方法
		Variant v = execActivexMethod(OcxConstant.CLEARVOUCHER, null);
		// 0表示成功，-1表示失败
		if (v.getLong() == 0) {
			return true;
		} else {
	...
	public static int setCurrentVoucher(String voucherNO) {
		// 设置当前显示的方法
		Variant v = execActivexMethod(OcxConstant.SETCURRENTVOUCHER,
	...
	public static int AddVoucherXmls(String XmlVouchers) {
		// 设置当前显示的方法
		Variant v = execActivexMethod("AddVoucherXmls", new String[] {
				XmlVouchers, "0" });
		// 0表示成功，-1表示失败
		return v.getInt();
	}
	...
	public String getSignedVouchers() {
		// 设置当前显示的方法
		Variant v = execActivexMethod(OcxConstant.GETSIGNEDVOUCHERS, null);
		// 0表示成功，-1表示失败
		if (null == v) {
			return null;
		} else {
	...
	public String getOperateVouchers(String certId, String rgCode,
		...
		// 执行ocx对操作报文的签名
		Variant v = execActivexMethod(OcxConstant.GETOPERATEVOUCHRS,
				new Object[] { certId, rgCode, new Long(setYear), voucherType,
						voucherNO, opCode });
		// 0表示成功，-1表示失败
		if (null == v) {
			return null;
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeBaseDataOcx.execActivexMethod(
				OcxConstant.PRINTONEVOUCHER, new Object[] { voucherNo,
						new Long(joint) });
		return v.getInt();
	}
	...
	public int pageDown() {
		Variant v = ActiveXCompositeBaseDataOcx.execActivexMethod(
				OcxConstant.PAGEDOWN, new Object[] {});
		return v.getInt();
	}
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				...
				voucherDtoList = ((BasedataBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto) {
		if (!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)
				...
				|| dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_5502) || dto
				.getSvtcode().equals(MsgConstant.VOUCHER_NO_2502))) {
			try {

				dto = ((BasedataBean) context.getModelHolder().getModel())
						.getDto(dto);
			...
			if (loginInfo.getPublicparam().contains(",notverifyprint,")) {
				if (dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3513)
						|| dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3512)
						|| dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3511)) {
					if (dto.getSstatus().equals(
							DealCodeConstants.VOUCHER_SUCCESS_NO_BACK)
							&& (dto.getSstampid() == null || dto.getSstampid()
									.equals(""))) {
						MessageDialog.openMessageDialog(null, "凭证类型："
								+ dto.getSvtcode() + " 且未签章的凭证不能打印！");
						return false;
					}
				} else {
					if (!dto.getSstatus().equals(
						...
						MessageDialog.openMessageDialog(null, "凭证类型："
								+ dto.getSvtcode() + " 且未发送电子库成功的凭证不能打印！");
						return false;
					}
				}
			}
		...
		printCount = ((BasedataBean) context.getModelHolder().getModel())
				.queryVoucherPrintCount(dto);

		if (printCount.indexOf("异常") > 0) {
			MessageDialog.openMessageDialog(null, "凭证编号:  "
					+ dto.getSvoucherno() + "查询打印次数异常，不能打印！");
			return false;
		}
		int i = 1;
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				...
				MVCUtils.openEditor("基础数据元数据");
				MVCUtils.openComposite("基础数据元数据", "基础数据元数据");
				((BasedataBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((BasedataBean) context
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				...
				if ((stampNameCace == null || stampNameCace.equals(""))
						&& (stampName == null || stampName.equals(""))) {
				...
				list.add(((BasedataBean) context.getModelHolder().getModel())
						.getDto(voucherDtoList.get(listCount)));
				String stamptCounts = ((BasedataBean) context.getModelHolder()
						.getModel()).voucherStamp(list);
				if (stamptCounts.equals("0")) {
					// MessageDialog.openMessageDialog(null,
					// ((VoucherReportBean)
					// context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
					String returnmsg = ((BasedataBean) context.getModelHolder()
							.getModel()).getDto(voucherDtoList.get(listCount))
							.getSreturnerrmsg();
					((BasedataBean) context.getModelHolder().getModel())
							.getCheckList().get(listCount).setSreturnerrmsg(
									returnmsg);
					...
					String demo = ((BasedataBean) context.getModelHolder()
							.getModel()).getDto(voucherDtoList.get(listCount))
							.getSdemo();
					((BasedataBean) context.getModelHolder().getModel())
							.getCheckList().get(listCount).setSstatus(
									DealCodeConstants.VOUCHER_STAMP);
					((BasedataBean) context.getModelHolder().getModel())
							.getCheckList().get(listCount).setSdemo(demo);
					MessageDialog.openMessageDialog(null, stampName + "签章成功！");
				}
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeBaseDataOcx.stampName = stampName;
	}
	...
	public static void setModelId(String modelId) {
		ActiveXCompositeBaseDataOcx.modelId = modelId;
	}
}