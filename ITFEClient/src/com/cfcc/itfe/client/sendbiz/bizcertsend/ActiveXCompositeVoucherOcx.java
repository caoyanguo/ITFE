...
public class ActiveXCompositeVoucherOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((VoucherLoadBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	 ...
	 * @throws FileOperateException 
	 */
	public static String getVoucherSign(TvVoucherinfoDto vDto,String certID,String stampPosition)  {
		String voucherStampXML="";
		String signInfo="";
		String err=null;
		String voucherXML="";
		try {
			voucherXML = ((VoucherLoadBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
			logger.debug("凭证签名写入的签名报文："+voucherXML);
		} catch (Exception e) {
			logger.error(e);
			err="凭证报文解析异常";
			return err+"@";
		...
		try {
			signInfo = getStampPotisionXML(stampPosition, certID);
			logger.debug("凭证签章写入的签章位置报文："+signInfo);
		} catch (Exception e) {
			logger.error(e);
			err="组装签章位置报文异常";
			return err+"@";
		...
		voucherStampXML=getVoucherSign(certID, vDto.getSadmdivcode(), Integer.parseInt(vDto.getSstyear()), vDto.getSvtcode(), signInfo, voucherXML);
		if(voucherStampXML==null||voucherStampXML.equals("")){
			 err = GetLastErr();
			if (err != null && !err.equals("")) {
					return err+"@";
	 ...
	 * @return
	 */
	public static String getVoucherSign(String certID, String admDivCode,
			long stYear, String vtCode, String certInfo, String voucherXML) {
		Variant v = execActivexMethod(
				OcxConstant.GETVOUCHERSIGN, new Object[] {
						certID, admDivCode, new Long(stYear), vtCode, certInfo,
						voucherXML });
		if (null == v) {
			return null;
		} else {
	 ...
	 * @param num
	 */
	public static void init(int num,boolean isaddvoucher) {
		ITFELoginInfo loginInfo = (ITFELoginInfo)ApplicationActionBarAdvisor.getDefault().getLoginInfo();
		if(loginInfo.getPublicparam().contains(",printpagestame=true,")&&isaddvoucher)
		{
			execActivexMethod("ResetContent",new Object[]{});
		}
		comboStampType.removeAll();
		voucherDtoList = ((VoucherLoadBean) context.getModelHolder().getModel())
				.getCheckList();
		...
		if(loginInfo!=null&&loginInfo.getPublicparam().indexOf(",display=true,")>=0)
		{
			if(!"display".equals(infoDto.getSext4())&&(infoDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||infoDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||infoDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_5207)))
			{
				ICommonDataAccessService commonDataAccessService = (ICommonDataAccessService)((VoucherLoadBean) context.getModelHolder().getModel()).getService(ICommonDataAccessService.class);
				infoDto.setSext4("display");//是否还原展示
				try {
					TvVoucherinfoPK qpk = new TvVoucherinfoPK();
					qpk.setSdealno(infoDto.getSdealno());
					TvVoucherinfoDto stv = (TvVoucherinfoDto) commonDataAccessService.find(qpk);
					if(stv!=null&&infoDto.getSstatus().equals(stv.getSstatus()))
						commonDataAccessService.updateData(infoDto);
				} catch (ITFEBizException e) {
				}
			}
		}
		if(loginInfo!=null&&loginInfo.getPublicparam().indexOf(",display=all,")>=0)
		{
			if(!"display".equals(infoDto.getSext4())&&(infoDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_5106)||infoDto.getSvtcode().equals(MsgConstant.VOUCHER_NO_5108)))
			{
				ICommonDataAccessService commonDataAccessService = (ICommonDataAccessService)((VoucherLoadBean) context.getModelHolder().getModel()).getService(ICommonDataAccessService.class);
				infoDto.setSext4("display");//是否还原展示
				try {
		...
		int count=((VoucherLoadBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((VoucherLoadBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}
		if(comboStampType.getItemCount()==1){
			comboStampType.select(0);
		}
		String printCounts= ((VoucherLoadBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
					...
					public void run() {
						...
						for(List list:(List<List>)lists)
						{
							if(list!=null&&list.size()>1)
							{
								...
								{
									dto = (TvVoucherinfoDto)list.get(i);
									addVoucher(dto.getSvoucherno());
								}
							}
						}
					}
...
//				ZoomToFit();
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("99d5a4d2-d6b4-4f44-8007-961c083cab09");
	...
	//			MessageDialog.openMessageDialog(null, "enter3............."+params.length);
			}
			...
			variant = _auto.invoke(activeXMethodID, paramVariant);// VT_144{999}
		}catch(Exception e)
		{
			MessageDialog.openMessageDialog(null, "调用凭证库异常:"+e.toString());
		}
		return variant;
	}
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherOcx.execActivexMethod(
	...
	public int printVoucher(String certID, String admDivCode, int stYear,
			String vtCode, String voucherNo,List list) {
		certID = "001";
		
		Variant v = null;
		if(loginInfo.getPublicparam().contains(",printpagestame=true,")&&list!=null&&list.size()>0)
			v = ActiveXCompositeVoucherOcx.execActivexMethod("printvoucherbyindexrange", new Object[] { new Long(0),new Long((list.size()-1)), new Long(joint)});
		else
			v = ActiveXCompositeVoucherOcx.execActivexMethod(OcxConstant.PRINTALLVOUCHER, new Object[] { certID, admDivCode,new Long(stYear), vtCode, new Long(joint), voucherNo });;
		return v.getInt();
	}
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((VoucherLoadBean) context.getModelHolder()
				...
				if (i == 0) {
					...
					if(loginInfo.getPublicparam().contains(",printcount=true,"))
					{
						...
						try {
							...
							if(stv!=null)
							{
								stv.setSext3(infoDto.getSext3());
								commonDataAccessService.updateData(stv);
							}
						} catch (ITFEBizException ex) {
						}
					}
				} else {
	...
	private class BtnPrintAll extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				...
				ICommonDataAccessService commonDataAccessService = null;
				...
				for(List list:(List<List>)lists){
					...
					// 打印完毕后，当前按钮失效，防止重复打印
					int i = printVoucher(null, vDto.getSadmdivcode(), Integer.parseInt(vDto.getSstyear()), vDto.getSvtcode(),voucherNo,list);
					if (i == 0) {
						j++;
						if(lists.size()==j){
							...
							if(loginInfo.getPublicparam().contains(",printcount=true,"))
							{
								if(commonDataAccessService==null)
									commonDataAccessService = (ICommonDataAccessService)((VoucherLoadBean) context.getModelHolder().getModel()).getService(ICommonDataAccessService.class);
								try {
									List<TvVoucherinfoDto> updatelist = new ArrayList<TvVoucherinfoDto>();
									TvVoucherinfoDto updto = null;
									for(int up=0;up<list.size();up++)
									{
										TvVoucherinfoPK qpk = new TvVoucherinfoPK();
										qpk.setSdealno(((TvVoucherinfoDto)list.get(up)).getSdealno());
										updto = (TvVoucherinfoDto) commonDataAccessService.find(qpk);
										if(updto!=null)
										{
											updto.setSext3(((TvVoucherinfoDto)list.get(up)).getSext3());
											updatelist.add(updto);
										}
									}
						...
						String err = GetLastErr();
						if (err != null && !err.equals("")) {
							MessageDialog.openMessageDialog(null, "凭证打印失败："+ err);
						} else {
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((VoucherLoadBean) context.getModelHolder()
					.getModel()).getDto(dto);
			} catch (ITFEBizException e) {
				logger.error(e);				
			...
			if(loginInfo.getPublicparam().contains(",notverifyprint,"))
			{
				if(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3401)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3402)||
					dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3208)){
					if(dto.getSstatus().equals(DealCodeConstants.VOUCHER_SUCCESS_NO_BACK)&&(dto.getSstampid()==null||dto.getSstampid().equals(""))){
						MessageDialog.openMessageDialog(null, "凭证类型："+dto.getSvtcode()+" 且未签章的凭证不能打印！");
						return false;							
					}				
				}else {
					//如果凭证5253直接支付调整、5351授权支付调整、8207财政支付批量信息和2252商业银行退款通知时 ，凭证直接打印在TC进行录入
					if(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_5253) || dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_5351) || dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_8207) || dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2252)){
						
					}else if(!dto.getSstatus().equals(DealCodeConstants.VOUCHER_SUCCESS)){
						MessageDialog.openMessageDialog(null, "凭证类型："+dto.getSvtcode()+" 且未发送电子库成功的凭证不能打印！");
						return false;	
					}
					/* 源码
		...
		String printCount=((VoucherLoadBean) context.getModelHolder()
				.getModel()).queryVoucherPrintCount(dto);
		...
		if(loginInfo.getPublicparam().contains(",printcount=true,"))
			dto.setSext3((Integer.parseInt(printCount)+1)+"次");//打印次数
		return true;
	}
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_018", "业务凭证处理", "授权打印凭证", msg)){
...
//				((VoucherLoadBean) context.getModelHolder().getModel()).getCheckList().clear();
//				((VoucherLoadBean) context.getModelHolder().getModel()).refreshTable();
				ls_AllCount = "";
				listCount = 0;
				nowCount.setText("" + (listCount + 1));
			} catch (Exception ex) {
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((VoucherLoadBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((VoucherLoadBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					String returnmsg = ((VoucherLoadBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((VoucherLoadBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
					MessageDialog.openMessageDialog(null, returnmsg);
				}else if(stamptCounts.equals("1")){
					String demo = ((VoucherLoadBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((VoucherLoadBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((VoucherLoadBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
