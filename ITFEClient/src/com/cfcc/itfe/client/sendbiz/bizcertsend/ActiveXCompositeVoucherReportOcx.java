...
import com.cfcc.itfe.client.recbiz.voucherreport.VoucherReportBean;
...
public class ActiveXCompositeVoucherReportOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherReportOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((VoucherReportBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((VoucherReportBean) context.getModelHolder().getModel())
				.getCheckList();
		...
		int count=((VoucherReportBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((VoucherReportBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}		
		String printCounts= ((VoucherReportBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((VoucherReportBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherReportOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("a8bbf8a5-b163-48da-a396-84b0eefd8a12");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherReportOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherReportOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherReportOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((VoucherReportBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((VoucherReportBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((VoucherReportBean) context.getModelHolder()
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("报表凭证处理");
				MVCUtils.openComposite("报表凭证处理", "维护界面");
				((VoucherReportBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((VoucherReportBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((VoucherReportBean) context.getModelHolder().getModel()).voucherStamp(list);
...
//					MessageDialog.openMessageDialog(null, ((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
					String returnmsg = ((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((VoucherReportBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
...
//					MessageDialog.openMessageDialog(null, stampName+"签章成功！");
					String demo = ((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((VoucherReportBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((VoucherReportBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
					MessageDialog.openMessageDialog(null, stampName+"签章成功！");
				}init(listCount);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeVoucherReportOcx.stampName = stampName;
	}
}