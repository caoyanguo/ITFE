...
import com.cfcc.itfe.client.sendbiz.annualreportincome.AnnualReportIncomeBean;
...
public class ActiveXCompositeVoucherAnnualReportOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherAnnualReportOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((AnnualReportIncomeBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((AnnualReportIncomeBean) context.getModelHolder().getModel())
				.getCheckList();
		...
		int count=((AnnualReportIncomeBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((AnnualReportIncomeBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}		
		String printCounts= ((AnnualReportIncomeBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((AnnualReportIncomeBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherAnnualReportOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("32a8f353-109a-4c70-9554-52214be47d4f");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherAnnualReportOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherAnnualReportOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherAnnualReportOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((AnnualReportIncomeBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((AnnualReportIncomeBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((AnnualReportIncomeBean) context.getModelHolder()
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("各地收入年报表");
				MVCUtils.openComposite("各地收入年报表", "生成凭证报表界面");
				((AnnualReportIncomeBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((AnnualReportIncomeBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((AnnualReportIncomeBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((AnnualReportIncomeBean) context.getModelHolder().getModel()).voucherStamp(list);
...
//					MessageDialog.openMessageDialog(null, ((AnnualReportIncomeBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
					String returnmsg = ((AnnualReportIncomeBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((AnnualReportIncomeBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
...
//					MessageDialog.openMessageDialog(null, stampName+"签章成功！");
					String demo = ((AnnualReportIncomeBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((AnnualReportIncomeBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((AnnualReportIncomeBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
					MessageDialog.openMessageDialog(null, stampName+"签章成功！");
				}init(listCount);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeVoucherAnnualReportOcx.stampName = stampName;
	}
}