...
import com.cfcc.itfe.client.recbiz.voucherallocateincomegenerate.VoucherAllocateIncomeGenerateBean;
...
public class ActiveXCompositeVoucherAllocateIncomeGenerateOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherAllocateIncomeGenerateOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		...
		String printCounts= ((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherAllocateIncomeGenerateOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("2556facb-05a5-4cc6-a12e-c28ba3d09b31");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherAllocateIncomeGenerateOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherAllocateIncomeGenerateOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherAllocateIncomeGenerateOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((VoucherAllocateIncomeGenerateBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((VoucherAllocateIncomeGenerateBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((VoucherAllocateIncomeGenerateBean) context.getModelHolder()
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_265", "国库往来票据生成", "授权打印凭证", msg)){
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("国库往来票据生成");
				MVCUtils.openComposite("国库往来票据生成", "维护界面");
				((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((VoucherAllocateIncomeGenerateBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					String returnmsg = ((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
					MessageDialog.openMessageDialog(null, returnmsg);
				}else if(stamptCounts.equals("1")){
					String demo = ((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((VoucherAllocateIncomeGenerateBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeVoucherAllocateIncomeGenerateOcx.stampName = stampName;
	}
}