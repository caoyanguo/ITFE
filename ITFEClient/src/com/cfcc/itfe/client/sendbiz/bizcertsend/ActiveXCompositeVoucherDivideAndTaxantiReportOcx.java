...
import com.cfcc.itfe.client.sendbiz.divideandtaxanti.DivideAndTaxantiBean;
...
public class ActiveXCompositeVoucherDivideAndTaxantiReportOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherDivideAndTaxantiReportOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((DivideAndTaxantiBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((DivideAndTaxantiBean) context.getModelHolder().getModel())
				.getCheckList();
		...
		int count=((DivideAndTaxantiBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((DivideAndTaxantiBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}		
		String printCounts= ((DivideAndTaxantiBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((DivideAndTaxantiBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherDivideAndTaxantiReportOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("d7d289b8-63d8-46ed-b196-57c7f4d40e8f");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherDivideAndTaxantiReportOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherDivideAndTaxantiReportOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherDivideAndTaxantiReportOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((DivideAndTaxantiBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((DivideAndTaxantiBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((DivideAndTaxantiBean) context.getModelHolder()
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("总额分成、两税返还报表生成");
				MVCUtils.openComposite("总额分成、两税返还报表生成", "生成凭证报表界面");
				((DivideAndTaxantiBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((DivideAndTaxantiBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((DivideAndTaxantiBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((DivideAndTaxantiBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
//					MessageDialog.openMessageDialog(null, ((AnnualReportIncomeBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
					String returnmsg = ((DivideAndTaxantiBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((DivideAndTaxantiBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
...
//					MessageDialog.openMessageDialog(null, stampName+"签章成功！");
					String demo = ((DivideAndTaxantiBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((DivideAndTaxantiBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((DivideAndTaxantiBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
					MessageDialog.openMessageDialog(null, stampName+"签章成功！");
				}init(listCount);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeVoucherDivideAndTaxantiReportOcx.stampName = stampName;
	}
}