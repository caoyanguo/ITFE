...
import com.cfcc.itfe.client.sendbiz.stockaccountvoucher.StockAccountVoucherForMonthBean;
...
public class ActiveXCompositeVoucherStockAccountOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherStockAccountOcx.class);
	// ole控件对象
	...
	public static void init(int num) {		
		voucherDtoList = ((StockAccountVoucherForMonthBean) context.getModelHolder().getModel())
				.getCheckList();
...
//		Refresh (infoDto.getSvoucherno());
		int count=((StockAccountVoucherForMonthBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		String printCounts= ((StockAccountVoucherForMonthBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((StockAccountVoucherForMonthBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));
			} catch (Exception e) {
...
//				Refresh (infoDto.getSvoucherno());
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherStockAccountOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("e02ac3c9-23b4-4afc-9b33-48d4490aec49");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherStockAccountOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherStockAccountOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherStockAccountOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((StockAccountVoucherForMonthBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((StockAccountVoucherForMonthBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((StockAccountVoucherForMonthBean) context.getModelHolder()
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_266", "库款账户月度对账", "授权打印凭证", msg)){
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("发起库款账户月度对账3552");
				MVCUtils.openComposite("发起库款账户月度对账3552", "维护界面");
				((StockAccountVoucherForMonthBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((StockAccountVoucherForMonthBean) context
