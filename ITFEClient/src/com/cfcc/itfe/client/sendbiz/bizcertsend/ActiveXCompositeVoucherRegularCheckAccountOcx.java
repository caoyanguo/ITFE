...
import com.cfcc.itfe.client.recbiz.vouchercheckaccount.VoucherRegularCheckAccountBean;
...
public class ActiveXCompositeVoucherRegularCheckAccountOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherRegularCheckAccountOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = getVoucherStampXML(vDto);
			logger.debug("凭证签章写入的签章报文："+stamp);
		} catch (Exception e) {
	 ...
	 * 
	 */
	public static String getVoucherStampXML(TvVoucherinfoDto dto )throws Exception {
		...
		Voucher.setText(base64Encode(com.cfcc.itfe.util.FileUtil.getInstance().readFile(dto.getSfilename())));
		
		return successDoc.asXML().replaceAll("&lt;", "<")
		.replaceAll("&gt;", ">");
	}
	...
	public static void init(int num) {	
		...
		voucherDtoList = ((VoucherRegularCheckAccountBean) context.getModelHolder().getModel())
				.getCheckList();
		...
		int count=((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		...
		String printCounts= ((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {
				i = AddVoucherXmls(((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherRegularCheckAccountOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("3530909e-1b1e-460c-b9ee-dcf2cdc382aa");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherRegularCheckAccountOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherRegularCheckAccountOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherRegularCheckAccountOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((VoucherRegularCheckAccountBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((VoucherRegularCheckAccountBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((VoucherRegularCheckAccountBean) context.getModelHolder()
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_246", "定期对账", "授权打印凭证", msg)){
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("业务对账");
				MVCUtils.openComposite("业务对账", "维护界面");
				((VoucherRegularCheckAccountBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((VoucherRegularCheckAccountBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					String returnmsg = ((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
					MessageDialog.openMessageDialog(null, returnmsg);
				}else if(stamptCounts.equals("1")){
					String demo = ((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((VoucherRegularCheckAccountBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
