...
import com.cfcc.itfe.client.sendbiz.newbusinessreconciliation.NewBusinessReconciliationBean;
...
public class ActiveXCompositeNewSendBussOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeNewSendBussOcx.class);
	// ole控件对象
	...
	public static String modelId = "bd6026ce-cbb9-4116-968a-a76d88ad81e0";
	/**
	 * 生成签章报文（读取UKEY印章）
	 * 
	 * @param dto
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
				voucherXML = ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
			voucherDtoList = ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=0;
		if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
			count = ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
			retList=((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		...
		String printCounts= "";
		if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
			printCounts=((NewBusinessReconciliationBean)context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);
		...
		if (i == 0) {
			try {
				if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
					i = AddVoucherXmls(((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeNewSendBussOcx(Composite parent) {
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeNewSendBussOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeNewSendBussOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeNewSendBussOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	return v.getInt();
	}
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
				{
					dto=((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(dto);
				}
		...
		if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
		{
			printCount = ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).queryVoucherPrintCount(dto);
		}
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
				{
					...
					((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().clear();
					((AbstractMetaDataEditorPart) ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getModelHolder()).fireModelChanged(ModelChangedEvent.REFRESH_ALL_EVENT);
				}
				ls_AllCount = "";
				listCount = 0;
				nowCount.setText("" + (listCount + 1));
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				if(context.getModelHolder().getModel() instanceof NewBusinessReconciliationBean)
				{
					list.add(((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
					String stamptCounts= ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).voucherStamp(list);
...
//						MessageDialog.openMessageDialog(null, ((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
						String returnmsg = ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
						((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
...
//						MessageDialog.openMessageDialog(null, stampName+"签章成功！");
						String demo = ((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
						((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
						((NewBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
						MessageDialog.openMessageDialog(null, stampName+"签章成功！");
					}
				}
				init(listCount);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeNewSendBussOcx.stampName = stampName;
	}
	...
	public static void setModelId(String modelId) {
		ActiveXCompositeNewSendBussOcx.modelId = modelId;
	}
}