...
import com.cfcc.itfe.client.sendbiz.sendbusinessreconciliation.SendBusinessReconciliationBean;
import com.cfcc.itfe.client.sendbiz.sendreportreconciliation.SendReportReconciliationBean;
...
public class ActiveXCompositeReconciliationOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeReconciliationOcx.class);
	// ole控件对象
	...
	public static String modelId = "cc26da80-cc2d-444c-93d7-ee9c598f88da";
	/**
	 * 生成签章报文（读取UKEY印章）
	 * 
	 * @param dto
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
				voucherXML = ((SendReportReconciliationBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
			else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
				voucherXML = ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);;
	...
	public static void init(int num) {
		...
		if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
			voucherDtoList = ((SendReportReconciliationBean) context.getModelHolder().getModel()).getCheckList();
		else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
			voucherDtoList = ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=0;
		if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
			count = ((SendReportReconciliationBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
			count = ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
			retList=((SendReportReconciliationBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
			retList=((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		...
		String printCounts= "";
		if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
			printCounts=((SendReportReconciliationBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);
		else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
			printCounts=((SendBusinessReconciliationBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);	
		...
		if (i == 0) {
			try {
				if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
					i = AddVoucherXmls(((SendReportReconciliationBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));
				else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
					i = AddVoucherXmls(((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeReconciliationOcx(Composite parent) {
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeReconciliationOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeReconciliationOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeReconciliationOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	return v.getInt();
	}
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
				{
					dto=((SendReportReconciliationBean) context.getModelHolder().getModel()).getDto(dto);
				}else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
				{
					dto=((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(dto);
				}
			...
			{
				if(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3513)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_3512)||
		...
		if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
		{
			printCount = ((SendReportReconciliationBean) context.getModelHolder().getModel()).queryVoucherPrintCount(dto);
		}else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
		{
			printCount = ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).queryVoucherPrintCount(dto);
		}
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
				{
					 MVCUtils.openEditor("发送报表对账");
					MVCUtils.openComposite("发送报表对账", "维护界面");
					((SendReportReconciliationBean) context.getModelHolder().getModel()).getCheckList().clear();
					((AbstractMetaDataEditorPart) ((SendReportReconciliationBean) context.getModelHolder().getModel()).getModelHolder()).fireModelChanged(ModelChangedEvent.REFRESH_ALL_EVENT);
				}else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
				{
					MVCUtils.openEditor("发送业务对账");
					MVCUtils.openComposite("发送业务对账", "维护界面");
					((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().clear();
					((AbstractMetaDataEditorPart) ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getModelHolder()).fireModelChanged(ModelChangedEvent.REFRESH_ALL_EVENT);
				}
				ls_AllCount = "";
				listCount = 0;
				nowCount.setText("" + (listCount + 1));
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				if(context.getModelHolder().getModel() instanceof SendReportReconciliationBean)
				{
					list.add(((SendReportReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
					String stamptCounts= ((SendReportReconciliationBean) context.getModelHolder().getModel()).voucherStamp(list);
...
//						MessageDialog.openMessageDialog(null, ((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
						String returnmsg = ((SendReportReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
						((SendReportReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
...
//						MessageDialog.openMessageDialog(null, stampName+"签章成功！");
						String demo = ((SendReportReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
						((SendReportReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
						((SendReportReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
						MessageDialog.openMessageDialog(null, stampName+"签章成功！");
					}
				}else if(context.getModelHolder().getModel() instanceof SendBusinessReconciliationBean)
				{
					list.add(((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
					String stamptCounts= ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).voucherStamp(list);
					if(stamptCounts.equals("0")){
//						MessageDialog.openMessageDialog(null, ((VoucherReportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg());
						String returnmsg = ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
						((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
						MessageDialog.openMessageDialog(null, returnmsg);
					}else if(stamptCounts.equals("1")){
//						MessageDialog.openMessageDialog(null, stampName+"签章成功！");
						String demo = ((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
						((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
						((SendBusinessReconciliationBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
						MessageDialog.openMessageDialog(null, stampName+"签章成功！");
					}
				}
				init(listCount);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeReconciliationOcx.stampName = stampName;
	}
	...
	public static void setModelId(String modelId) {
		ActiveXCompositeReconciliationOcx.modelId = modelId;
	}
}