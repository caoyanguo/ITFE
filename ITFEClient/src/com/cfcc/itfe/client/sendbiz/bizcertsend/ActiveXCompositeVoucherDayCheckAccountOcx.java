...
import com.cfcc.itfe.client.sendbiz.createvoucherforreport.CreateVoucherForReportBean;
...
public class ActiveXCompositeVoucherDayCheckAccountOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherDayCheckAccountOcx.class);
	// ole控件对象
	...
	public static void init(int num) {		
		voucherDtoList = ((CreateVoucherForReportBean) context.getModelHolder().getModel())
				.getCheckList();
...
//		Refresh (infoDto.getSvoucherno());
		int count=((CreateVoucherForReportBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		String printCounts= ((CreateVoucherForReportBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((CreateVoucherForReportBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));
			} catch (Exception e) {
...
//				Refresh (infoDto.getSvoucherno());
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherDayCheckAccountOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("98e7de5c-3d97-417a-bbcf-5b6f7be701ec");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherDayCheckAccountOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherDayCheckAccountOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherDayCheckAccountOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((CreateVoucherForReportBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((CreateVoucherForReportBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((CreateVoucherForReportBean) context.getModelHolder()
	...
	public boolean verifyPrintPermission(){
		if(voucherno!=null&&!voucherno.equals("")){
			...
			voucherno="";
			if(!AdminConfirmDialogFacade.open("B_258", "发起凭证对账", "授权打印凭证", msg)){
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("发起凭证对账");
				MVCUtils.openComposite("发起凭证对账", "维护界面");
				((CreateVoucherForReportBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((CreateVoucherForReportBean) context
