...
import com.cfcc.itfe.client.sendbiz.directpaydayreport.DirectPayDayreportBean;
...
public class ActiveXCompositeDirectPayDayreportOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeDirectPayDayreportOcx.class);
	// ole控件对象
	...
	public static String modelId = "96794114-ec37-4057-ba94-1f85bd840094";
	/**
	 * 生成签章报文（读取UKEY印章）
	 * 
	 * @param dto
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
				voucherXML = ((DirectPayDayreportBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((DirectPayDayreportBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=0;
		count = ((DirectPayDayreportBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((DirectPayDayreportBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		for(Mapper map : retList){
			comboStampType.add(map.getDisplayValue().toString());
		}		
		String printCounts= "";
		printCounts=((DirectPayDayreportBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);	
		...
		if (i == 0) {
			try {
				i = AddVoucherXmls(((DirectPayDayreportBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeDirectPayDayreportOcx(Composite parent) {
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeDirectPayDayreportOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeDirectPayDayreportOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeDirectPayDayreportOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	return v.getInt();
	}
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((DirectPayDayreportBean) context.getModelHolder().getModel()).getDto(dto);
		...
		String printCount="";
		printCount = ((DirectPayDayreportBean) context.getModelHolder().getModel()).queryVoucherPrintCount(dto);
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				MVCUtils.openEditor("财政直接支付日报表2206");
				MVCUtils.openComposite("财政直接支付日报表2206", "维护界面");
				((DirectPayDayreportBean) context.getModelHolder().getModel()).getCheckList().clear();
				((AbstractMetaDataEditorPart) ((DirectPayDayreportBean) context.getModelHolder().getModel()).getModelHolder()).fireModelChanged(ModelChangedEvent.REFRESH_ALL_EVENT);
				ls_AllCount = "";
				listCount = 0;
				nowCount.setText("" + (listCount + 1));
			} catch (Exception ex) {
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((DirectPayDayreportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				String stamptCounts= ((DirectPayDayreportBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					String returnmsg = ((DirectPayDayreportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((DirectPayDayreportBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
					MessageDialog.openMessageDialog(null, returnmsg);
				}else if(stamptCounts.equals("1")){
					String demo = ((DirectPayDayreportBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((DirectPayDayreportBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((DirectPayDayreportBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeDirectPayDayreportOcx.stampName = stampName;
	}
	...
	public static void setModelId(String modelId) {
		ActiveXCompositeDirectPayDayreportOcx.modelId = modelId;
	}
}