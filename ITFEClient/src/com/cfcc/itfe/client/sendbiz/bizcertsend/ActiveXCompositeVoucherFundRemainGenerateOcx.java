...
import com.cfcc.itfe.client.recbiz.voucherfundremaingenerate.VoucherFundRemainGenerateBean;
...
public class ActiveXCompositeVoucherFundRemainGenerateOcx extends Composite {
	
	private static Log logger=LogFactory.getLog(ActiveXCompositeVoucherFundRemainGenerateOcx.class);
	// ole控件对象
	...
	public static String getVoucherStamp(TvVoucherinfoDto vDto,String certID,String stampPosition,String stampID)  {
		...
		String voucherXML="";
		try {
			voucherXML = ((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getVoucherXMl(vDto);
	...
	public static void init(int num) {
		...
		voucherDtoList = ((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getCheckList();
		...
		int count=((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).queryVoucherJOintCount(infoDto);
		if(count==-1){
		...
		retList=((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getStampEnums(infoDto);
		...
		String printCounts= ((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).queryVoucherPrintCount(infoDto);		
		...
		if (i == 0) {
			try {				
				i = AddVoucherXmls(((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getVoucherXMl(infoDto));	
			} catch (Exception e) {
				logger.error(e);
	 ...
	 * @param parent
	 */
	public ActiveXCompositeVoucherFundRemainGenerateOcx(Composite parent) {
	...
	protected void createStampContents(Composite composite) {
		...
		context = DataBindingContext
				.createInstance("19616865-87d9-4e55-b256-334449c31b20");
	...
	public int printVoucher(String voucherNo, int pageNo) {
		...
		Variant v = ActiveXCompositeVoucherFundRemainGenerateOcx.execActivexMethod(
	 ...
	 */
	public int pageUp() {
		Variant v = ActiveXCompositeVoucherFundRemainGenerateOcx.execActivexMethod(OcxConstant.PAGEUP,
				new Object[] {});
		int result = v.getInt();
	 ...
	 */
	public int pageDown() {
		Variant v = ActiveXCompositeVoucherFundRemainGenerateOcx.execActivexMethod(OcxConstant.PAGEDOWN,
				new Object[] {});
	...
	private class BtnPrintSelection extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {											
				voucherDtoList = ((VoucherFundRemainGenerateBean) context.getModelHolder()
	...
	public boolean verifyPrintCount(TvVoucherinfoDto dto){		
		if(!(dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2301)||dto.getSvtcode().equals(MsgConstant.VOUCHER_NO_2302)||
			...
			try {
				dto=((VoucherFundRemainGenerateBean) context.getModelHolder()
					.getModel()).getDto(dto);
		...
		String printCount=((VoucherFundRemainGenerateBean) context.getModelHolder()
	...
	private class BtnReturn extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			try {
				IModelHolder holder = MVCUtils.openEditor("各地资金留用情况表生成");
				MVCUtils.openComposite("各地资金留用情况表生成", "维护界面");
				((VoucherFundRemainGenerateBean) context.getModelHolder().getModel())
						.getCheckList().clear();
				((AbstractMetaDataEditorPart) ((VoucherFundRemainGenerateBean) context
	...
	private class BtnStamp extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			try{
				...
				list.add(stampName);
				list.add(((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)));
				
				String stamptCounts= ((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).voucherStamp(list);
				if(stamptCounts.equals("0")){
					String returnmsg = ((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSreturnerrmsg();
					((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSreturnerrmsg(returnmsg);
					MessageDialog.openMessageDialog(null, returnmsg);
				}else if(stamptCounts.equals("1")){
					String demo = ((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getDto(voucherDtoList.get(listCount)).getSdemo();
					((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSstatus(DealCodeConstants.VOUCHER_STAMP);
					((VoucherFundRemainGenerateBean) context.getModelHolder().getModel()).getCheckList().get(listCount).setSdemo(demo);
	...
	public static void setStampName(String stampName) {
		ActiveXCompositeVoucherFundRemainGenerateOcx.stampName = stampName;
	}
}