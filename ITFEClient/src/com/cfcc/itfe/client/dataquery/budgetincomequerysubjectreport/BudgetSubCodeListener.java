...
public class BudgetSubCodeListener extends KeyAdapter{
	private static Log log = LogFactory.getLog(BudgetSubCodeListener.class);
	private Composite composite;
	private ContainerMetaData metadata;
	AbstractMetaDataEditorPart editor;
	private String maname ;
	private BudgetIncomeQuerySubjectReportBean bean;

	public BudgetSubCodeListener(Composite composite, ContainerMetaData metadata,BasicModel bean,String metaname) {
		this.composite = composite;
		this.metadata = metadata;
		this.editor = MVCUtils.getEditor(metadata.editorid);
		this.bean = (BudgetIncomeQuerySubjectReportBean) bean;
						...
						if (controls.get(i) instanceof TextMetaData) {
							...
							if ("预算科目代码".equals(textMeta.caption)) {
								...
								if(null!=subcodes&&subcodes.size()>0){
									...
									bean.setSbudgetsubcode(subjectcodes);
								}else{
									bean.setSbudgetsubcode(null);
								}
