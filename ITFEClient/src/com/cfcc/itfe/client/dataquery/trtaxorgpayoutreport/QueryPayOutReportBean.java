...
public class QueryPayOutReportBean implements IPageDataProvider {

	private static Log log = LogFactory.getLog(QueryPayOutReportBean.class);
	private ICommonDataAccessService commonDataAccessService;
	private TrTaxorgPayoutReportDto dto;
	private PagingContext queryPayOutReport;
	
	public QueryPayOutReportBean(ICommonDataAccessService commonDataAccessService,TrTaxorgPayoutReportDto dto ){
		this.commonDataAccessService = commonDataAccessService;
		this.dto = dto;
		this.queryPayOutReport = new PagingContext(this);
	}
	...
	public PageResponse retrieve(PageRequest pageRequest) {
		try {
			...
			queryPayOutReport.setPage(response);
			return response;
		} catch (Exception e) {
			log.error(e);
	...
	public PagingContext getQueryPayOutReport() {
		return queryPayOutReport;
	}

	public void setQueryPayOutReport(PagingContext queryPayOutReport) {
		this.queryPayOutReport = queryPayOutReport;
	}
