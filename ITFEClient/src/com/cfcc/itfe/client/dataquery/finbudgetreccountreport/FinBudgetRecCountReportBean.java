...
/**
 * codecomment:财政预算收入统计报表
 * 
 * @author db2admin
 * @time 10-08-25 13:33:20 子系统: DataQuery 模块:FinBudgetRecCountReport
...
public class FinBudgetRecCountReportBean extends
	...
	public String query(Object o) {
		...
		if (reporttype.equals("1")) {
			...
			// 报表标题
			reporttitle = sleTrename + sbudgetlevelname + "预算收入"
					+ slebilltypename + "表";
			...
			// 报表标题
			reporttitle = sleTrename + sbudgetlevelname + "预算支出"
					+ slebilltypename + "表";
		}
		...
		try {
			if (reporttype.equals("1")) {
				rptTitle = sleTrename + "(" + sleOfFlagName + ")" + "预算收入"
						+ slebilltypename + "表" + datename;
				...
				rptTitle = sleTrename + "(" + sleOfFlagName + ")" + "预算支出"
						+ slebilltypename + "表" + datename;
				// 编制年月日
				date = date.substring(0, 4) + "年" + date.substring(4, 6) + "月"
						+ date.substring(6, 8) + "日";
	...
	public List budgetIncomeQueryList(TrIncomedayrptDto incomedto)
		...
		try {
			...
			// sleSubjectType--科目类型 sleSubjectAttribute--科目属性 smoveflag--调拨标志
			// sleMoneyUnit--金额单位
