...
public class TempTemplateGen {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < tables.size(); i++) {
			...
			// if (tabName.startsWith("TBS_TV_")) {
			if (tabName.equals("TBS_TV_PAYOUT")) {
	...
	private static void genExtBean(String moduleUUID, String eTable,
		...
		try {
			...
			Velocity.mergeTemplate("./src/com/cfcc/model/vm/ExtBean.vm", "GBK",
					context, w);
			FileOper.saveFile(w.toString(), "c:/testmodel/" + fileName);
		} catch (Exception e) {
	...
	private static void genModule(String moduleUUID, String eTable,
		...
		try {
			...
			StringWriter w = new StringWriter();
			Velocity.mergeTemplate("./src/com/cfcc/model/vm/Testmodule.vm", "GBK",
					context, w);
			FileOper.saveFile(w.toString(), "c:/testmodel/" + fileName);
		} catch (Exception e) {
	...
	private static void genUIComponent(String moduleUUID, String eTable,
		...
		try {
			...
			Velocity.mergeTemplate(
					"./src/com/cfcc/model/vm/TestJUI.vm", "GBK",
					context, w);
			FileOper.saveFile(w.toString(), "c:/testmodel/JUIComponent_"
					+ compuuid + ".model");
