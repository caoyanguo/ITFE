...
/**
 * <p>Title: DAO of table: TS_CODEREGTYPE</p>
...
public class TsCoderegtypeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CODEREGTYPE ("
          + "S_ORGCODE,S_REGCODE,S_REGNAME"
        + ") VALUES ("
        + "?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CODEREGTYPE.S_ORGCODE, TS_CODEREGTYPE.S_REGCODE, TS_CODEREGTYPE.S_REGNAME "
        + "FROM TS_CODEREGTYPE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_REGCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CODEREGTYPE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_REGCODE = ? FOR UPDATE"
        ;
  ...
  {
     TsCoderegtypeDto dto = (TsCoderegtypeDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getSregcode());

          ps.setString(3, dto.getSregname());
   ...
   */
	...
	*/
    ...
    {
        TsCoderegtypeDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsCoderegtypeDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsCoderegtypePK pk = (TsCoderegtypePK)_pk ;
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsCoderegtypePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getSregcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsCoderegtypeDto[] dtos = new TsCoderegtypeDto[0];
		    dtos = (TsCoderegtypeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsCoderegtypeDto  dto = new  TsCoderegtypeDto ();
             //S_ORGCODE
             ...
             //S_REGCODE
             str = rs.getString("S_REGCODE");
             if (str == null)
                dto.setSregcode(null);
             else
                dto.setSregcode(str.trim());

             //S_REGNAME
             str = rs.getString("S_REGNAME");
             if (str == null)
                dto.setSregname(null);
             else
                dto.setSregname(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_REGNAME
            ps.setString(pos, dto.getSregname());
            pos++;
           ...
           pos++;
           //S_REGCODE
           ps.setString(pos, dto.getSregcode());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
   		throw new SQLException("数据库表：TS_CODEREGTYPE没有检查修改的字段");
    }
