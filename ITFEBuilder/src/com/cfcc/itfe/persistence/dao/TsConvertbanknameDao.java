...
/**
 * <p>Title: DAO of table: TS_CONVERTBANKNAME</p>
...
public class TsConvertbanknameDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CONVERTBANKNAME ("
          + "S_ORGCODE,S_BANKNAME,S_TCBANKNAME,S_BANKCODE"
        + ") VALUES ("
        + "?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CONVERTBANKNAME.S_ORGCODE, TS_CONVERTBANKNAME.S_BANKNAME, TS_CONVERTBANKNAME.S_TCBANKNAME, TS_CONVERTBANKNAME.S_BANKCODE "
        + "FROM TS_CONVERTBANKNAME "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_BANKNAME = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CONVERTBANKNAME "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_BANKNAME = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CONVERTBANKNAME SET "
        + "S_TCBANKNAME =?,S_BANKCODE =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_BANKNAME = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TS_CONVERTBANKNAME " 
        + " WHERE "
        + "S_ORGCODE = ? AND S_BANKNAME = ?"
        ;
  ...
  {
     TsConvertbanknameDto dto = (TsConvertbanknameDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getSbankname());

          ps.setString(3, dto.getStcbankname());

          ps.setString(4, dto.getSbankcode());
   ...
   */
	...
	*/
    ...
    {
        TsConvertbanknameDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsConvertbanknameDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsConvertbanknamePK pk = (TsConvertbanknamePK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getSbankname());
    ...
    {
        TsConvertbanknamePK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsConvertbanknamePK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsConvertbanknamePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getSbankname());
        ...
        if (results!=null && results.size() > 0)
        {
            TsConvertbanknameDto[] dtos = new TsConvertbanknameDto[0];
		    dtos = (TsConvertbanknameDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsConvertbanknameDto  dto = new  TsConvertbanknameDto ();
             //S_ORGCODE
             ...
             //S_TCBANKNAME
             str = rs.getString("S_TCBANKNAME");
             if (str == null)
                dto.setStcbankname(null);
             else
                dto.setStcbankname(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_TCBANKNAME
            ps.setString(pos, dto.getStcbankname());
            pos++;
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_CONVERTBANKNAME没有检查修改的字段");
    }
