...
/**
 * <p>Title: DAO of table: TS_BUDGETSUBJECT_ADJUST</p>
...
public class TsBudgetsubjectAdjustDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_BUDGETSUBJECT_ADJUST ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_BUDGETSUBJECT_ADJUST.S_ORGCODE, TS_BUDGETSUBJECT_ADJUST.S_SUBJECTCODE, TS_BUDGETSUBJECT_ADJUST.S_SUBJECTNAME, TS_BUDGETSUBJECT_ADJUST.S_SUBJECTCLASS, TS_BUDGETSUBJECT_ADJUST.S_SUBJECTTYPE, "
        + "TS_BUDGETSUBJECT_ADJUST.S_SUBJECTATTR, TS_BUDGETSUBJECT_ADJUST.S_INOUTFLAG, TS_BUDGETSUBJECT_ADJUST.S_WRITEFLAG, TS_BUDGETSUBJECT_ADJUST.S_MOVEFLAG, TS_BUDGETSUBJECT_ADJUST.S_BUDGETTYPE, "
        + "TS_BUDGETSUBJECT_ADJUST.S_CLASSFLAG, TS_BUDGETSUBJECT_ADJUST.S_DRAWBACKTYPE, TS_BUDGETSUBJECT_ADJUST.I_MODICOUNT "
        + "FROM TS_BUDGETSUBJECT_ADJUST "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_BUDGETSUBJECT_ADJUST SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TS_BUDGETSUBJECT_ADJUST " 
        + " WHERE "
        + "S_ORGCODE = ? AND S_SUBJECTCODE = ? AND S_SUBJECTTYPE = ? AND S_BUDGETTYPE = ?"
        ;
  ...
  {
     TsBudgetsubjectAdjustDto dto = (TsBudgetsubjectAdjustDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TsBudgetsubjectAdjustDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsBudgetsubjectAdjustDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsBudgetsubjectAdjustPK pk = (TsBudgetsubjectAdjustPK)_pk ;
    ...
    {
        TsBudgetsubjectAdjustPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsBudgetsubjectAdjustPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsBudgetsubjectAdjustPK)(pks.get(i)) ; 
        ...
        if (results!=null && results.size() > 0)
        {
            TsBudgetsubjectAdjustDto[] dtos = new TsBudgetsubjectAdjustDto[0];
		    dtos = (TsBudgetsubjectAdjustDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                           		throw new SQLException("数据库表：TS_BUDGETSUBJECT_ADJUST没有检查修改的字段");
    }
