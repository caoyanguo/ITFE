...
/**
 * <p>Title: DAO of table: TS_DRAWBACKREASON</p>
...
public class TsDrawbackreasonDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_DRAWBACKREASON ("
          + "S_BOOKORGCODE,S_TBSDRAWCODE,S_TBSDRAWNAME,S_TCBSDRAWCODE,S_TCBSDRAWNAME"
          + ",TS_SYSUPDATE"
        + ") VALUES ("
        + "?,?,?,?,?,CURRENT TIMESTAMP )";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_DRAWBACKREASON.S_BOOKORGCODE, TS_DRAWBACKREASON.S_TBSDRAWCODE, TS_DRAWBACKREASON.S_TBSDRAWNAME, TS_DRAWBACKREASON.S_TCBSDRAWCODE, TS_DRAWBACKREASON.S_TCBSDRAWNAME, "
        + "TS_DRAWBACKREASON.TS_SYSUPDATE "
        + "FROM TS_DRAWBACKREASON "
        +" WHERE " 
        + "S_BOOKORGCODE = ? AND S_TBSDRAWCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_DRAWBACKREASON "
        +" WHERE " 
        + "S_BOOKORGCODE = ? AND S_TBSDRAWCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_DRAWBACKREASON SET "
        + "S_TBSDRAWNAME =?,S_TCBSDRAWCODE =?,S_TCBSDRAWNAME =?,TS_SYSUPDATE =CURRENT TIMESTAMP "
        + "WHERE "
        + "S_BOOKORGCODE = ? AND S_TBSDRAWCODE = ?"
        ;
  ...
  {
     TsDrawbackreasonDto dto = (TsDrawbackreasonDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getStbsdrawcode());

          ps.setString(3, dto.getStbsdrawname());

          ps.setString(4, dto.getStcbsdrawcode());

          ps.setString(5, dto.getStcbsdrawname());
   ...
   */
	...
	*/
    ...
    {
        TsDrawbackreasonDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsDrawbackreasonDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsDrawbackreasonPK pk = (TsDrawbackreasonPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getStbsdrawcode());
    ...
    {
        TsDrawbackreasonPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsDrawbackreasonPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsDrawbackreasonPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSbookorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getStbsdrawcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsDrawbackreasonDto[] dtos = new TsDrawbackreasonDto[0];
		    dtos = (TsDrawbackreasonDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsDrawbackreasonDto  dto = new  TsDrawbackreasonDto ();
             //S_BOOKORGCODE
             ...
             //S_TBSDRAWCODE
             str = rs.getString("S_TBSDRAWCODE");
             if (str == null)
                dto.setStbsdrawcode(null);
             else
                dto.setStbsdrawcode(str.trim());

             //S_TBSDRAWNAME
             str = rs.getString("S_TBSDRAWNAME");
             if (str == null)
                dto.setStbsdrawname(null);
             else
                dto.setStbsdrawname(str.trim());

             //S_TCBSDRAWCODE
             str = rs.getString("S_TCBSDRAWCODE");
             if (str == null)
                dto.setStcbsdrawcode(null);
             else
                dto.setStcbsdrawcode(str.trim());

             //S_TCBSDRAWNAME
             str = rs.getString("S_TCBSDRAWNAME");
             if (str == null)
                dto.setStcbsdrawname(null);
             else
                dto.setStcbsdrawname(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_TBSDRAWNAME
            ps.setString(pos, dto.getStbsdrawname());
            pos++;

            //S_TCBSDRAWCODE
            ps.setString(pos, dto.getStcbsdrawcode());
            pos++;

            //S_TCBSDRAWNAME
            ps.setString(pos, dto.getStcbsdrawname());
            pos++;
           ...
           pos++;
           //S_TBSDRAWCODE
           ps.setString(pos, dto.getStbsdrawcode());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
            		throw new SQLException("数据库表：TS_DRAWBACKREASON没有检查修改的字段");
    }
