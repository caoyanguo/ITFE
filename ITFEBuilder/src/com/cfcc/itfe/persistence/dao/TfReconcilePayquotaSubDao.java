...
import com.cfcc.itfe.persistence.pk.TfReconcilePayquotaSubPK ;


/**
 * <p>Title: DAO of table: TF_RECONCILE_PAYQUOTA_SUB</p>
 * <p>Description:清算额度对帐子表3510 Data Access Object  </p>
...
public class TfReconcilePayquotaSubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TF_RECONCILE_PAYQUOTA_SUB ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TF_RECONCILE_PAYQUOTA_SUB.I_VOUSRLNO, TF_RECONCILE_PAYQUOTA_SUB.I_SEQNO, TF_RECONCILE_PAYQUOTA_SUB.S_ID, TF_RECONCILE_PAYQUOTA_SUB.S_SUPDEPCODE, TF_RECONCILE_PAYQUOTA_SUB.S_SUPDEPNAME, "
        + "TF_RECONCILE_PAYQUOTA_SUB.S_FUNDTYPECODE, TF_RECONCILE_PAYQUOTA_SUB.S_FUNDTYPENAME, TF_RECONCILE_PAYQUOTA_SUB.S_PAYBANKCODE, TF_RECONCILE_PAYQUOTA_SUB.S_PAYBANKNAME, TF_RECONCILE_PAYQUOTA_SUB.S_PAYBANKNO, "
        + "TF_RECONCILE_PAYQUOTA_SUB.S_EXPFUNCCODE, TF_RECONCILE_PAYQUOTA_SUB.S_EXPFUNCNAME, TF_RECONCILE_PAYQUOTA_SUB.S_PROCATCODE, TF_RECONCILE_PAYQUOTA_SUB.S_PROCATNAME, TF_RECONCILE_PAYQUOTA_SUB.S_PAYTYPECODE, "
        + "TF_RECONCILE_PAYQUOTA_SUB.S_PAYTYPENAME, TF_RECONCILE_PAYQUOTA_SUB.N_PREDATEMONEY, TF_RECONCILE_PAYQUOTA_SUB.N_CLEARAMT, TF_RECONCILE_PAYQUOTA_SUB.N_CURRECKMONEY, TF_RECONCILE_PAYQUOTA_SUB.N_CURDATEMONEY, "
        + "TF_RECONCILE_PAYQUOTA_SUB.S_XCHECKRESULT, TF_RECONCILE_PAYQUOTA_SUB.S_XCHECKREASON, TF_RECONCILE_PAYQUOTA_SUB.S_HOLD1, TF_RECONCILE_PAYQUOTA_SUB.S_HOLD2, TF_RECONCILE_PAYQUOTA_SUB.S_HOLD3, "
        + "TF_RECONCILE_PAYQUOTA_SUB.S_HOLD4, TF_RECONCILE_PAYQUOTA_SUB.S_EXT1, TF_RECONCILE_PAYQUOTA_SUB.S_EXT2, TF_RECONCILE_PAYQUOTA_SUB.S_EXT3, TF_RECONCILE_PAYQUOTA_SUB.S_EXT4, "
        + "TF_RECONCILE_PAYQUOTA_SUB.S_EXT5 "
        + "FROM TF_RECONCILE_PAYQUOTA_SUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TF_RECONCILE_PAYQUOTA_SUB SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TF_RECONCILE_PAYQUOTA_SUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     TfReconcilePayquotaSubDto dto = (TfReconcilePayquotaSubDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TfReconcilePayquotaSubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TfReconcilePayquotaSubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TfReconcilePayquotaSubPK pk = (TfReconcilePayquotaSubPK)_pk ;
    ...
    {
        TfReconcilePayquotaSubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TfReconcilePayquotaSubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TfReconcilePayquotaSubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TfReconcilePayquotaSubDto[] dtos = new TfReconcilePayquotaSubDto[0];
		    dtos = (TfReconcilePayquotaSubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TfReconcilePayquotaSubDto  dto = new  TfReconcilePayquotaSubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                                                       		throw new SQLException("数据库表：TF_RECONCILE_PAYQUOTA_SUB没有检查修改的字段");
    }
