...
import com.cfcc.itfe.persistence.dto.HtfUnitrecordsubDto ;
import com.cfcc.itfe.persistence.pk.HtfUnitrecordsubPK ;


/**
 * <p>Title: DAO of table: HTF_UNITRECORDSUB</p>
...
public class HtfUnitrecordsubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_UNITRECORDSUB ("
          + "I_VOUSRLNO,I_SEQNO,S_ID,S_AGENCYCODE,S_AGENCYNAME"
          + ",S_FUNDTYPECODE,S_FUNDTYPENAME,S_AGENCYACCONO,S_AGENCYACCONAME,S_AGENCYBANKNAME"
          + ",S_ACCSTATUS,S_HOLD1,S_HOLD2,S_HOLD3,S_HOLD4"
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_UNITRECORDSUB.I_VOUSRLNO, HTF_UNITRECORDSUB.I_SEQNO, HTF_UNITRECORDSUB.S_ID, HTF_UNITRECORDSUB.S_AGENCYCODE, HTF_UNITRECORDSUB.S_AGENCYNAME, "
        + "HTF_UNITRECORDSUB.S_FUNDTYPECODE, HTF_UNITRECORDSUB.S_FUNDTYPENAME, HTF_UNITRECORDSUB.S_AGENCYACCONO, HTF_UNITRECORDSUB.S_AGENCYACCONAME, HTF_UNITRECORDSUB.S_AGENCYBANKNAME, "
        + "HTF_UNITRECORDSUB.S_ACCSTATUS, HTF_UNITRECORDSUB.S_HOLD1, HTF_UNITRECORDSUB.S_HOLD2, HTF_UNITRECORDSUB.S_HOLD3, HTF_UNITRECORDSUB.S_HOLD4, "
        + "HTF_UNITRECORDSUB.S_EXT1, HTF_UNITRECORDSUB.S_EXT2, HTF_UNITRECORDSUB.S_EXT3 "
        + "FROM HTF_UNITRECORDSUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_UNITRECORDSUB SET "
        + "S_ID =?,S_AGENCYCODE =?,S_AGENCYNAME =?,S_FUNDTYPECODE =?,S_FUNDTYPENAME =?, "
        + "S_AGENCYACCONO =?,S_AGENCYACCONAME =?,S_AGENCYBANKNAME =?,S_ACCSTATUS =?,S_HOLD1 =?, "
  ...
  {
     HtfUnitrecordsubDto dto = (HtfUnitrecordsubDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(4, dto.getSagencycode());

          ps.setString(5, dto.getSagencyname());

          ps.setString(6, dto.getSfundtypecode());

          ps.setString(7, dto.getSfundtypename());

          ps.setString(8, dto.getSagencyaccono());

          ps.setString(9, dto.getSagencyacconame());

          ps.setString(10, dto.getSagencybankname());

          ps.setString(11, dto.getSaccstatus());
   ...
   */
	...
	*/
    ...
    {
        HtfUnitrecordsubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfUnitrecordsubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfUnitrecordsubPK pk = (HtfUnitrecordsubPK)_pk ;
    ...
    {
        HtfUnitrecordsubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfUnitrecordsubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfUnitrecordsubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfUnitrecordsubDto[] dtos = new HtfUnitrecordsubDto[0];
		    dtos = (HtfUnitrecordsubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfUnitrecordsubDto  dto = new  HtfUnitrecordsubDto ();
             //I_VOUSRLNO
             ...
             //S_AGENCYACCONO
             str = rs.getString("S_AGENCYACCONO");
             if (str == null)
                dto.setSagencyaccono(null);
             else
                dto.setSagencyaccono(str.trim());

             //S_AGENCYACCONAME
             str = rs.getString("S_AGENCYACCONAME");
             if (str == null)
                dto.setSagencyacconame(null);
             else
                dto.setSagencyacconame(str.trim());

             //S_AGENCYBANKNAME
             str = rs.getString("S_AGENCYBANKNAME");
             if (str == null)
                dto.setSagencybankname(null);
             else
                dto.setSagencybankname(str.trim());

             //S_ACCSTATUS
             str = rs.getString("S_ACCSTATUS");
             if (str == null)
                dto.setSaccstatus(null);
             else
                dto.setSaccstatus(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_AGENCYACCONO
            ps.setString(pos, dto.getSagencyaccono());
            pos++;

            //S_AGENCYACCONAME
            ps.setString(pos, dto.getSagencyacconame());
            pos++;

            //S_AGENCYBANKNAME
            ps.setString(pos, dto.getSagencybankname());
            pos++;

            //S_ACCSTATUS
            ps.setString(pos, dto.getSaccstatus());
            pos++;
    ...
    */
    ...
    {
                                                		throw new SQLException("数据库表：HTF_UNITRECORDSUB没有检查修改的字段");
    }
