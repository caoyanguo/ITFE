...
import com.cfcc.itfe.persistence.dto.HtfReportDepositSubDto ;
import com.cfcc.itfe.persistence.pk.HtfReportDepositSubPK ;


/**
 * <p>Title: DAO of table: HTF_REPORT_DEPOSIT_SUB</p>
 * <p>Description:库款账户对帐子表历史表3513 Data Access Object  </p>
...
public class HtfReportDepositSubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_REPORT_DEPOSIT_SUB ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_REPORT_DEPOSIT_SUB.I_VOUSRLNO, HTF_REPORT_DEPOSIT_SUB.I_SEQNO, HTF_REPORT_DEPOSIT_SUB.S_ID, HTF_REPORT_DEPOSIT_SUB.S_ACCTDATE, HTF_REPORT_DEPOSIT_SUB.N_INCOMEAMT, "
        + "HTF_REPORT_DEPOSIT_SUB.N_PAYAMT, HTF_REPORT_DEPOSIT_SUB.S_XCHECKRESULT, HTF_REPORT_DEPOSIT_SUB.S_XCHECKREASON, HTF_REPORT_DEPOSIT_SUB.S_HOLD1, HTF_REPORT_DEPOSIT_SUB.S_HOLD2, "
        + "HTF_REPORT_DEPOSIT_SUB.S_HOLD3, HTF_REPORT_DEPOSIT_SUB.S_HOLD4, HTF_REPORT_DEPOSIT_SUB.S_EXT1, HTF_REPORT_DEPOSIT_SUB.S_EXT2, HTF_REPORT_DEPOSIT_SUB.S_EXT3, "
        + "HTF_REPORT_DEPOSIT_SUB.S_EXT4, HTF_REPORT_DEPOSIT_SUB.S_EXT5 "
        + "FROM HTF_REPORT_DEPOSIT_SUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_REPORT_DEPOSIT_SUB SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_REPORT_DEPOSIT_SUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     HtfReportDepositSubDto dto = (HtfReportDepositSubDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtfReportDepositSubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfReportDepositSubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfReportDepositSubPK pk = (HtfReportDepositSubPK)_pk ;
    ...
    {
        HtfReportDepositSubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfReportDepositSubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfReportDepositSubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfReportDepositSubDto[] dtos = new HtfReportDepositSubDto[0];
		    dtos = (HtfReportDepositSubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfReportDepositSubDto  dto = new  HtfReportDepositSubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                             		throw new SQLException("数据库表：HTF_REPORT_DEPOSIT_SUB没有检查修改的字段");
    }
