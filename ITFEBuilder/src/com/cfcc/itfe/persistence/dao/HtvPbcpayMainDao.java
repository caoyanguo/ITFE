...
/**
 * <p>Title: DAO of table: HTV_PBCPAY_MAIN</p>
...
public class HtvPbcpayMainDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTV_PBCPAY_MAIN ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTV_PBCPAY_MAIN.I_VOUSRLNO, HTV_PBCPAY_MAIN.S_TRANO, HTV_PBCPAY_MAIN.S_ORGCODE, HTV_PBCPAY_MAIN.S_TRECODE, HTV_PBCPAY_MAIN.S_BILLORG, "
        + "HTV_PBCPAY_MAIN.S_ENTRUSTDATE, HTV_PBCPAY_MAIN.S_PACKNO, HTV_PBCPAY_MAIN.S_PAYOUTVOUTYPENO, HTV_PBCPAY_MAIN.S_PAYMODE, HTV_PBCPAY_MAIN.S_VOUNO, "
        + "HTV_PBCPAY_MAIN.D_VOUCHER, HTV_PBCPAY_MAIN.S_PAYERACCT, HTV_PBCPAY_MAIN.S_PAYERNAME, HTV_PBCPAY_MAIN.S_PAYERADDR, HTV_PBCPAY_MAIN.S_PAYEEACCT, "
        + "HTV_PBCPAY_MAIN.S_PAYEENAME, HTV_PBCPAY_MAIN.S_PAYEEADDR, HTV_PBCPAY_MAIN.S_RCVBNKNO, HTV_PBCPAY_MAIN.S_PAYEEOPNBNKNO, HTV_PBCPAY_MAIN.S_ADDWORD, "
        + "HTV_PBCPAY_MAIN.C_BDGKIND, HTV_PBCPAY_MAIN.I_OFYEAR, HTV_PBCPAY_MAIN.S_BDGADMTYPE, HTV_PBCPAY_MAIN.F_AMT, HTV_PBCPAY_MAIN.S_TRASTATE, "
        + "HTV_PBCPAY_MAIN.S_DESCRIPTION, HTV_PBCPAY_MAIN.D_ACCT, HTV_PBCPAY_MAIN.C_TRIMFLAG, HTV_PBCPAY_MAIN.I_DETAILNIO, HTV_PBCPAY_MAIN.S_STATUS, "
        + "HTV_PBCPAY_MAIN.S_BIZTYPE, HTV_PBCPAY_MAIN.S_BACKFLAG, HTV_PBCPAY_MAIN.D_ORIENTRUSTDATE, HTV_PBCPAY_MAIN.S_ORITRANO, HTV_PBCPAY_MAIN.S_ORIVOUNO, "
        + "HTV_PBCPAY_MAIN.D_ORIVOUDATE, HTV_PBCPAY_MAIN.I_CHGNUM, HTV_PBCPAY_MAIN.S_INPUTERID, HTV_PBCPAY_MAIN.TS_SYSUPDATE "
        + "FROM HTV_PBCPAY_MAIN "
        +" WHERE " 
        + "I_VOUSRLNO = ?"
        ;
  ...
  {
     HtvPbcpayMainDto dto = (HtvPbcpayMainDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtvPbcpayMainDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtvPbcpayMainDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtvPbcpayMainPK pk = (HtvPbcpayMainPK)_pk ;
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        if (results!=null && results.size() > 0)
        {
            HtvPbcpayMainDto[] dtos = new HtvPbcpayMainDto[0];
		    dtos = (HtvPbcpayMainDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtvPbcpayMainDto  dto = new  HtvPbcpayMainDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    public void check(IDto _dto, Connection conn) throws SQLException
    {
		...
		HtvPbcpayMainDto dtoNew = (HtvPbcpayMainDto)_dto ;
		HtvPbcpayMainDto dtoOld =(HtvPbcpayMainDto) findForUpdate(_dto.getPK(), conn,false) ;
		if (dtoOld != null) {
			if (! dtoOld.getIchgnum().equals(dtoNew.getIchgnum()))
				throw new SQLException("数据库表：HTV_PBCPAY_MAIN记录已经被修改!数据库中记录修改次数为:"+dtoOld.getIchgnum()+"; 当前记录修改次数为:"+dtoNew.getIchgnum()) ;
		}else{
			throw new SQLException("数据库表：HTV_PBCPAY_MAIN记录已经被删除!");
		}
    }
