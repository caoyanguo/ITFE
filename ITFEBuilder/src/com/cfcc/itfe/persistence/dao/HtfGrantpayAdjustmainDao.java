...
import com.cfcc.itfe.persistence.pk.HtfGrantpayAdjustmainPK ;


/**
 * <p>Title: DAO of table: HTF_GRANTPAY_ADJUSTMAIN</p>
...
public class HtfGrantpayAdjustmainDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_GRANTPAY_ADJUSTMAIN ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_GRANTPAY_ADJUSTMAIN.I_VOUSRLNO, HTF_GRANTPAY_ADJUSTMAIN.S_ORGCODE, HTF_GRANTPAY_ADJUSTMAIN.S_TRECODE, HTF_GRANTPAY_ADJUSTMAIN.S_STATUS, HTF_GRANTPAY_ADJUSTMAIN.S_DEMO, "
        + "HTF_GRANTPAY_ADJUSTMAIN.TS_SYSUPDATE, HTF_GRANTPAY_ADJUSTMAIN.S_PACKAGENO, HTF_GRANTPAY_ADJUSTMAIN.S_ID, HTF_GRANTPAY_ADJUSTMAIN.S_ADMDIVCODE, HTF_GRANTPAY_ADJUSTMAIN.S_STYEAR, "
        + "HTF_GRANTPAY_ADJUSTMAIN.S_VTCODE, HTF_GRANTPAY_ADJUSTMAIN.S_VOUDATE, HTF_GRANTPAY_ADJUSTMAIN.S_VOUCHERNO, HTF_GRANTPAY_ADJUSTMAIN.S_BGTTYPECODE, HTF_GRANTPAY_ADJUSTMAIN.S_BGTTYPENAME, "
        + "HTF_GRANTPAY_ADJUSTMAIN.S_FUNDTYPECODE, HTF_GRANTPAY_ADJUSTMAIN.S_FUNDTYPENAME, HTF_GRANTPAY_ADJUSTMAIN.N_PAYAMT, HTF_GRANTPAY_ADJUSTMAIN.S_PAYBANKCODE, HTF_GRANTPAY_ADJUSTMAIN.S_PAYBANKNAME, "
        + "HTF_GRANTPAY_ADJUSTMAIN.S_REMARK, HTF_GRANTPAY_ADJUSTMAIN.S_XACCDATE, HTF_GRANTPAY_ADJUSTMAIN.S_HOLD1, HTF_GRANTPAY_ADJUSTMAIN.S_HOLD2, HTF_GRANTPAY_ADJUSTMAIN.S_EXT1, "
        + "HTF_GRANTPAY_ADJUSTMAIN.S_EXT2, HTF_GRANTPAY_ADJUSTMAIN.S_EXT3, HTF_GRANTPAY_ADJUSTMAIN.S_EXT4, HTF_GRANTPAY_ADJUSTMAIN.S_EXT5 "
        + "FROM HTF_GRANTPAY_ADJUSTMAIN "
        +" WHERE " 
        + "I_VOUSRLNO = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_GRANTPAY_ADJUSTMAIN SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_GRANTPAY_ADJUSTMAIN " 
        + " WHERE "
        + "I_VOUSRLNO = ?"
        ;
  ...
  {
     HtfGrantpayAdjustmainDto dto = (HtfGrantpayAdjustmainDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtfGrantpayAdjustmainDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfGrantpayAdjustmainDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfGrantpayAdjustmainPK pk = (HtfGrantpayAdjustmainPK)_pk ;
    ...
    {
        HtfGrantpayAdjustmainPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfGrantpayAdjustmainPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfGrantpayAdjustmainPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfGrantpayAdjustmainDto[] dtos = new HtfGrantpayAdjustmainDto[0];
		    dtos = (HtfGrantpayAdjustmainDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfGrantpayAdjustmainDto  dto = new  HtfGrantpayAdjustmainDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                                                    		throw new SQLException("数据库表：HTF_GRANTPAY_ADJUSTMAIN没有检查修改的字段");
    }
