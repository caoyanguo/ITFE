...
/**
 * <p>Title: DAO of table: TS_CONVERTTOBANK</p>
...
public class TsConverttobankDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CONVERTTOBANK ("
          + "S_ORGCODE,S_RECVUNIT,S_TRECODE,S_AGENTBANK,S_TOBANK"
          + ",I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CONVERTTOBANK.S_ORGCODE, TS_CONVERTTOBANK.S_RECVUNIT, TS_CONVERTTOBANK.S_TRECODE, TS_CONVERTTOBANK.S_AGENTBANK, TS_CONVERTTOBANK.S_TOBANK, "
        + "TS_CONVERTTOBANK.I_MODICOUNT "
        + "FROM TS_CONVERTTOBANK "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_RECVUNIT = ? AND S_TRECODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CONVERTTOBANK "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_RECVUNIT = ? AND S_TRECODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CONVERTTOBANK SET "
        + "S_AGENTBANK =?,S_TOBANK =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_RECVUNIT = ? AND S_TRECODE = ?"
        ;
  ...
  {
     TsConverttobankDto dto = (TsConverttobankDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getSrecvunit());

          ps.setString(3, dto.getStrecode());

          ps.setString(4, dto.getSagentbank());

          ps.setString(5, dto.getStobank());
   ...
   */
	...
	*/
    ...
    {
        TsConverttobankDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsConverttobankDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsConverttobankPK pk = (TsConverttobankPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getSrecvunit());
    ...
    {
        TsConverttobankPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsConverttobankPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsConverttobankPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*3+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*3+2, pk.getSrecvunit());
        ...
        if (results!=null && results.size() > 0)
        {
            TsConverttobankDto[] dtos = new TsConverttobankDto[0];
		    dtos = (TsConverttobankDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsConverttobankDto  dto = new  TsConverttobankDto ();
             //S_ORGCODE
             ...
             //S_RECVUNIT
             str = rs.getString("S_RECVUNIT");
             if (str == null)
                dto.setSrecvunit(null);
             else
                dto.setSrecvunit(str.trim());
             ...
             //S_AGENTBANK
             str = rs.getString("S_AGENTBANK");
             if (str == null)
                dto.setSagentbank(null);
             else
                dto.setSagentbank(str.trim());

             //S_TOBANK
             str = rs.getString("S_TOBANK");
             if (str == null)
                dto.setStobank(null);
             else
                dto.setStobank(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_AGENTBANK
            ps.setString(pos, dto.getSagentbank());
            pos++;

            //S_TOBANK
            ps.setString(pos, dto.getStobank());
            pos++;
           ...
           pos++;
           //S_RECVUNIT
           ps.setString(pos, dto.getSrecvunit());
           pos++;
           //S_TRECODE
    ...
    */
    ...
    {
         		throw new SQLException("数据库表：TS_CONVERTTOBANK没有检查修改的字段");
    }
