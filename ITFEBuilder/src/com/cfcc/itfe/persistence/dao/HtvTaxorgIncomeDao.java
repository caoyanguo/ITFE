...
import com.cfcc.itfe.persistence.pk.HtvTaxorgIncomePK ;


/**
 * <p>Title: DAO of table: HTV_TAXORG_INCOME</p>
...
public class HtvTaxorgIncomeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTV_TAXORG_INCOME ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTV_TAXORG_INCOME.S_TAXORGCODE, HTV_TAXORG_INCOME.S_TRECODE, HTV_TAXORG_INCOME.S_INTREDATE, HTV_TAXORG_INCOME.I_PKGSEQNO, HTV_TAXORG_INCOME.S_EXPVOUNO, "
        + "HTV_TAXORG_INCOME.S_EXPVOUTYPE, HTV_TAXORG_INCOME.C_BDGLEVEL, HTV_TAXORG_INCOME.S_BDGSBTCODE, HTV_TAXORG_INCOME.C_BDGKIND, HTV_TAXORG_INCOME.F_AMT, "
        + "HTV_TAXORG_INCOME.C_VOUCHANNEL "
        + "FROM HTV_TAXORG_INCOME "
        +" WHERE " 
  ...
  {
     HtvTaxorgIncomeDto dto = (HtvTaxorgIncomeDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtvTaxorgIncomeDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtvTaxorgIncomeDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtvTaxorgIncomePK pk = (HtvTaxorgIncomePK)_pk ;
    ...
    {
        HtvTaxorgIncomePK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtvTaxorgIncomePK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtvTaxorgIncomePK)(pks.get(i)) ; 
        ...
        if (results!=null && results.size() > 0)
        {
            HtvTaxorgIncomeDto[] dtos = new HtvTaxorgIncomeDto[0];
		    dtos = (HtvTaxorgIncomeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtvTaxorgIncomeDto  dto = new  HtvTaxorgIncomeDto ();
             //S_TAXORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                           		throw new SQLException("数据库表：HTV_TAXORG_INCOME没有检查修改的字段");
    }
