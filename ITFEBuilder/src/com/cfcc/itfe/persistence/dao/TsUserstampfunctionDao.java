...
/**
 * <p>Title: DAO of table: TS_USERSTAMPFUNCTION</p>
...
public class TsUserstampfunctionDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_USERSTAMPFUNCTION ("
          + "S_ORGCODE,S_USERCODE,S_STAMPTYPECODE,S_MODELID,S_PLACEID"
          + ",I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_USERSTAMPFUNCTION.S_ORGCODE, TS_USERSTAMPFUNCTION.S_USERCODE, TS_USERSTAMPFUNCTION.S_STAMPTYPECODE, TS_USERSTAMPFUNCTION.S_MODELID, TS_USERSTAMPFUNCTION.S_PLACEID, "
        + "TS_USERSTAMPFUNCTION.I_MODICOUNT "
        + "FROM TS_USERSTAMPFUNCTION "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ? AND S_MODELID = ? AND S_PLACEID = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_USERSTAMPFUNCTION "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ? AND S_MODELID = ? AND S_PLACEID = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_USERSTAMPFUNCTION SET "
        + "I_MODICOUNT =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ? AND S_MODELID = ? AND S_PLACEID = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TS_USERSTAMPFUNCTION " 
        + " WHERE "
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ? AND S_MODELID = ? AND S_PLACEID = ?"
        ;
  ...
  {
     TsUserstampfunctionDto dto = (TsUserstampfunctionDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(3, dto.getSstamptypecode());

          ps.setString(4, dto.getSmodelid());

          ps.setString(5, dto.getSplaceid());
   ...
   */
	...
	*/
    ...
    {
        TsUserstampfunctionDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsUserstampfunctionDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsUserstampfunctionPK pk = (TsUserstampfunctionPK)_pk ;
       ...
       try {
           ...
           ps.setString(3, pk.getSstamptypecode());

           ps.setString(4, pk.getSmodelid());

           ps.setString(5, pk.getSplaceid());
    ...
    {
        TsUserstampfunctionPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsUserstampfunctionPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsUserstampfunctionPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*5+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*5+2, pk.getSusercode());

                   ps.setString((i-iBegin)*5+3, pk.getSstamptypecode());

                   ps.setString((i-iBegin)*5+4, pk.getSmodelid());

                   ps.setString((i-iBegin)*5+5, pk.getSplaceid());
        ...
        if (results!=null && results.size() > 0)
        {
            TsUserstampfunctionDto[] dtos = new TsUserstampfunctionDto[0];
		    dtos = (TsUserstampfunctionDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
   		throw new SQLException("数据库表：TS_USERSTAMPFUNCTION没有检查修改的字段");
    }
