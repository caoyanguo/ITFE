...
import com.cfcc.itfe.persistence.pk.TfReconcilePayinfoMainPK ;


/**
 * <p>Title: DAO of table: TF_RECONCILE_PAYINFO_MAIN</p>
 * <p>Description:清算信息对帐主表3507 Data Access Object  </p>
...
public class TfReconcilePayinfoMainDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TF_RECONCILE_PAYINFO_MAIN ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TF_RECONCILE_PAYINFO_MAIN.I_VOUSRLNO, TF_RECONCILE_PAYINFO_MAIN.S_ORGCODE, TF_RECONCILE_PAYINFO_MAIN.S_TRECODE, TF_RECONCILE_PAYINFO_MAIN.S_STATUS, TF_RECONCILE_PAYINFO_MAIN.S_DEMO, "
        + "TF_RECONCILE_PAYINFO_MAIN.TS_SYSUPDATE, TF_RECONCILE_PAYINFO_MAIN.S_PACKAGENO, TF_RECONCILE_PAYINFO_MAIN.S_ADMDIVCODE, TF_RECONCILE_PAYINFO_MAIN.S_STYEAR, TF_RECONCILE_PAYINFO_MAIN.S_VTCODE, "
        + "TF_RECONCILE_PAYINFO_MAIN.S_VOUDATE, TF_RECONCILE_PAYINFO_MAIN.S_VOUCHERNO, TF_RECONCILE_PAYINFO_MAIN.S_VOUCHERCHECKNO, TF_RECONCILE_PAYINFO_MAIN.S_CHILDPACKNUM, TF_RECONCILE_PAYINFO_MAIN.S_CURPACKNO, "
        + "TF_RECONCILE_PAYINFO_MAIN.S_CLEARBANKCODE, TF_RECONCILE_PAYINFO_MAIN.S_CLEARBANKNAME, TF_RECONCILE_PAYINFO_MAIN.S_CLEARACCNO, TF_RECONCILE_PAYINFO_MAIN.S_CLEARACCNANME, TF_RECONCILE_PAYINFO_MAIN.S_BEGINDATE, "
        + "TF_RECONCILE_PAYINFO_MAIN.S_ENDDATE, TF_RECONCILE_PAYINFO_MAIN.S_ALLNUM, TF_RECONCILE_PAYINFO_MAIN.N_ALLAMT, TF_RECONCILE_PAYINFO_MAIN.S_XCHECKRESULT, TF_RECONCILE_PAYINFO_MAIN.S_XDIFFNUM, "
        + "TF_RECONCILE_PAYINFO_MAIN.S_HOLD1, TF_RECONCILE_PAYINFO_MAIN.S_HOLD2, TF_RECONCILE_PAYINFO_MAIN.S_EXT1, TF_RECONCILE_PAYINFO_MAIN.S_EXT2, TF_RECONCILE_PAYINFO_MAIN.S_EXT3, "
        + "TF_RECONCILE_PAYINFO_MAIN.S_EXT4, TF_RECONCILE_PAYINFO_MAIN.S_EXT5 "
        + "FROM TF_RECONCILE_PAYINFO_MAIN "
        +" WHERE " 
        + "I_VOUSRLNO = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TF_RECONCILE_PAYINFO_MAIN SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TF_RECONCILE_PAYINFO_MAIN " 
        + " WHERE "
        + "I_VOUSRLNO = ?"
        ;
  ...
  {
     TfReconcilePayinfoMainDto dto = (TfReconcilePayinfoMainDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TfReconcilePayinfoMainDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TfReconcilePayinfoMainDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TfReconcilePayinfoMainPK pk = (TfReconcilePayinfoMainPK)_pk ;
    ...
    {
        TfReconcilePayinfoMainPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TfReconcilePayinfoMainPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TfReconcilePayinfoMainPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TfReconcilePayinfoMainDto[] dtos = new TfReconcilePayinfoMainDto[0];
		    dtos = (TfReconcilePayinfoMainDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TfReconcilePayinfoMainDto  dto = new  TfReconcilePayinfoMainDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                                                             		throw new SQLException("数据库表：TF_RECONCILE_PAYINFO_MAIN没有检查修改的字段");
    }
