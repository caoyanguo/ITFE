...
/**
 * <p>Title: DAO of table: TS_USERSTAMP</p>
...
public class TsUserstampDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_USERSTAMP ("
          + "S_ORGCODE,S_USERCODE,S_STAMPTYPECODE,S_STAMPID,S_ISVALID"
          + ",I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_USERSTAMP.S_ORGCODE, TS_USERSTAMP.S_USERCODE, TS_USERSTAMP.S_STAMPTYPECODE, TS_USERSTAMP.S_STAMPID, TS_USERSTAMP.S_ISVALID, "
        + "TS_USERSTAMP.I_MODICOUNT "
        + "FROM TS_USERSTAMP "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_USERSTAMP "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_USERSTAMP SET "
        + "S_STAMPID =?,S_ISVALID =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_USERCODE = ? AND S_STAMPTYPECODE = ?"
        ;
  ...
  {
     TsUserstampDto dto = (TsUserstampDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(4, dto.getSstampid());

          ps.setString(5, dto.getSisvalid());
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsUserstampPK pk = (TsUserstampPK)_pk ;
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsUserstampPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*3+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*3+2, pk.getSusercode());

                   ps.setString((i-iBegin)*3+3, pk.getSstamptypecode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsUserstampDto[] dtos = new TsUserstampDto[0];
		    dtos = (TsUserstampDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_ISVALID
             str = rs.getString("S_ISVALID");
             if (str == null)
                dto.setSisvalid(null);
             else
                dto.setSisvalid(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_ISVALID
            ps.setString(pos, dto.getSisvalid());
            pos++;
    ...
    */
    ...
    {
         		throw new SQLException("数据库表：TS_USERSTAMP没有检查修改的字段");
    }
