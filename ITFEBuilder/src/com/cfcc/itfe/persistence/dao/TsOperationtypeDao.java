...
/**
 * <p>Title: DAO of table: TS_OPERATIONTYPE</p>
...
public class TsOperationtypeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_OPERATIONTYPE ("
          + "S_OPERATIONTYPECODE,S_OPERATIONTYPENAME,S_ISSEND,S_ISRECV,S_SHOWSCALE"
          + ",I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_OPERATIONTYPE.S_OPERATIONTYPECODE, TS_OPERATIONTYPE.S_OPERATIONTYPENAME, TS_OPERATIONTYPE.S_ISSEND, TS_OPERATIONTYPE.S_ISRECV, TS_OPERATIONTYPE.S_SHOWSCALE, "
        + "TS_OPERATIONTYPE.I_MODICOUNT "
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_OPERATIONTYPE "
        +" WHERE " 
        + "S_OPERATIONTYPECODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_OPERATIONTYPE SET "
        + "S_OPERATIONTYPENAME =?,S_ISSEND =?,S_ISRECV =?,S_SHOWSCALE =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_OPERATIONTYPECODE = ?"
        ;
  ...
  {
     TsOperationtypeDto dto = (TsOperationtypeDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ps.setString(1, dto.getSoperationtypecode());

          ps.setString(2, dto.getSoperationtypename());

          ps.setString(3, dto.getSissend());

          ps.setString(4, dto.getSisrecv());

          if (dto.getSshowscale()==null)
            ps.setNull(5, java.sql.Types.INTEGER);
         else
            ps.setInt(5, dto.getSshowscale().intValue());
   ...
   */
	...
	*/
    ...
    {
        TsOperationtypeDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsOperationtypeDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsOperationtypePK pk = (TsOperationtypePK)_pk ;

       PreparedStatement ps = null;
       ResultSet rs = null;
       try {
           ps = conn.prepareStatement(SQL_SELECT);
           ps.setString(1, pk.getSoperationtypecode());
    ...
    {
        TsOperationtypePK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsOperationtypePK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsOperationtypePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*1+1, pk.getSoperationtypecode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsOperationtypeDto[] dtos = new TsOperationtypeDto[0];
		    dtos = (TsOperationtypeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_OPERATIONTYPENAME
             str = rs.getString("S_OPERATIONTYPENAME");
             if (str == null)
                dto.setSoperationtypename(null);
             else
                dto.setSoperationtypename(str.trim());

             //S_ISSEND
             str = rs.getString("S_ISSEND");
             if (str == null)
                dto.setSissend(null);
             else
                dto.setSissend(str.trim());

             //S_ISRECV
             str = rs.getString("S_ISRECV");
             if (str == null)
                dto.setSisrecv(null);
             else
                dto.setSisrecv(str.trim());

             //S_SHOWSCALE
             str = rs.getString("S_SHOWSCALE");
             if(str!=null){
                dto.setSshowscale(new Integer(str));
             }
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_OPERATIONTYPENAME
            ps.setString(pos, dto.getSoperationtypename());
            pos++;

            //S_ISSEND
            ps.setString(pos, dto.getSissend());
            pos++;

            //S_ISRECV
            ps.setString(pos, dto.getSisrecv());
            pos++;
            ...
            if (dto.getSshowscale()==null)
                ...
                ps.setInt(pos, dto.getSshowscale().intValue());
            pos++;
    ...
    */
    ...
    {
               		throw new SQLException("数据库表：TS_OPERATIONTYPE没有检查修改的字段");
    }
