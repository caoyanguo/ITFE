...
import com.cfcc.itfe.persistence.dto.TsBankorgcodeDto ;
import com.cfcc.itfe.persistence.pk.TsBankorgcodePK ;


/**
 * <p>Title: DAO of table: TS_BANKORGCODE</p>
...
public class TsBankorgcodeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_BANKORGCODE ("
          + "S_ORGCODE,S_BANKNO,S_BANKNAME,S_BANKORGCODE"
        + ") VALUES ("
        + "?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_BANKORGCODE.S_ORGCODE, TS_BANKORGCODE.S_BANKNO, TS_BANKORGCODE.S_BANKNAME, TS_BANKORGCODE.S_BANKORGCODE "
        + "FROM TS_BANKORGCODE "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_BANKORGCODE SET "
        + "S_BANKNAME =?,S_BANKORGCODE =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_BANKNO = ?"
        ;
  ...
  {
     TsBankorgcodeDto dto = (TsBankorgcodeDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(4, dto.getSbankorgcode());
   ...
   */
	...
	*/
    ...
    {
        TsBankorgcodeDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsBankorgcodeDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsBankorgcodePK pk = (TsBankorgcodePK)_pk ;
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        if (results!=null && results.size() > 0)
        {
            TsBankorgcodeDto[] dtos = new TsBankorgcodeDto[0];
		    dtos = (TsBankorgcodeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsBankorgcodeDto  dto = new  TsBankorgcodeDto ();
             //S_ORGCODE
             ...
             //S_BANKORGCODE
             str = rs.getString("S_BANKORGCODE");
             if (str == null)
                dto.setSbankorgcode(null);
             else
                dto.setSbankorgcode(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_BANKORGCODE
            ps.setString(pos, dto.getSbankorgcode());
            pos++;
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_BANKORGCODE没有检查修改的字段");
    }
