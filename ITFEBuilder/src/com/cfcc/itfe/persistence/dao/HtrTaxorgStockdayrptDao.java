...
import com.cfcc.itfe.persistence.dto.HtrTaxorgStockdayrptDto ;
import com.cfcc.itfe.persistence.pk.HtrTaxorgStockdayrptPK ;


/**
 * <p>Title: DAO of table: HTR_TAXORG_STOCKDAYRPT</p>
...
public class HtrTaxorgStockdayrptDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTR_TAXORG_STOCKDAYRPT ("
          + "S_TAXORGCODE,S_TRECODE,S_RPTDATE,S_ACCNO,S_ACCNAME"
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTR_TAXORG_STOCKDAYRPT.S_TAXORGCODE, HTR_TAXORG_STOCKDAYRPT.S_TRECODE, HTR_TAXORG_STOCKDAYRPT.S_RPTDATE, HTR_TAXORG_STOCKDAYRPT.S_ACCNO, HTR_TAXORG_STOCKDAYRPT.S_ACCNAME, "
        + "HTR_TAXORG_STOCKDAYRPT.S_ACCDATE, HTR_TAXORG_STOCKDAYRPT.N_MONEYYESTERDAY, HTR_TAXORG_STOCKDAYRPT.N_MONEYIN, HTR_TAXORG_STOCKDAYRPT.N_MONEYOUT, HTR_TAXORG_STOCKDAYRPT.N_MONEYTODAY "
        + "FROM HTR_TAXORG_STOCKDAYRPT "
        +" WHERE " 
        + "S_TAXORGCODE = ? AND S_TRECODE = ? AND S_RPTDATE = ? AND S_ACCNO = ? AND S_ACCDATE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM HTR_TAXORG_STOCKDAYRPT "
        +" WHERE " 
        + "S_TAXORGCODE = ? AND S_TRECODE = ? AND S_RPTDATE = ? AND S_ACCNO = ? AND S_ACCDATE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTR_TAXORG_STOCKDAYRPT SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTR_TAXORG_STOCKDAYRPT " 
        + " WHERE "
        + "S_TAXORGCODE = ? AND S_TRECODE = ? AND S_RPTDATE = ? AND S_ACCNO = ? AND S_ACCDATE = ?"
        ;
  ...
  {
     HtrTaxorgStockdayrptDto dto = (HtrTaxorgStockdayrptDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtrTaxorgStockdayrptDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtrTaxorgStockdayrptDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtrTaxorgStockdayrptPK pk = (HtrTaxorgStockdayrptPK)_pk ;
    ...
    {
        HtrTaxorgStockdayrptPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtrTaxorgStockdayrptPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtrTaxorgStockdayrptPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*5+1, pk.getStaxorgcode());
        ...
        if (results!=null && results.size() > 0)
        {
            HtrTaxorgStockdayrptDto[] dtos = new HtrTaxorgStockdayrptDto[0];
		    dtos = (HtrTaxorgStockdayrptDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtrTaxorgStockdayrptDto  dto = new  HtrTaxorgStockdayrptDto ();
             //S_TAXORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
               		throw new SQLException("数据库表：HTR_TAXORG_STOCKDAYRPT没有检查修改的字段");
    }
