...
/**
 * <p>Title: DAO of table: TS_INFOCONNORGACC</p>
...
public class TsInfoconnorgaccDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_INFOCONNORGACC ("
          + "S_ORGCODE,S_TRECODE,S_PAYERACCOUNT,S_PAYERNAME,S_PAYERADDR"
          + ",S_BILLORGCODE,S_BIZTYPE,I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_INFOCONNORGACC.S_ORGCODE, TS_INFOCONNORGACC.S_TRECODE, TS_INFOCONNORGACC.S_PAYERACCOUNT, TS_INFOCONNORGACC.S_PAYERNAME, TS_INFOCONNORGACC.S_PAYERADDR, "
        + "TS_INFOCONNORGACC.S_BILLORGCODE, TS_INFOCONNORGACC.S_BIZTYPE, TS_INFOCONNORGACC.I_MODICOUNT "
        + "FROM TS_INFOCONNORGACC "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_PAYERACCOUNT = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_INFOCONNORGACC "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_PAYERACCOUNT = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_INFOCONNORGACC SET "
        + "S_PAYERNAME =?,S_PAYERADDR =?,S_BILLORGCODE =?,S_BIZTYPE =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_PAYERACCOUNT = ?"
        ;
  ...
  {
     TsInfoconnorgaccDto dto = (TsInfoconnorgaccDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(3, dto.getSpayeraccount());

          ps.setString(4, dto.getSpayername());

          ps.setString(5, dto.getSpayeraddr());

          ps.setString(6, dto.getSbillorgcode());

          ps.setString(7, dto.getSbiztype());
   ...
   */
	...
	*/
    ...
    {
        TsInfoconnorgaccDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsInfoconnorgaccDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsInfoconnorgaccPK pk = (TsInfoconnorgaccPK)_pk ;
       ...
       try {
           ...
           ps.setString(3, pk.getSpayeraccount());
    ...
    {
        TsInfoconnorgaccPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsInfoconnorgaccPK)_pks[i] ; 
        ...
        if (results!=null && results.size() > 0)
        {
            TsInfoconnorgaccDto[] dtos = new TsInfoconnorgaccDto[0];
		    dtos = (TsInfoconnorgaccDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsInfoconnorgaccDto  dto = new  TsInfoconnorgaccDto ();
             //S_ORGCODE
             ...
             //S_PAYERACCOUNT
             str = rs.getString("S_PAYERACCOUNT");
             if (str == null)
                dto.setSpayeraccount(null);
             else
                dto.setSpayeraccount(str.trim());
             ...
             //S_BILLORGCODE
             str = rs.getString("S_BILLORGCODE");
             if (str == null)
                dto.setSbillorgcode(null);
             else
                dto.setSbillorgcode(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_BILLORGCODE
            ps.setString(pos, dto.getSbillorgcode());
            pos++;
           ...
           pos++;
           //S_PAYERACCOUNT
           ps.setString(pos, dto.getSpayeraccount());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
               		throw new SQLException("数据库表：TS_INFOCONNORGACC没有检查修改的字段");
    }
