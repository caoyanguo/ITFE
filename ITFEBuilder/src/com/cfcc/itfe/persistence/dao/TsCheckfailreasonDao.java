...
/**
 * <p>Title: DAO of table: TS_CHECKFAILREASON</p>
...
public class TsCheckfailreasonDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CHECKFAILREASON ("
          + "S_ORGCODE,S_CHECKFAILCODE,S_CHECKFAILDSP,TS_SYSUPDATE"
        + ") VALUES ("
        + "?,?,?,CURRENT TIMESTAMP )";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CHECKFAILREASON.S_ORGCODE, TS_CHECKFAILREASON.S_CHECKFAILCODE, TS_CHECKFAILREASON.S_CHECKFAILDSP, TS_CHECKFAILREASON.TS_SYSUPDATE "
        + "FROM TS_CHECKFAILREASON "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_CHECKFAILCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CHECKFAILREASON "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_CHECKFAILCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CHECKFAILREASON SET "
        + "S_CHECKFAILDSP =?,TS_SYSUPDATE =CURRENT TIMESTAMP "
        + "WHERE "
        + "S_ORGCODE = ? AND S_CHECKFAILCODE = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TS_CHECKFAILREASON " 
        + " WHERE "
        + "S_ORGCODE = ? AND S_CHECKFAILCODE = ?"
        ;
  ...
  {
     TsCheckfailreasonDto dto = (TsCheckfailreasonDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getScheckfailcode());

          ps.setString(3, dto.getScheckfaildsp());
   ...
   */
	...
	*/
    ...
    {
        TsCheckfailreasonDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsCheckfailreasonDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsCheckfailreasonPK pk = (TsCheckfailreasonPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getScheckfailcode());
    ...
    {
        TsCheckfailreasonPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsCheckfailreasonPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsCheckfailreasonPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getScheckfailcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsCheckfailreasonDto[] dtos = new TsCheckfailreasonDto[0];
		    dtos = (TsCheckfailreasonDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_CHECKFAILCODE
             str = rs.getString("S_CHECKFAILCODE");
             if (str == null)
                dto.setScheckfailcode(null);
             else
                dto.setScheckfailcode(str.trim());

             //S_CHECKFAILDSP
             str = rs.getString("S_CHECKFAILDSP");
             if (str == null)
                dto.setScheckfaildsp(null);
             else
                dto.setScheckfaildsp(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_CHECKFAILDSP
            ps.setString(pos, dto.getScheckfaildsp());
            pos++;
           ...
           pos++;
           //S_CHECKFAILCODE
           ps.setString(pos, dto.getScheckfailcode());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_CHECKFAILREASON没有检查修改的字段");
    }
