...
/**
 * <p>Title: DAO of table: HTV_FIN_INCOME_DETAIL</p>
...
public class HtvFinIncomeDetailDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTV_FIN_INCOME_DETAIL ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTV_FIN_INCOME_DETAIL.S_ORGCODE, HTV_FIN_INCOME_DETAIL.S_TRECODE, HTV_FIN_INCOME_DETAIL.S_INTREDATE, HTV_FIN_INCOME_DETAIL.I_PKGSEQNO, HTV_FIN_INCOME_DETAIL.S_EXPVOUNO, "
        + "HTV_FIN_INCOME_DETAIL.S_EXPVOUTYPE, HTV_FIN_INCOME_DETAIL.C_BDGLEVEL, HTV_FIN_INCOME_DETAIL.S_BDGSBTCODE, HTV_FIN_INCOME_DETAIL.C_BDGKIND, HTV_FIN_INCOME_DETAIL.F_AMT, "
        + "HTV_FIN_INCOME_DETAIL.C_VOUCHANNEL, HTV_FIN_INCOME_DETAIL.S_TAXORGCODE, HTV_FIN_INCOME_DETAIL.S_SEQ "
        + "FROM HTV_FIN_INCOME_DETAIL "
        +" WHERE " 
        + "S_SEQ = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTV_FIN_INCOME_DETAIL SET "
        + "S_ORGCODE =?,S_TRECODE =?,S_INTREDATE =?,I_PKGSEQNO =?,S_EXPVOUNO =?, "
        + "S_EXPVOUTYPE =?,C_BDGLEVEL =?,S_BDGSBTCODE =?,C_BDGKIND =?,F_AMT =?, "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTV_FIN_INCOME_DETAIL " 
        + " WHERE "
        + "S_SEQ = ?"
        ;
  ...
  {
     HtvFinIncomeDetailDto dto = (HtvFinIncomeDetailDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtvFinIncomeDetailDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtvFinIncomeDetailDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtvFinIncomeDetailPK pk = (HtvFinIncomeDetailPK)_pk ;
    ...
    {
        HtvFinIncomeDetailPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtvFinIncomeDetailPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtvFinIncomeDetailPK)(pks.get(i)) ; 
        ...
        if (results!=null && results.size() > 0)
        {
            HtvFinIncomeDetailDto[] dtos = new HtvFinIncomeDetailDto[0];
		    dtos = (HtvFinIncomeDetailDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                    		throw new SQLException("数据库表：HTV_FIN_INCOME_DETAIL没有检查修改的字段");
    }
