...
import com.cfcc.itfe.persistence.dto.HtfReconcilePayquotaSubDto ;
import com.cfcc.itfe.persistence.pk.HtfReconcilePayquotaSubPK ;


/**
 * <p>Title: DAO of table: HTF_RECONCILE_PAYQUOTA_SUB</p>
 * <p>Description:清算额度对帐子表历史表3510 Data Access Object  </p>
...
public class HtfReconcilePayquotaSubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_RECONCILE_PAYQUOTA_SUB ("
          + "I_VOUSRLNO,I_SEQNO,S_ID,S_SUPDEPCODE,S_SUPDEPNAME"
          + ",S_FUNDTYPECODE,S_FUNDTYPENAME,S_PAYBANKCODE,S_PAYBANKNAME,S_PAYBANKNO"
          + ",S_EXPFUNCCODE,S_EXPFUNCNAME,S_PROCATCODE,S_PROCATNAME,S_PAYTYPECODE"
          + ",S_PAYTYPENAME,N_PREDATEMONEY,N_CLEARAMT,N_CURRECKMONEY,N_CURDATEMONEY"
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_RECONCILE_PAYQUOTA_SUB.I_VOUSRLNO, HTF_RECONCILE_PAYQUOTA_SUB.I_SEQNO, HTF_RECONCILE_PAYQUOTA_SUB.S_ID, HTF_RECONCILE_PAYQUOTA_SUB.S_SUPDEPCODE, HTF_RECONCILE_PAYQUOTA_SUB.S_SUPDEPNAME, "
        + "HTF_RECONCILE_PAYQUOTA_SUB.S_FUNDTYPECODE, HTF_RECONCILE_PAYQUOTA_SUB.S_FUNDTYPENAME, HTF_RECONCILE_PAYQUOTA_SUB.S_PAYBANKCODE, HTF_RECONCILE_PAYQUOTA_SUB.S_PAYBANKNAME, HTF_RECONCILE_PAYQUOTA_SUB.S_PAYBANKNO, "
        + "HTF_RECONCILE_PAYQUOTA_SUB.S_EXPFUNCCODE, HTF_RECONCILE_PAYQUOTA_SUB.S_EXPFUNCNAME, HTF_RECONCILE_PAYQUOTA_SUB.S_PROCATCODE, HTF_RECONCILE_PAYQUOTA_SUB.S_PROCATNAME, HTF_RECONCILE_PAYQUOTA_SUB.S_PAYTYPECODE, "
        + "HTF_RECONCILE_PAYQUOTA_SUB.S_PAYTYPENAME, HTF_RECONCILE_PAYQUOTA_SUB.N_PREDATEMONEY, HTF_RECONCILE_PAYQUOTA_SUB.N_CLEARAMT, HTF_RECONCILE_PAYQUOTA_SUB.N_CURRECKMONEY, HTF_RECONCILE_PAYQUOTA_SUB.N_CURDATEMONEY, "
        + "HTF_RECONCILE_PAYQUOTA_SUB.S_XCHECKRESULT, HTF_RECONCILE_PAYQUOTA_SUB.S_XCHECKREASON, HTF_RECONCILE_PAYQUOTA_SUB.S_HOLD1, HTF_RECONCILE_PAYQUOTA_SUB.S_HOLD2, HTF_RECONCILE_PAYQUOTA_SUB.S_HOLD3, "
        + "HTF_RECONCILE_PAYQUOTA_SUB.S_HOLD4, HTF_RECONCILE_PAYQUOTA_SUB.S_EXT1, HTF_RECONCILE_PAYQUOTA_SUB.S_EXT2, HTF_RECONCILE_PAYQUOTA_SUB.S_EXT3, HTF_RECONCILE_PAYQUOTA_SUB.S_EXT4, "
        + "HTF_RECONCILE_PAYQUOTA_SUB.S_EXT5 "
        + "FROM HTF_RECONCILE_PAYQUOTA_SUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_RECONCILE_PAYQUOTA_SUB SET "
        + "S_ID =?,S_SUPDEPCODE =?,S_SUPDEPNAME =?,S_FUNDTYPECODE =?,S_FUNDTYPENAME =?, "
        + "S_PAYBANKCODE =?,S_PAYBANKNAME =?,S_PAYBANKNO =?,S_EXPFUNCCODE =?,S_EXPFUNCNAME =?, "
        + "S_PROCATCODE =?,S_PROCATNAME =?,S_PAYTYPECODE =?,S_PAYTYPENAME =?,N_PREDATEMONEY =?, "
        + "N_CLEARAMT =?,N_CURRECKMONEY =?,N_CURDATEMONEY =?,S_XCHECKRESULT =?,S_XCHECKREASON =?, "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_RECONCILE_PAYQUOTA_SUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     HtfReconcilePayquotaSubDto dto = (HtfReconcilePayquotaSubDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(4, dto.getSsupdepcode());

          ps.setString(5, dto.getSsupdepname());

          ps.setString(6, dto.getSfundtypecode());

          ps.setString(7, dto.getSfundtypename());

          ps.setString(8, dto.getSpaybankcode());

          ps.setString(9, dto.getSpaybankname());

          ps.setString(10, dto.getSpaybankno());

          ps.setString(11, dto.getSexpfunccode());

          ps.setString(12, dto.getSexpfuncname());

          ps.setString(13, dto.getSprocatcode());

          ps.setString(14, dto.getSprocatname());

          ps.setString(15, dto.getSpaytypecode());

          ps.setString(16, dto.getSpaytypename());

          ps.setBigDecimal(17, dto.getNpredatemoney());

          ps.setBigDecimal(18, dto.getNclearamt());

          ps.setBigDecimal(19, dto.getNcurreckmoney());

          ps.setBigDecimal(20, dto.getNcurdatemoney());

          ps.setString(21, dto.getSxcheckresult());

          ps.setString(22, dto.getSxcheckreason());

          ps.setString(23, dto.getShold1());

          ps.setString(24, dto.getShold2());

          ps.setString(25, dto.getShold3());

          ps.setString(26, dto.getShold4());
   ...
   */
	...
	*/
    ...
    {
        HtfReconcilePayquotaSubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfReconcilePayquotaSubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfReconcilePayquotaSubPK pk = (HtfReconcilePayquotaSubPK)_pk ;
    ...
    {
        HtfReconcilePayquotaSubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfReconcilePayquotaSubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfReconcilePayquotaSubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfReconcilePayquotaSubDto[] dtos = new HtfReconcilePayquotaSubDto[0];
		    dtos = (HtfReconcilePayquotaSubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfReconcilePayquotaSubDto  dto = new  HtfReconcilePayquotaSubDto ();
             //I_VOUSRLNO
             ...
             //N_PREDATEMONEY
           dto.setNpredatemoney(rs.getBigDecimal("N_PREDATEMONEY"));

             //N_CLEARAMT
           dto.setNclearamt(rs.getBigDecimal("N_CLEARAMT"));

             //N_CURRECKMONEY
           dto.setNcurreckmoney(rs.getBigDecimal("N_CURRECKMONEY"));

             //N_CURDATEMONEY
           dto.setNcurdatemoney(rs.getBigDecimal("N_CURDATEMONEY"));
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //N_PREDATEMONEY
            ps.setBigDecimal(pos, dto.getNpredatemoney());
            pos++;

            //N_CLEARAMT
            ps.setBigDecimal(pos, dto.getNclearamt());
            pos++;

            //N_CURRECKMONEY
            ps.setBigDecimal(pos, dto.getNcurreckmoney());
            pos++;

            //N_CURDATEMONEY
            ps.setBigDecimal(pos, dto.getNcurdatemoney());
            pos++;
    ...
    */
    ...
    {
                                                                                       		throw new SQLException("数据库表：HTF_RECONCILE_PAYQUOTA_SUB没有检查修改的字段");
    }
