...
/**
 * <p>Title: DAO of table: TD_BANK</p>
...
public class TdBankDao  implements IDao
{
    ...
    private static final String SQL_INSERT =
        "INSERT INTO TD_BANK ("
          + "I_SEQNO,S_BOOKORGCODE,S_BNKCODE,S_BNKNAME,S_OFBNKFLAG"
          + ",S_OFDRCTBNKNO,S_OFINDRCTBNKNO,S_CITYEXCHGNO,TS_SYSUPDATE"
        + ") VALUES ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TD_BANK.I_SEQNO, TD_BANK.S_BOOKORGCODE, TD_BANK.S_BNKCODE, TD_BANK.S_BNKNAME, TD_BANK.S_OFBNKFLAG, "
        + "TD_BANK.S_OFDRCTBNKNO, TD_BANK.S_OFINDRCTBNKNO, TD_BANK.S_CITYEXCHGNO, TD_BANK.TS_SYSUPDATE "
        + "FROM TD_BANK "
        +" WHERE " 
        + "I_SEQNO = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TD_BANK SET "
        + "S_BOOKORGCODE =?,S_BNKCODE =?,S_BNKNAME =?,S_OFBNKFLAG =?,S_OFDRCTBNKNO =?, "
        + "S_OFINDRCTBNKNO =?,S_CITYEXCHGNO =?,TS_SYSUPDATE =CURRENT TIMESTAMP "
        + "WHERE "
        + "I_SEQNO = ?"
        ;
  ...
  public void create(IDto _dto,  Connection conn) throws SQLException
  {
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getSbnkcode());

          ps.setString(3, dto.getSbnkname());

          ps.setString(4, dto.getSofbnkflag());

          ps.setString(5, dto.getSofdrctbnkno());

          ps.setString(6, dto.getSofindrctbnkno());

          ps.setString(7, dto.getScityexchgno());
   ...
   */
	...
	*/
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        if (results!=null && results.size() > 0)
        {
            TdBankDto[] dtos = new TdBankDto[0];
		    dtos = (TdBankDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_BNKCODE
             str = rs.getString("S_BNKCODE");
             if (str == null)
                dto.setSbnkcode(null);
             else
                dto.setSbnkcode(str.trim());

             //S_BNKNAME
             str = rs.getString("S_BNKNAME");
             if (str == null)
                dto.setSbnkname(null);
             else
                dto.setSbnkname(str.trim());

             //S_OFBNKFLAG
             str = rs.getString("S_OFBNKFLAG");
             if (str == null)
                dto.setSofbnkflag(null);
             else
                dto.setSofbnkflag(str.trim());

             //S_OFDRCTBNKNO
             str = rs.getString("S_OFDRCTBNKNO");
             if (str == null)
                dto.setSofdrctbnkno(null);
             else
                dto.setSofdrctbnkno(str.trim());

             //S_OFINDRCTBNKNO
             str = rs.getString("S_OFINDRCTBNKNO");
             if (str == null)
                dto.setSofindrctbnkno(null);
             else
                dto.setSofindrctbnkno(str.trim());

             //S_CITYEXCHGNO
             str = rs.getString("S_CITYEXCHGNO");
             if (str == null)
                dto.setScityexchgno(null);
             else
                dto.setScityexchgno(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_BNKCODE
            ps.setString(pos, dto.getSbnkcode());
            pos++;

            //S_BNKNAME
            ps.setString(pos, dto.getSbnkname());
            pos++;

            //S_OFBNKFLAG
            ps.setString(pos, dto.getSofbnkflag());
            pos++;

            //S_OFDRCTBNKNO
            ps.setString(pos, dto.getSofdrctbnkno());
            pos++;

            //S_OFINDRCTBNKNO
            ps.setString(pos, dto.getSofindrctbnkno());
            pos++;

            //S_CITYEXCHGNO
            ps.setString(pos, dto.getScityexchgno());
            pos++;
    ...
    */
    ...
    {
                        		throw new SQLException("数据库表：TD_BANK没有检查修改的字段");
    }
