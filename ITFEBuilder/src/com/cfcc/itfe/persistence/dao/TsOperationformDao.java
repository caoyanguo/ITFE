...
/**
 * <p>Title: DAO of table: TS_OPERATIONFORM</p>
...
public class TsOperationformDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_OPERATIONFORM ("
          + "S_MODELID,S_FORMID,S_FORMTYPE,I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_OPERATIONFORM.S_MODELID, TS_OPERATIONFORM.S_FORMID, TS_OPERATIONFORM.S_FORMTYPE, TS_OPERATIONFORM.I_MODICOUNT "
        + "FROM TS_OPERATIONFORM "
        +" WHERE " 
        + "S_MODELID = ? AND S_FORMID = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_OPERATIONFORM "
        +" WHERE " 
        + "S_MODELID = ? AND S_FORMID = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_OPERATIONFORM SET "
        + "S_FORMTYPE =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_MODELID = ? AND S_FORMID = ?"
        ;
  ...
  {
     TsOperationformDto dto = (TsOperationformDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(3, dto.getSformtype());
   ...
   */
	...
	*/
    ...
    {
        TsOperationformDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsOperationformDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsOperationformPK pk = (TsOperationformPK)_pk ;
    ...
    {
        TsOperationformPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsOperationformPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsOperationformPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSmodelid());

                   ps.setString((i-iBegin)*2+2, pk.getSformid());
        ...
        if (results!=null && results.size() > 0)
        {
            TsOperationformDto[] dtos = new TsOperationformDto[0];
		    dtos = (TsOperationformDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsOperationformDto  dto = new  TsOperationformDto ();
             //S_MODELID
             ...
             //S_FORMTYPE
             str = rs.getString("S_FORMTYPE");
             if (str == null)
                dto.setSformtype(null);
             else
                dto.setSformtype(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_FORMTYPE
            ps.setString(pos, dto.getSformtype());
            pos++;
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_OPERATIONFORM没有检查修改的字段");
    }
