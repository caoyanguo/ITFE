...
/**
 * <p>Title: DAO of table: TS_TAXORG</p>
...
public class TsTaxorgDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_TAXORG ("
          + "S_ORGCODE,S_TRECODE,S_TAXORGCODE,S_TAXORGNAME,S_TAXPROP"
        + ") VALUES ("
        + "?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_TAXORG.S_ORGCODE, TS_TAXORG.S_TRECODE, TS_TAXORG.S_TAXORGCODE, TS_TAXORG.S_TAXORGNAME, TS_TAXORG.S_TAXPROP "
        + "FROM TS_TAXORG "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_TAXORGCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_TAXORG "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_TAXORGCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_TAXORG SET "
        + "S_TAXORGNAME =?,S_TAXPROP =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_TAXORGCODE = ?"
        ;
  ...
  {
     TsTaxorgDto dto = (TsTaxorgDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(5, dto.getStaxprop());
   ...
   */
	...
	*/
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        if (results!=null && results.size() > 0)
        {
            TsTaxorgDto[] dtos = new TsTaxorgDto[0];
		    dtos = (TsTaxorgDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_TAXPROP
             str = rs.getString("S_TAXPROP");
             if (str == null)
                dto.setStaxprop(null);
             else
                dto.setStaxprop(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_TAXPROP
            ps.setString(pos, dto.getStaxprop());
            pos++;
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_TAXORG没有检查修改的字段");
    }
