...
/**
 * <p>Title: DAO of table: TS_BUDGETSUBJECTFORQUERY</p>
...
public class TsBudgetsubjectforqueryDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_BUDGETSUBJECTFORQUERY ("
          + "S_ORGCODE,S_SUBJECTCODE,S_SUBJECTNAME"
        + ") VALUES ("
        + "?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_BUDGETSUBJECTFORQUERY.S_ORGCODE, TS_BUDGETSUBJECTFORQUERY.S_SUBJECTCODE, TS_BUDGETSUBJECTFORQUERY.S_SUBJECTNAME "
        + "FROM TS_BUDGETSUBJECTFORQUERY "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_SUBJECTCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_BUDGETSUBJECTFORQUERY "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_SUBJECTCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_BUDGETSUBJECTFORQUERY SET "
        + "S_SUBJECTNAME =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_SUBJECTCODE = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TS_BUDGETSUBJECTFORQUERY " 
        + " WHERE "
        + "S_ORGCODE = ? AND S_SUBJECTCODE = ?"
        ;
  ...
  {
     TsBudgetsubjectforqueryDto dto = (TsBudgetsubjectforqueryDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TsBudgetsubjectforqueryDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsBudgetsubjectforqueryDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsBudgetsubjectforqueryPK pk = (TsBudgetsubjectforqueryPK)_pk ;
    ...
    {
        TsBudgetsubjectforqueryPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsBudgetsubjectforqueryPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsBudgetsubjectforqueryPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getSsubjectcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsBudgetsubjectforqueryDto[] dtos = new TsBudgetsubjectforqueryDto[0];
		    dtos = (TsBudgetsubjectforqueryDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsBudgetsubjectforqueryDto  dto = new  TsBudgetsubjectforqueryDto ();
             //S_ORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
   		throw new SQLException("数据库表：TS_BUDGETSUBJECTFORQUERY没有检查修改的字段");
    }
