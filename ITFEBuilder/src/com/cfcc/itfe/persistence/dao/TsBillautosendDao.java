...
/**
 * <p>Title: DAO of table: TS_BILLAUTOSEND</p>
...
public class TsBillautosendDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_BILLAUTOSEND ("
          + "S_ORGCODE,S_TRECODE,S_TAXORGCODE,S_BILLKIND,S_BUDGETTYPE"
          + ",S_BUDGETLEVELCODE,S_BELONGFLAG,S_TRIMFLAG,S_ANDWHERE,S_ISAUTO"
          + ",TS_SYSUPDATE"
        + ") VALUES ("
        + "?,?,?,?,?,?,?,?,?,?,CURRENT TIMESTAMP )";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_BILLAUTOSEND.S_ORGCODE, TS_BILLAUTOSEND.S_TRECODE, TS_BILLAUTOSEND.S_TAXORGCODE, TS_BILLAUTOSEND.S_BILLKIND, TS_BILLAUTOSEND.S_BUDGETTYPE, "
        + "TS_BILLAUTOSEND.S_BUDGETLEVELCODE, TS_BILLAUTOSEND.S_BELONGFLAG, TS_BILLAUTOSEND.S_TRIMFLAG, TS_BILLAUTOSEND.S_ANDWHERE, TS_BILLAUTOSEND.S_ISAUTO, "
        + "TS_BILLAUTOSEND.TS_SYSUPDATE "
        + "FROM TS_BILLAUTOSEND "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_TAXORGCODE = ? AND S_BILLKIND = ? AND S_BUDGETTYPE = ? AND "
        + "S_BUDGETLEVELCODE = ? AND S_TRIMFLAG = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_BILLAUTOSEND "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_TAXORGCODE = ? AND S_BILLKIND = ? AND S_BUDGETTYPE = ? AND  FOR UPDATE"
        + "S_BUDGETLEVELCODE = ? AND S_TRIMFLAG = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_BILLAUTOSEND SET "
        + "S_BELONGFLAG =?,S_ANDWHERE =?,S_ISAUTO =?,TS_SYSUPDATE =CURRENT TIMESTAMP "
        + "WHERE "
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_TAXORGCODE = ? AND S_BILLKIND = ? AND S_BUDGETTYPE = ? AND "
        + "S_BUDGETLEVELCODE = ? AND S_TRIMFLAG = ?"
        ;
  ...
  {
     TsBillautosendDto dto = (TsBillautosendDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(4, dto.getSbillkind());

          ps.setString(5, dto.getSbudgettype());

          ps.setString(6, dto.getSbudgetlevelcode());

          ps.setString(7, dto.getSbelongflag());

          ps.setString(8, dto.getStrimflag());

          ps.setString(9, dto.getSandwhere());

          ps.setString(10, dto.getSisauto());
   ...
   */
	...
	*/
    ...
    {
        TsBillautosendDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsBillautosendDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsBillautosendPK pk = (TsBillautosendPK)_pk ;
       ...
       try {
           ...
           ps.setString(3, pk.getStaxorgcode());

           ps.setString(4, pk.getSbillkind());

           ps.setString(5, pk.getSbudgettype());

           ps.setString(6, pk.getSbudgetlevelcode());

           ps.setString(7, pk.getStrimflag());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsBillautosendPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*7+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*7+2, pk.getStrecode());

                   ps.setString((i-iBegin)*7+3, pk.getStaxorgcode());

                   ps.setString((i-iBegin)*7+4, pk.getSbillkind());

                   ps.setString((i-iBegin)*7+5, pk.getSbudgettype());

                   ps.setString((i-iBegin)*7+6, pk.getSbudgetlevelcode());

                   ps.setString((i-iBegin)*7+7, pk.getStrimflag());
        ...
        if (results!=null && results.size() > 0)
        {
            TsBillautosendDto[] dtos = new TsBillautosendDto[0];
		    dtos = (TsBillautosendDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsBillautosendDto  dto = new  TsBillautosendDto ();
             //S_ORGCODE
             ...
             //S_ANDWHERE
             str = rs.getString("S_ANDWHERE");
             if (str == null)
                dto.setSandwhere(null);
             else
                dto.setSandwhere(str.trim());

             //S_ISAUTO
             str = rs.getString("S_ISAUTO");
             if (str == null)
                dto.setSisauto(null);
             else
                dto.setSisauto(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_ANDWHERE
            ps.setString(pos, dto.getSandwhere());
            pos++;

            //S_ISAUTO
            ps.setString(pos, dto.getSisauto());
            pos++;
    ...
    */
    ...
    {
            		throw new SQLException("数据库表：TS_BILLAUTOSEND没有检查修改的字段");
    }
