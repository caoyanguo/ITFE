...
/**
 * <p>Title: DAO of table: TS_CONNECTION</p>
...
public class TsConnectionDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CONNECTION ("
          + "S_CONNORGCODEA,S_CONNORGCODEB,I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CONNECTION.S_CONNORGCODEA, TS_CONNECTION.S_CONNORGCODEB, TS_CONNECTION.I_MODICOUNT "
        + "FROM TS_CONNECTION "
        +" WHERE " 
        + "S_CONNORGCODEA = ? AND S_CONNORGCODEB = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CONNECTION "
        +" WHERE " 
        + "S_CONNORGCODEA = ? AND S_CONNORGCODEB = ? FOR UPDATE"
        ;
  ...
  {
     TsConnectionDto dto = (TsConnectionDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ps.setString(1, dto.getSconnorgcodea());

          ps.setString(2, dto.getSconnorgcodeb());
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsConnectionPK pk = (TsConnectionPK)_pk ;

       PreparedStatement ps = null;
       ResultSet rs = null;
       try {
           ps = conn.prepareStatement(SQL_SELECT);
           ps.setString(1, pk.getSconnorgcodea());

           ps.setString(2, pk.getSconnorgcodeb());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsConnectionPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSconnorgcodea());

                   ps.setString((i-iBegin)*2+2, pk.getSconnorgcodeb());
        ...
        if (results!=null && results.size() > 0)
        {
            TsConnectionDto[] dtos = new TsConnectionDto[0];
		    dtos = (TsConnectionDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsConnectionDto  dto = new  TsConnectionDto ();
             //S_CONNORGCODEA
             str = rs.getString("S_CONNORGCODEA");
             if (str == null)
                dto.setSconnorgcodea(null);
             else
                dto.setSconnorgcodea(str.trim());

             //S_CONNORGCODEB
             str = rs.getString("S_CONNORGCODEB");
             if (str == null)
                dto.setSconnorgcodeb(null);
             else
                dto.setSconnorgcodeb(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
           ...
           //S_CONNORGCODEA
           ps.setString(pos, dto.getSconnorgcodea());
           pos++;
           //S_CONNORGCODEB
           ps.setString(pos, dto.getSconnorgcodeb());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
   		throw new SQLException("数据库表：TS_CONNECTION没有检查修改的字段");
    }
