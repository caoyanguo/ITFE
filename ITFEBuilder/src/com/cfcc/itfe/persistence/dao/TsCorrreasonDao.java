...
/**
 * <p>Title: DAO of table: TS_CORRREASON</p>
...
public class TsCorrreasonDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CORRREASON ("
          + "S_BOOKORGCODE,S_TBSCORRCODE,S_TBSCORRNAME,S_TCBSCORRCODE,S_TCBSCORRNAME"
          + ",TS_SYSUPDATE"
        + ") VALUES ("
        + "?,?,?,?,?,CURRENT TIMESTAMP )";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CORRREASON.S_BOOKORGCODE, TS_CORRREASON.S_TBSCORRCODE, TS_CORRREASON.S_TBSCORRNAME, TS_CORRREASON.S_TCBSCORRCODE, TS_CORRREASON.S_TCBSCORRNAME, "
        + "TS_CORRREASON.TS_SYSUPDATE "
        + "FROM TS_CORRREASON "
        +" WHERE " 
        + "S_BOOKORGCODE = ? AND S_TBSCORRCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CORRREASON "
        +" WHERE " 
        + "S_BOOKORGCODE = ? AND S_TBSCORRCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CORRREASON SET "
        + "S_TBSCORRNAME =?,S_TCBSCORRCODE =?,S_TCBSCORRNAME =?,TS_SYSUPDATE =CURRENT TIMESTAMP "
        + "WHERE "
        + "S_BOOKORGCODE = ? AND S_TBSCORRCODE = ?"
        ;
  ...
  {
     TsCorrreasonDto dto = (TsCorrreasonDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getStbscorrcode());

          ps.setString(3, dto.getStbscorrname());

          ps.setString(4, dto.getStcbscorrcode());

          ps.setString(5, dto.getStcbscorrname());
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsCorrreasonPK pk = (TsCorrreasonPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getStbscorrcode());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsCorrreasonPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSbookorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getStbscorrcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsCorrreasonDto[] dtos = new TsCorrreasonDto[0];
		    dtos = (TsCorrreasonDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsCorrreasonDto  dto = new  TsCorrreasonDto ();
             //S_BOOKORGCODE
             ...
             //S_TBSCORRCODE
             str = rs.getString("S_TBSCORRCODE");
             if (str == null)
                dto.setStbscorrcode(null);
             else
                dto.setStbscorrcode(str.trim());

             //S_TBSCORRNAME
             str = rs.getString("S_TBSCORRNAME");
             if (str == null)
                dto.setStbscorrname(null);
             else
                dto.setStbscorrname(str.trim());

             //S_TCBSCORRCODE
             str = rs.getString("S_TCBSCORRCODE");
             if (str == null)
                dto.setStcbscorrcode(null);
             else
                dto.setStcbscorrcode(str.trim());

             //S_TCBSCORRNAME
             str = rs.getString("S_TCBSCORRNAME");
             if (str == null)
                dto.setStcbscorrname(null);
             else
                dto.setStcbscorrname(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_TBSCORRNAME
            ps.setString(pos, dto.getStbscorrname());
            pos++;

            //S_TCBSCORRCODE
            ps.setString(pos, dto.getStcbscorrcode());
            pos++;

            //S_TCBSCORRNAME
            ps.setString(pos, dto.getStcbscorrname());
            pos++;
           ...
           pos++;
           //S_TBSCORRCODE
           ps.setString(pos, dto.getStbscorrcode());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
            		throw new SQLException("数据库表：TS_CORRREASON没有检查修改的字段");
    }
