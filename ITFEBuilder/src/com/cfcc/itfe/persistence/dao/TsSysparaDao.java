...
/**
 * <p>Title: DAO of table: TS_SYSPARA</p>
...
public class TsSysparaDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_SYSPARA ("
          + "S_ORGCODE,S_CODE,S_VALUE,S_DESCRIPT,S_OPERTYPE"
        + ") VALUES ("
        + "?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_SYSPARA.S_ORGCODE, TS_SYSPARA.S_CODE, TS_SYSPARA.S_VALUE, TS_SYSPARA.S_DESCRIPT, TS_SYSPARA.S_OPERTYPE "
        + "FROM TS_SYSPARA "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_CODE = ? AND S_OPERTYPE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_SYSPARA "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_CODE = ? AND S_OPERTYPE = ? FOR UPDATE"
        ;
  ...
  {
     TsSysparaDto dto = (TsSysparaDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(3, dto.getSvalue());

          ps.setString(4, dto.getSdescript());

          ps.setString(5, dto.getSopertype());
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsSysparaPK pk = (TsSysparaPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getScode());

           ps.setString(3, pk.getSopertype());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsSysparaPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*3+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*3+2, pk.getScode());

                   ps.setString((i-iBegin)*3+3, pk.getSopertype());
        ...
        if (results!=null && results.size() > 0)
        {
            TsSysparaDto[] dtos = new TsSysparaDto[0];
		    dtos = (TsSysparaDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_CODE
             str = rs.getString("S_CODE");
             if (str == null)
                dto.setScode(null);
             else
                dto.setScode(str.trim());
             ...
             //S_DESCRIPT
             str = rs.getString("S_DESCRIPT");
             if (str == null)
                dto.setSdescript(null);
             else
                dto.setSdescript(str.trim());

             //S_OPERTYPE
             str = rs.getString("S_OPERTYPE");
             if (str == null)
                dto.setSopertype(null);
             else
                dto.setSopertype(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_DESCRIPT
            ps.setString(pos, dto.getSdescript());
            pos++;
           ...
           pos++;
           //S_CODE
           ps.setString(pos, dto.getScode());
           pos++;
           //S_OPERTYPE
           ps.setString(pos, dto.getSopertype());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_SYSPARA没有检查修改的字段");
    }
