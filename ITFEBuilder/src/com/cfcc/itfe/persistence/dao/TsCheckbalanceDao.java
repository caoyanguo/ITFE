...
/**
 * <p>Title: DAO of table: TS_CHECKBALANCE</p>
...
public class TsCheckbalanceDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CHECKBALANCE ("
          + "S_ORGCODE,S_GROUPID,S_USERCODE,I_TOTALNUM,N_TOTALMONEY"
          + ",I_CHECKEDNUM,N_CHECKMONEY,S_ISBALANCE"
        + ") VALUES ("
        + "?,?,?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CHECKBALANCE.S_ORGCODE, TS_CHECKBALANCE.S_GROUPID, TS_CHECKBALANCE.S_USERCODE, TS_CHECKBALANCE.I_TOTALNUM, TS_CHECKBALANCE.N_TOTALMONEY, "
        + "TS_CHECKBALANCE.I_CHECKEDNUM, TS_CHECKBALANCE.N_CHECKMONEY, TS_CHECKBALANCE.S_ISBALANCE "
        + "FROM TS_CHECKBALANCE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_GROUPID = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CHECKBALANCE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_GROUPID = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CHECKBALANCE SET "
        + "S_USERCODE =?,I_TOTALNUM =?,N_TOTALMONEY =?,I_CHECKEDNUM =?,N_CHECKMONEY =?, "
        + "S_ISBALANCE =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_GROUPID = ?"
        ;
  ...
  {
     TsCheckbalanceDto dto = (TsCheckbalanceDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getSgroupid());

          ps.setString(3, dto.getSusercode());

          if (dto.getItotalnum()==null)
            ps.setNull(4, java.sql.Types.INTEGER);
         else
            ps.setInt(4, dto.getItotalnum().intValue());
          ps.setBigDecimal(5, dto.getNtotalmoney());

          if (dto.getIcheckednum()==null)
            ps.setNull(6, java.sql.Types.INTEGER);
         else
            ps.setInt(6, dto.getIcheckednum().intValue());
          ps.setBigDecimal(7, dto.getNcheckmoney());

          ps.setString(8, dto.getSisbalance());
   ...
   */
	...
	*/
    ...
    {
        TsCheckbalanceDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsCheckbalanceDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsCheckbalancePK pk = (TsCheckbalancePK)_pk ;
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsCheckbalancePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getSgroupid());
        ...
        if (results!=null && results.size() > 0)
        {
            TsCheckbalanceDto[] dtos = new TsCheckbalanceDto[0];
		    dtos = (TsCheckbalanceDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsCheckbalanceDto  dto = new  TsCheckbalanceDto ();
             //S_ORGCODE
             ...
             //I_TOTALNUM
             str = rs.getString("I_TOTALNUM");
             if(str!=null){
                dto.setItotalnum(new Integer(str));
             }

             //N_TOTALMONEY
           dto.setNtotalmoney(rs.getBigDecimal("N_TOTALMONEY"));

             //I_CHECKEDNUM
             str = rs.getString("I_CHECKEDNUM");
             if(str!=null){
                dto.setIcheckednum(new Integer(str));
             }
           ...
           dto.setNcheckmoney(rs.getBigDecimal("N_CHECKMONEY"));
             ...
             //S_ISBALANCE
             str = rs.getString("S_ISBALANCE");
             if (str == null)
                dto.setSisbalance(null);
             else
                dto.setSisbalance(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            if (dto.getItotalnum()==null)
                ...
                ps.setInt(pos, dto.getItotalnum().intValue());
            pos++;

            //N_TOTALMONEY
            ps.setBigDecimal(pos, dto.getNtotalmoney());
            pos++;
            ...
            if (dto.getIcheckednum()==null)
                ...
                ps.setInt(pos, dto.getIcheckednum().intValue());
            pos++;
            ...
            //S_ISBALANCE
            ps.setString(pos, dto.getSisbalance());
            pos++;
    ...
    */
    ...
    {
                  		throw new SQLException("数据库表：TS_CHECKBALANCE没有检查修改的字段");
    }
