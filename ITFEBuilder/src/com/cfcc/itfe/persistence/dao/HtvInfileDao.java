...
/**
 * <p>Title: DAO of table: HTV_INFILE</p>
...
public class HtvInfileDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTV_INFILE ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTV_INFILE.S_ORGCODE, HTV_INFILE.S_COMMITDATE, HTV_INFILE.S_ACCDATE, HTV_INFILE.S_PACKAGENO, HTV_INFILE.S_TAXORGCODE, "
        + "HTV_INFILE.S_TBS_TAXORGCODE, HTV_INFILE.S_FILENAME, HTV_INFILE.S_RECVTRECODE, HTV_INFILE.S_RECVTRENAME, HTV_INFILE.S_DEALNO, "
        + "HTV_INFILE.S_TRASRLNO, HTV_INFILE.N_MONEY, HTV_INFILE.S_TAXTICKETNO, HTV_INFILE.S_GENTICKETDATE, HTV_INFILE.S_BUDGETTYPE, "
        + "HTV_INFILE.S_TRIMFLAG, HTV_INFILE.S_BUDGETSUBCODE, HTV_INFILE.S_BUDGETLEVELCODE, HTV_INFILE.S_ASSITSIGN, HTV_INFILE.S_TBS_ASSITSIGN, "
        + "HTV_INFILE.S_UNITCODE, HTV_INFILE.S_PAYBOOKKIND, HTV_INFILE.S_OPENACCBANKCODE, HTV_INFILE.S_STATUS, HTV_INFILE.S_USERCODE, "
        + "HTV_INFILE.S_DEMO, HTV_INFILE.S_TAXPAYCODE, HTV_INFILE.S_TAXPAYNAME, HTV_INFILE.S_CORPTYPE, HTV_INFILE.S_LIMITDATE, "
        + "HTV_INFILE.S_TAXTYPECODE, HTV_INFILE.S_TAXTYPENAME, HTV_INFILE.S_TAXSUBJECTNAME, HTV_INFILE.S_PAYBNKNO, HTV_INFILE.S_PAYACCT, "
        + "HTV_INFILE.S_TAXSTARTDATE, HTV_INFILE.S_TAXENDDATE, HTV_INFILE.S_TAXNUMBER, HTV_INFILE.N_TAXAMT, HTV_INFILE.N_TAXRATE, "
        + "HTV_INFILE.N_DISCOUNTTAXAMT, HTV_INFILE.N_FACTTAXAMT, HTV_INFILE.S_SOURCEFLAG "
        + "FROM HTV_INFILE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_COMMITDATE = ? AND S_ACCDATE = ? AND S_DEALNO = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM HTV_INFILE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_COMMITDATE = ? AND S_ACCDATE = ? AND S_DEALNO = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTV_INFILE SET "
        + "S_PACKAGENO =?,S_TAXORGCODE =?,S_TBS_TAXORGCODE =?,S_FILENAME =?,S_RECVTRECODE =?, "
        + "S_RECVTRENAME =?,S_TRASRLNO =?,N_MONEY =?,S_TAXTICKETNO =?,S_GENTICKETDATE =?, "
        + "S_BUDGETTYPE =?,S_TRIMFLAG =?,S_BUDGETSUBCODE =?,S_BUDGETLEVELCODE =?,S_ASSITSIGN =?, "
        + "S_TBS_ASSITSIGN =?,S_UNITCODE =?,S_PAYBOOKKIND =?,S_OPENACCBANKCODE =?,S_STATUS =?, "
        + "S_USERCODE =?,S_DEMO =?,S_TAXPAYCODE =?,S_TAXPAYNAME =?,S_CORPTYPE =?, "
        + "S_LIMITDATE =?,S_TAXTYPECODE =?,S_TAXTYPENAME =?,S_TAXSUBJECTNAME =?,S_PAYBNKNO =?, "
        + "S_PAYACCT =?,S_TAXSTARTDATE =?,S_TAXENDDATE =?,S_TAXNUMBER =?,N_TAXAMT =?, "
        + "N_TAXRATE =?,N_DISCOUNTTAXAMT =?,N_FACTTAXAMT =?,S_SOURCEFLAG =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_COMMITDATE = ? AND S_ACCDATE = ? AND S_DEALNO = ?"
        ;
  ...
  {
     HtvInfileDto dto = (HtvInfileDto)_dto ;
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtvInfilePK pk = (HtvInfilePK)_pk ;
       ...
       try {
           ...
           ps.setString(3, pk.getSaccdate());

           ps.setString(4, pk.getSdealno());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtvInfilePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*4+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*4+2, pk.getScommitdate());

                   ps.setString((i-iBegin)*4+3, pk.getSaccdate());

                   ps.setString((i-iBegin)*4+4, pk.getSdealno());
        ...
        if (results!=null && results.size() > 0)
        {
            HtvInfileDto[] dtos = new HtvInfileDto[0];
		    dtos = (HtvInfileDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtvInfileDto  dto = new  HtvInfileDto ();
             //S_ORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                                                                                     		throw new SQLException("数据库表：HTV_INFILE没有检查修改的字段");
    }
