...
/**
 * <p>Title: DAO of table: TS_CONVERTBANKTYPE</p>
...
public class TsConvertbanktypeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CONVERTBANKTYPE ("
          + "S_ORGCODE,S_TRECODE,S_BANKCODE,S_BANKNAME,S_BANKTYPE"
        + ") VALUES ("
        + "?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CONVERTBANKTYPE.S_ORGCODE, TS_CONVERTBANKTYPE.S_TRECODE, TS_CONVERTBANKTYPE.S_BANKCODE, TS_CONVERTBANKTYPE.S_BANKNAME, TS_CONVERTBANKTYPE.S_BANKTYPE "
        + "FROM TS_CONVERTBANKTYPE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_BANKCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CONVERTBANKTYPE "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_BANKCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CONVERTBANKTYPE SET "
        + "S_BANKNAME =?,S_BANKTYPE =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_BANKCODE = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TS_CONVERTBANKTYPE " 
        + " WHERE "
        + "S_ORGCODE = ? AND S_TRECODE = ? AND S_BANKCODE = ?"
        ;
  ...
  {
     TsConvertbanktypeDto dto = (TsConvertbanktypeDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(5, dto.getSbanktype());
   ...
   */
	...
	*/
    ...
    {
        TsConvertbanktypeDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsConvertbanktypeDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsConvertbanktypePK pk = (TsConvertbanktypePK)_pk ;
    ...
    {
        TsConvertbanktypePK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsConvertbanktypePK)_pks[i] ; 
        ...
        if (results!=null && results.size() > 0)
        {
            TsConvertbanktypeDto[] dtos = new TsConvertbanktypeDto[0];
		    dtos = (TsConvertbanktypeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_CONVERTBANKTYPE没有检查修改的字段");
    }
