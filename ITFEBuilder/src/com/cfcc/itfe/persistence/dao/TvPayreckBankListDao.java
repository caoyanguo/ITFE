...
import com.cfcc.itfe.persistence.pk.TvPayreckBankListPK ;


/**
 * <p>Title: DAO of table: TV_PAYRECK_BANK_LIST</p>
...
public class TvPayreckBankListDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TV_PAYRECK_BANK_LIST ("
          + "I_VOUSRLNO,I_SEQNO,S_BDGORGCODE,S_FUNCBDGSBTCODE,S_ECNOMICSUBJECTCODE"
          + ",F_AMT,S_ACCTPROP,TS_UPDATE,S_SUPDEPNAME,S_EXPFUNCNAME"
          + ",S_PAYSUMMARYNAME,S_HOLD1,S_HOLD2,S_HOLD3,S_HOLD4"
          + ",S_VOUCHERN0,S_ID,S_PAYDATE"
        + ") VALUES ("
        + "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TV_PAYRECK_BANK_LIST.I_VOUSRLNO, TV_PAYRECK_BANK_LIST.I_SEQNO, TV_PAYRECK_BANK_LIST.S_BDGORGCODE, TV_PAYRECK_BANK_LIST.S_FUNCBDGSBTCODE, TV_PAYRECK_BANK_LIST.S_ECNOMICSUBJECTCODE, "
        + "TV_PAYRECK_BANK_LIST.F_AMT, TV_PAYRECK_BANK_LIST.S_ACCTPROP, TV_PAYRECK_BANK_LIST.TS_UPDATE, TV_PAYRECK_BANK_LIST.S_SUPDEPNAME, TV_PAYRECK_BANK_LIST.S_EXPFUNCNAME, "
        + "TV_PAYRECK_BANK_LIST.S_PAYSUMMARYNAME, TV_PAYRECK_BANK_LIST.S_HOLD1, TV_PAYRECK_BANK_LIST.S_HOLD2, TV_PAYRECK_BANK_LIST.S_HOLD3, TV_PAYRECK_BANK_LIST.S_HOLD4, "
        + "TV_PAYRECK_BANK_LIST.S_VOUCHERN0, TV_PAYRECK_BANK_LIST.S_ID, TV_PAYRECK_BANK_LIST.S_PAYDATE "
        + "FROM TV_PAYRECK_BANK_LIST "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TV_PAYRECK_BANK_LIST SET "
        + "S_BDGORGCODE =?,S_FUNCBDGSBTCODE =?,S_ECNOMICSUBJECTCODE =?,F_AMT =?,S_ACCTPROP =?, "
        + "TS_UPDATE =?,S_SUPDEPNAME =?,S_EXPFUNCNAME =?,S_PAYSUMMARYNAME =?,S_HOLD1 =?, "
        + "S_HOLD2 =?,S_HOLD3 =?,S_HOLD4 =?,S_VOUCHERN0 =?,S_ID =?, "
        + "S_PAYDATE =? "
        + "WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TV_PAYRECK_BANK_LIST " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     TvPayreckBankListDto dto = (TvPayreckBankListDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(5, dto.getSecnomicsubjectcode());

          ps.setBigDecimal(6, dto.getFamt());

          ps.setString(7, dto.getSacctprop());

          ps.setTimestamp(8, dto.getTsupdate());

          ps.setString(9, dto.getSsupdepname());

          ps.setString(10, dto.getSexpfuncname());

          ps.setString(11, dto.getSpaysummaryname());

          ps.setString(12, dto.getShold1());

          ps.setString(13, dto.getShold2());

          ps.setString(14, dto.getShold3());

          ps.setString(15, dto.getShold4());

          ps.setString(16, dto.getSvouchern0());

          ps.setString(17, dto.getSid());

          ps.setString(18, dto.getSpaydate());
   ...
   */
	...
	*/
    ...
    {
        TvPayreckBankListDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TvPayreckBankListDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TvPayreckBankListPK pk = (TvPayreckBankListPK)_pk ;
    ...
    {
        TvPayreckBankListPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TvPayreckBankListPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TvPayreckBankListPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TvPayreckBankListDto[] dtos = new TvPayreckBankListDto[0];
		    dtos = (TvPayreckBankListDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TvPayreckBankListDto  dto = new  TvPayreckBankListDto ();
             //I_VOUSRLNO
           ...
           dto.setTsupdate(rs.getTimestamp("TS_UPDATE"));
             ...
             //S_VOUCHERN0
             str = rs.getString("S_VOUCHERN0");
             if (str == null)
                dto.setSvouchern0(null);
             else
                dto.setSvouchern0(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_VOUCHERN0
            ps.setString(pos, dto.getSvouchern0());
            pos++;
    ...
    */
    ...
    {
                                                		throw new SQLException("数据库表：TV_PAYRECK_BANK_LIST没有检查修改的字段");
    }
