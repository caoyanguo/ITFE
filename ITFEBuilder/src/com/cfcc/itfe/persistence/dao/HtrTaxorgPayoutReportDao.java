...
import com.cfcc.itfe.persistence.dto.HtrTaxorgPayoutReportDto ;
import com.cfcc.itfe.persistence.pk.HtrTaxorgPayoutReportPK ;


/**
 * <p>Title: DAO of table: HTR_TAXORG_PAYOUT_REPORT</p>
...
public class HtrTaxorgPayoutReportDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTR_TAXORG_PAYOUT_REPORT ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTR_TAXORG_PAYOUT_REPORT.S_TAXORGCODE, HTR_TAXORG_PAYOUT_REPORT.S_TRECODE, HTR_TAXORG_PAYOUT_REPORT.S_RPTDATE, HTR_TAXORG_PAYOUT_REPORT.S_BUDGETTYPE, HTR_TAXORG_PAYOUT_REPORT.S_BUDGETLEVELCODE, "
        + "HTR_TAXORG_PAYOUT_REPORT.S_BUDGETSUBCODE, HTR_TAXORG_PAYOUT_REPORT.S_BUDGETSUBNAME, HTR_TAXORG_PAYOUT_REPORT.S_ECONMICSUBCODE, HTR_TAXORG_PAYOUT_REPORT.S_ECONMICSUBNAME, HTR_TAXORG_PAYOUT_REPORT.N_MONEYDAY, "
        + "HTR_TAXORG_PAYOUT_REPORT.N_MONEYTENDAY, HTR_TAXORG_PAYOUT_REPORT.N_MONEYMONTH, HTR_TAXORG_PAYOUT_REPORT.N_MONEYQUARTER, HTR_TAXORG_PAYOUT_REPORT.N_MONEYYEAR, HTR_TAXORG_PAYOUT_REPORT.S_FINORGCODE "
        + "FROM HTR_TAXORG_PAYOUT_REPORT "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTR_TAXORG_PAYOUT_REPORT SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTR_TAXORG_PAYOUT_REPORT " 
        + " WHERE "
        + "S_TAXORGCODE = ? AND S_TRECODE = ? AND S_RPTDATE = ? AND S_BUDGETTYPE = ? AND S_BUDGETLEVELCODE = ? AND "
        + "S_BUDGETSUBCODE = ? AND S_FINORGCODE = ?"
        ;
  ...
  {
     HtrTaxorgPayoutReportDto dto = (HtrTaxorgPayoutReportDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtrTaxorgPayoutReportDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtrTaxorgPayoutReportDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtrTaxorgPayoutReportPK pk = (HtrTaxorgPayoutReportPK)_pk ;
    ...
    {
        HtrTaxorgPayoutReportPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtrTaxorgPayoutReportPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtrTaxorgPayoutReportPK)(pks.get(i)) ; 
        ...
        if (results!=null && results.size() > 0)
        {
            HtrTaxorgPayoutReportDto[] dtos = new HtrTaxorgPayoutReportDto[0];
		    dtos = (HtrTaxorgPayoutReportDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtrTaxorgPayoutReportDto  dto = new  HtrTaxorgPayoutReportDto ();
             //S_TAXORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                        		throw new SQLException("数据库表：HTR_TAXORG_PAYOUT_REPORT没有检查修改的字段");
    }
