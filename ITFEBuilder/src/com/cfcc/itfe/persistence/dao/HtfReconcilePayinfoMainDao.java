...
import com.cfcc.itfe.persistence.dto.HtfReconcilePayinfoMainDto ;
import com.cfcc.itfe.persistence.pk.HtfReconcilePayinfoMainPK ;


/**
 * <p>Title: DAO of table: HTF_RECONCILE_PAYINFO_MAIN</p>
...
public class HtfReconcilePayinfoMainDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_RECONCILE_PAYINFO_MAIN ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_RECONCILE_PAYINFO_MAIN.I_VOUSRLNO, HTF_RECONCILE_PAYINFO_MAIN.S_ORGCODE, HTF_RECONCILE_PAYINFO_MAIN.S_TRECODE, HTF_RECONCILE_PAYINFO_MAIN.S_STATUS, HTF_RECONCILE_PAYINFO_MAIN.S_DEMO, "
        + "HTF_RECONCILE_PAYINFO_MAIN.TS_SYSUPDATE, HTF_RECONCILE_PAYINFO_MAIN.S_PACKAGENO, HTF_RECONCILE_PAYINFO_MAIN.S_ADMDIVCODE, HTF_RECONCILE_PAYINFO_MAIN.S_STYEAR, HTF_RECONCILE_PAYINFO_MAIN.S_VTCODE, "
        + "HTF_RECONCILE_PAYINFO_MAIN.S_VOUDATE, HTF_RECONCILE_PAYINFO_MAIN.S_VOUCHERNO, HTF_RECONCILE_PAYINFO_MAIN.S_VOUCHERCHECKNO, HTF_RECONCILE_PAYINFO_MAIN.S_CHILDPACKNUM, HTF_RECONCILE_PAYINFO_MAIN.S_CURPACKNO, "
        + "HTF_RECONCILE_PAYINFO_MAIN.S_CLEARBANKCODE, HTF_RECONCILE_PAYINFO_MAIN.S_CLEARBANKNAME, HTF_RECONCILE_PAYINFO_MAIN.S_CLEARACCNO, HTF_RECONCILE_PAYINFO_MAIN.S_CLEARACCNANME, HTF_RECONCILE_PAYINFO_MAIN.S_BEGINDATE, "
        + "HTF_RECONCILE_PAYINFO_MAIN.S_ENDDATE, HTF_RECONCILE_PAYINFO_MAIN.S_ALLNUM, HTF_RECONCILE_PAYINFO_MAIN.N_ALLAMT, HTF_RECONCILE_PAYINFO_MAIN.S_XCHECKRESULT, HTF_RECONCILE_PAYINFO_MAIN.S_XDIFFNUM, "
        + "HTF_RECONCILE_PAYINFO_MAIN.S_HOLD1, HTF_RECONCILE_PAYINFO_MAIN.S_HOLD2, HTF_RECONCILE_PAYINFO_MAIN.S_EXT1, HTF_RECONCILE_PAYINFO_MAIN.S_EXT2, HTF_RECONCILE_PAYINFO_MAIN.S_EXT3, "
        + "HTF_RECONCILE_PAYINFO_MAIN.S_EXT4, HTF_RECONCILE_PAYINFO_MAIN.S_EXT5 "
        + "FROM HTF_RECONCILE_PAYINFO_MAIN "
        +" WHERE " 
        + "I_VOUSRLNO = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_RECONCILE_PAYINFO_MAIN SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_RECONCILE_PAYINFO_MAIN " 
        + " WHERE "
        + "I_VOUSRLNO = ?"
        ;
  ...
  {
     HtfReconcilePayinfoMainDto dto = (HtfReconcilePayinfoMainDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtfReconcilePayinfoMainDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfReconcilePayinfoMainDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfReconcilePayinfoMainPK pk = (HtfReconcilePayinfoMainPK)_pk ;
    ...
    {
        HtfReconcilePayinfoMainPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfReconcilePayinfoMainPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfReconcilePayinfoMainPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfReconcilePayinfoMainDto[] dtos = new HtfReconcilePayinfoMainDto[0];
		    dtos = (HtfReconcilePayinfoMainDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfReconcilePayinfoMainDto  dto = new  HtfReconcilePayinfoMainDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                                                             		throw new SQLException("数据库表：HTF_RECONCILE_PAYINFO_MAIN没有检查修改的字段");
    }
