...
/**
 * <p>Title: DAO of table: TV_GRANTPAYFILESUB</p>
...
public class TvGrantpayfilesubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TV_GRANTPAYFILESUB ("
          + "S_ORGCODE,I_NOBEFOREPACKAGE,S_FUNSUBJECTCODE,S_ECOSUBJECTCODE,N_MONEY"
          + ",S_ACCATTRIB,S_USERCODE,S_DEMO"
        + ") VALUES ("
        + "?,?,?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TV_GRANTPAYFILESUB.S_ORGCODE, TV_GRANTPAYFILESUB.I_NOBEFOREPACKAGE, TV_GRANTPAYFILESUB.S_FUNSUBJECTCODE, TV_GRANTPAYFILESUB.S_ECOSUBJECTCODE, TV_GRANTPAYFILESUB.N_MONEY, "
        + "TV_GRANTPAYFILESUB.S_ACCATTRIB, TV_GRANTPAYFILESUB.S_USERCODE, TV_GRANTPAYFILESUB.S_DEMO "
        + "FROM TV_GRANTPAYFILESUB "
        +" WHERE " 
        + "S_ORGCODE = ? AND I_NOBEFOREPACKAGE = ? AND S_FUNSUBJECTCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TV_GRANTPAYFILESUB "
        +" WHERE " 
        + "S_ORGCODE = ? AND I_NOBEFOREPACKAGE = ? AND S_FUNSUBJECTCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TV_GRANTPAYFILESUB SET "
        + "S_ECOSUBJECTCODE =?,N_MONEY =?,S_ACCATTRIB =?,S_USERCODE =?,S_DEMO =? "
        + "WHERE "
        + "S_ORGCODE = ? AND I_NOBEFOREPACKAGE = ? AND S_FUNSUBJECTCODE = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TV_GRANTPAYFILESUB " 
        + " WHERE "
        + "S_ORGCODE = ? AND I_NOBEFOREPACKAGE = ? AND S_FUNSUBJECTCODE = ?"
        ;
  ...
  {
     TvGrantpayfilesubDto dto = (TvGrantpayfilesubDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(3, dto.getSfunsubjectcode());

          ps.setString(4, dto.getSecosubjectcode());

          ps.setBigDecimal(5, dto.getNmoney());

          ps.setString(6, dto.getSaccattrib());
   ...
   */
	...
	*/
    ...
    {
        TvGrantpayfilesubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TvGrantpayfilesubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TvGrantpayfilesubPK pk = (TvGrantpayfilesubPK)_pk ;
       ...
       try {
           ...
           ps.setString(3, pk.getSfunsubjectcode());
    ...
    {
        TvGrantpayfilesubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TvGrantpayfilesubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TvGrantpayfilesubPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*3+1, pk.getSorgcode());

                   if (pk.getInobeforepackage()==null)
                      ps.setNull((i-iBegin)*3+ 2 , java.sql.Types.INTEGER);
                   else
                     ps.setInt((i-iBegin)*3+2, pk.getInobeforepackage().intValue());

                   ps.setString((i-iBegin)*3+3, pk.getSfunsubjectcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TvGrantpayfilesubDto[] dtos = new TvGrantpayfilesubDto[0];
		    dtos = (TvGrantpayfilesubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TvGrantpayfilesubDto  dto = new  TvGrantpayfilesubDto ();
             //S_ORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
               		throw new SQLException("数据库表：TV_GRANTPAYFILESUB没有检查修改的字段");
    }
