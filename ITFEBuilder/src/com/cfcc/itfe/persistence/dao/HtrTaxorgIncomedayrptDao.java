...
import com.cfcc.itfe.persistence.dto.HtrTaxorgIncomedayrptDto ;
import com.cfcc.itfe.persistence.pk.HtrTaxorgIncomedayrptPK ;


/**
 * <p>Title: DAO of table: HTR_TAXORG_INCOMEDAYRPT</p>
...
public class HtrTaxorgIncomedayrptDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTR_TAXORG_INCOMEDAYRPT ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTR_TAXORG_INCOMEDAYRPT.S_TAXORGCODE, HTR_TAXORG_INCOMEDAYRPT.S_TRECODE, HTR_TAXORG_INCOMEDAYRPT.S_RPTDATE, HTR_TAXORG_INCOMEDAYRPT.S_BUDGETTYPE, HTR_TAXORG_INCOMEDAYRPT.S_BUDGETLEVELCODE, "
        + "HTR_TAXORG_INCOMEDAYRPT.S_BUDGETSUBCODE, HTR_TAXORG_INCOMEDAYRPT.S_BUDGETSUBNAME, HTR_TAXORG_INCOMEDAYRPT.N_MONEYDAY, HTR_TAXORG_INCOMEDAYRPT.N_MONEYTENDAY, HTR_TAXORG_INCOMEDAYRPT.N_MONEYMONTH, "
        + "HTR_TAXORG_INCOMEDAYRPT.N_MONEYQUARTER, HTR_TAXORG_INCOMEDAYRPT.N_MONEYYEAR, HTR_TAXORG_INCOMEDAYRPT.S_BELONGFLAG, HTR_TAXORG_INCOMEDAYRPT.S_TRIMFLAG, HTR_TAXORG_INCOMEDAYRPT.S_DIVIDEGROUP, "
        + "HTR_TAXORG_INCOMEDAYRPT.S_BILLKIND "
        + "FROM HTR_TAXORG_INCOMEDAYRPT "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTR_TAXORG_INCOMEDAYRPT SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTR_TAXORG_INCOMEDAYRPT " 
        + " WHERE "
        + "S_TAXORGCODE = ? AND S_TRECODE = ? AND S_RPTDATE = ? AND S_BUDGETTYPE = ? AND S_BUDGETLEVELCODE = ? AND "
        + "S_BUDGETSUBCODE = ? AND S_DIVIDEGROUP = ? AND S_BILLKIND = ?"
        ;
  ...
  {
     HtrTaxorgIncomedayrptDto dto = (HtrTaxorgIncomedayrptDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtrTaxorgIncomedayrptDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtrTaxorgIncomedayrptDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtrTaxorgIncomedayrptPK pk = (HtrTaxorgIncomedayrptPK)_pk ;
    ...
    {
        HtrTaxorgIncomedayrptPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtrTaxorgIncomedayrptPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtrTaxorgIncomedayrptPK)(pks.get(i)) ; 
        ...
        if (results!=null && results.size() > 0)
        {
            HtrTaxorgIncomedayrptDto[] dtos = new HtrTaxorgIncomedayrptDto[0];
		    dtos = (HtrTaxorgIncomedayrptDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtrTaxorgIncomedayrptDto  dto = new  HtrTaxorgIncomedayrptDto ();
             //S_TAXORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                        		throw new SQLException("数据库表：HTR_TAXORG_INCOMEDAYRPT没有检查修改的字段");
    }
