...
/**
 * <p>Title: DAO of table: TBS_TV_GRANTPAYPLAN_SUB</p>
...
public class TbsTvGrantpayplanSubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TBS_TV_GRANTPAYPLAN_SUB ("
          + "I_VOUSRLNO,I_DETAILSEQNO,S_FUNCSBTCODE,S_ECOSBTCODE,F_ZEROSUMAMT"
          + ",F_SMALLSUMAMT,TS_SYSUPDATE"
        + ") VALUES ("
        + "?,?,?,?,?,?,CURRENT TIMESTAMP )";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TBS_TV_GRANTPAYPLAN_SUB.I_VOUSRLNO, TBS_TV_GRANTPAYPLAN_SUB.I_DETAILSEQNO, TBS_TV_GRANTPAYPLAN_SUB.S_FUNCSBTCODE, TBS_TV_GRANTPAYPLAN_SUB.S_ECOSBTCODE, TBS_TV_GRANTPAYPLAN_SUB.F_ZEROSUMAMT, "
        + "TBS_TV_GRANTPAYPLAN_SUB.F_SMALLSUMAMT, TBS_TV_GRANTPAYPLAN_SUB.TS_SYSUPDATE "
        + "FROM TBS_TV_GRANTPAYPLAN_SUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TBS_TV_GRANTPAYPLAN_SUB SET "
        + "S_FUNCSBTCODE =?,S_ECOSBTCODE =?,F_ZEROSUMAMT =?,F_SMALLSUMAMT =?,TS_SYSUPDATE =CURRENT TIMESTAMP "
        + "WHERE "
        + "I_VOUSRLNO = ? AND I_DETAILSEQNO = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TBS_TV_GRANTPAYPLAN_SUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_DETAILSEQNO = ?"
        ;
  ...
  {
     TbsTvGrantpayplanSubDto dto = (TbsTvGrantpayplanSubDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setBigDecimal(5, dto.getFzerosumamt());

          ps.setBigDecimal(6, dto.getFsmallsumamt());
   ...
   */
	...
	*/
    ...
    {
        TbsTvGrantpayplanSubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TbsTvGrantpayplanSubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TbsTvGrantpayplanSubPK pk = (TbsTvGrantpayplanSubPK)_pk ;
    ...
    {
        TbsTvGrantpayplanSubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TbsTvGrantpayplanSubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TbsTvGrantpayplanSubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TbsTvGrantpayplanSubDto[] dtos = new TbsTvGrantpayplanSubDto[0];
		    dtos = (TbsTvGrantpayplanSubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TbsTvGrantpayplanSubDto  dto = new  TbsTvGrantpayplanSubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
               		throw new SQLException("数据库表：TBS_TV_GRANTPAYPLAN_SUB没有检查修改的字段");
    }
