...
import com.cfcc.itfe.persistence.dto.HtfReconcilePayquotaMainDto ;
import com.cfcc.itfe.persistence.pk.HtfReconcilePayquotaMainPK ;


/**
 * <p>Title: DAO of table: HTF_RECONCILE_PAYQUOTA_MAIN</p>
...
public class HtfReconcilePayquotaMainDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_RECONCILE_PAYQUOTA_MAIN ("
          ...
          + ",S_CLEARBANKCODE,S_CLEARBANKNAME,S_CLEARACCNO,S_CLEARACCNANME,S_BEGINDATE"
          + ",S_ENDDATE,S_ALLNUM,N_ALLAMT,S_XCHECKRESULT,S_XDIFFNUM"
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.I_VOUSRLNO, HTF_RECONCILE_PAYQUOTA_MAIN.S_ORGCODE, HTF_RECONCILE_PAYQUOTA_MAIN.S_TRECODE, HTF_RECONCILE_PAYQUOTA_MAIN.S_STATUS, HTF_RECONCILE_PAYQUOTA_MAIN.S_DEMO, "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.TS_SYSUPDATE, HTF_RECONCILE_PAYQUOTA_MAIN.S_PACKAGENO, HTF_RECONCILE_PAYQUOTA_MAIN.S_ADMDIVCODE, HTF_RECONCILE_PAYQUOTA_MAIN.S_STYEAR, HTF_RECONCILE_PAYQUOTA_MAIN.S_VTCODE, "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.S_VOUDATE, HTF_RECONCILE_PAYQUOTA_MAIN.S_VOUCHERNO, HTF_RECONCILE_PAYQUOTA_MAIN.S_VOUCHERCHECKNO, HTF_RECONCILE_PAYQUOTA_MAIN.S_CHILDPACKNUM, HTF_RECONCILE_PAYQUOTA_MAIN.S_CURPACKNO, "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.S_CLEARBANKCODE, HTF_RECONCILE_PAYQUOTA_MAIN.S_CLEARBANKNAME, HTF_RECONCILE_PAYQUOTA_MAIN.S_CLEARACCNO, HTF_RECONCILE_PAYQUOTA_MAIN.S_CLEARACCNANME, HTF_RECONCILE_PAYQUOTA_MAIN.S_BEGINDATE, "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.S_ENDDATE, HTF_RECONCILE_PAYQUOTA_MAIN.S_ALLNUM, HTF_RECONCILE_PAYQUOTA_MAIN.N_ALLAMT, HTF_RECONCILE_PAYQUOTA_MAIN.S_XCHECKRESULT, HTF_RECONCILE_PAYQUOTA_MAIN.S_XDIFFNUM, "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.S_HOLD1, HTF_RECONCILE_PAYQUOTA_MAIN.S_HOLD2, HTF_RECONCILE_PAYQUOTA_MAIN.S_EXT1, HTF_RECONCILE_PAYQUOTA_MAIN.S_EXT2, HTF_RECONCILE_PAYQUOTA_MAIN.S_EXT3, "
        + "HTF_RECONCILE_PAYQUOTA_MAIN.S_EXT4, HTF_RECONCILE_PAYQUOTA_MAIN.S_EXT5 "
        + "FROM HTF_RECONCILE_PAYQUOTA_MAIN "
        +" WHERE " 
        + "I_VOUSRLNO = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_RECONCILE_PAYQUOTA_MAIN SET "
        + "S_ORGCODE =?,S_TRECODE =?,S_STATUS =?,S_DEMO =?,TS_SYSUPDATE =CURRENT TIMESTAMP, "
        + "S_PACKAGENO =?,S_ADMDIVCODE =?,S_STYEAR =?,S_VTCODE =?,S_VOUDATE =?, "
        + "S_VOUCHERNO =?,S_VOUCHERCHECKNO =?,S_CHILDPACKNUM =?,S_CURPACKNO =?,S_CLEARBANKCODE =?, "
        + "S_CLEARBANKNAME =?,S_CLEARACCNO =?,S_CLEARACCNANME =?,S_BEGINDATE =?,S_ENDDATE =?, "
        + "S_ALLNUM =?,N_ALLAMT =?,S_XCHECKRESULT =?,S_XDIFFNUM =?,S_HOLD1 =?, "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_RECONCILE_PAYQUOTA_MAIN " 
        + " WHERE "
        + "I_VOUSRLNO = ?"
        ;
  ...
  {
     HtfReconcilePayquotaMainDto dto = (HtfReconcilePayquotaMainDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(15, dto.getSclearbankcode());

          ps.setString(16, dto.getSclearbankname());

          ps.setString(17, dto.getSclearaccno());

          ps.setString(18, dto.getSclearaccnanme());

          ps.setString(19, dto.getSbegindate());

          ps.setString(20, dto.getSenddate());

          ps.setString(21, dto.getSallnum());

          ps.setBigDecimal(22, dto.getNallamt());

          ps.setString(23, dto.getSxcheckresult());

          ps.setString(24, dto.getSxdiffnum());

          ps.setString(25, dto.getShold1());

          ps.setString(26, dto.getShold2());

          ps.setString(27, dto.getSext1());

          ps.setString(28, dto.getSext2());

          ps.setString(29, dto.getSext3());

          ps.setString(30, dto.getSext4());

          ps.setString(31, dto.getSext5());
   ...
   */
	...
	*/
    ...
    {
        HtfReconcilePayquotaMainDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfReconcilePayquotaMainDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfReconcilePayquotaMainPK pk = (HtfReconcilePayquotaMainPK)_pk ;
    ...
    {
        HtfReconcilePayquotaMainPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfReconcilePayquotaMainPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfReconcilePayquotaMainPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfReconcilePayquotaMainDto[] dtos = new HtfReconcilePayquotaMainDto[0];
		    dtos = (HtfReconcilePayquotaMainDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfReconcilePayquotaMainDto  dto = new  HtfReconcilePayquotaMainDto ();
             //I_VOUSRLNO
           ...
           dto.setTssysupdate(rs.getTimestamp("TS_SYSUPDATE"));
             ...
             //S_CLEARACCNO
             str = rs.getString("S_CLEARACCNO");
             if (str == null)
                dto.setSclearaccno(null);
             else
                dto.setSclearaccno(str.trim());

             //S_CLEARACCNANME
             str = rs.getString("S_CLEARACCNANME");
             if (str == null)
                dto.setSclearaccnanme(null);
             else
                dto.setSclearaccnanme(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_CLEARACCNO
            ps.setString(pos, dto.getSclearaccno());
            pos++;

            //S_CLEARACCNANME
            ps.setString(pos, dto.getSclearaccnanme());
            pos++;
    ...
    */
    ...
    {
                                                                                             		throw new SQLException("数据库表：HTF_RECONCILE_PAYQUOTA_MAIN没有检查修改的字段");
    }
