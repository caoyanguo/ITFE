...
/**
 * <p>Title: DAO of table: TS_SYSFUNC</p>
...
public class TsSysfuncDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_SYSFUNC ("
          + "S_FUNCCODE,S_FUNCNAME,S_FLAG"
        + ") VALUES ("
        + "?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_SYSFUNC.S_FUNCCODE, TS_SYSFUNC.S_FUNCNAME, TS_SYSFUNC.S_FLAG "
        + "FROM TS_SYSFUNC "
        +" WHERE " 
        + "S_FUNCCODE = ?"
        ;
  ...
  {
     TsSysfuncDto dto = (TsSysfuncDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ps.setString(1, dto.getSfunccode());

          ps.setString(2, dto.getSfuncname());

          ps.setString(3, dto.getSflag());
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsSysfuncPK pk = (TsSysfuncPK)_pk ;

       PreparedStatement ps = null;
       ResultSet rs = null;
       try {
           ps = conn.prepareStatement(SQL_SELECT);
           ps.setString(1, pk.getSfunccode());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsSysfuncPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*1+1, pk.getSfunccode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsSysfuncDto[] dtos = new TsSysfuncDto[0];
		    dtos = (TsSysfuncDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_FUNCNAME
             str = rs.getString("S_FUNCNAME");
             if (str == null)
                dto.setSfuncname(null);
             else
                dto.setSfuncname(str.trim());

             //S_FLAG
             str = rs.getString("S_FLAG");
             if (str == null)
                dto.setSflag(null);
             else
                dto.setSflag(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_FUNCNAME
            ps.setString(pos, dto.getSfuncname());
            pos++;

            //S_FLAG
            ps.setString(pos, dto.getSflag());
            pos++;
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_SYSFUNC没有检查修改的字段");
    }
