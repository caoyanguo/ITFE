...
/**
 * <p>Title: DAO of table: HTF_PAYBANK_REFUNDSUB</p>
...
public class HtfPaybankRefundsubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_PAYBANK_REFUNDSUB ("
          + "I_VOUSRLNO,I_SEQNO,S_ID,S_VOUCHERBILLID,S_PAYACCTNO"
          + ",S_PAYACCTNAME,S_PAYACCTBANKNAME,S_PAYEEACCTNO,S_PAYEEACCTNAME,S_PAYEEACCTBANKNAME"
          + ",S_PAYEEACCTBANKNO,N_PAYAMT,S_REMARK,S_HOLD1,S_HOLD2"
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_PAYBANK_REFUNDSUB.I_VOUSRLNO, HTF_PAYBANK_REFUNDSUB.I_SEQNO, HTF_PAYBANK_REFUNDSUB.S_ID, HTF_PAYBANK_REFUNDSUB.S_VOUCHERBILLID, HTF_PAYBANK_REFUNDSUB.S_PAYACCTNO, "
        + "HTF_PAYBANK_REFUNDSUB.S_PAYACCTNAME, HTF_PAYBANK_REFUNDSUB.S_PAYACCTBANKNAME, HTF_PAYBANK_REFUNDSUB.S_PAYEEACCTNO, HTF_PAYBANK_REFUNDSUB.S_PAYEEACCTNAME, HTF_PAYBANK_REFUNDSUB.S_PAYEEACCTBANKNAME, "
        + "HTF_PAYBANK_REFUNDSUB.S_PAYEEACCTBANKNO, HTF_PAYBANK_REFUNDSUB.N_PAYAMT, HTF_PAYBANK_REFUNDSUB.S_REMARK, HTF_PAYBANK_REFUNDSUB.S_HOLD1, HTF_PAYBANK_REFUNDSUB.S_HOLD2, "
        + "HTF_PAYBANK_REFUNDSUB.S_HOLD3, HTF_PAYBANK_REFUNDSUB.S_HOLD4, HTF_PAYBANK_REFUNDSUB.S_EXT1, HTF_PAYBANK_REFUNDSUB.S_EXT2, HTF_PAYBANK_REFUNDSUB.S_EXT3 "
        + "FROM HTF_PAYBANK_REFUNDSUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_PAYBANK_REFUNDSUB SET "
        + "S_ID =?,S_VOUCHERBILLID =?,S_PAYACCTNO =?,S_PAYACCTNAME =?,S_PAYACCTBANKNAME =?, "
        + "S_PAYEEACCTNO =?,S_PAYEEACCTNAME =?,S_PAYEEACCTBANKNAME =?,S_PAYEEACCTBANKNO =?,N_PAYAMT =?, "
        + "S_REMARK =?,S_HOLD1 =?,S_HOLD2 =?,S_HOLD3 =?,S_HOLD4 =?, "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_PAYBANK_REFUNDSUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     HtfPaybankRefundsubDto dto = (HtfPaybankRefundsubDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(5, dto.getSpayacctno());

          ps.setString(6, dto.getSpayacctname());

          ps.setString(7, dto.getSpayacctbankname());
          ...
          ps.setBigDecimal(12, dto.getNpayamt());

          ps.setString(13, dto.getSremark());
          ...
          ps.setString(18, dto.getSext1());

          ps.setString(19, dto.getSext2());

          ps.setString(20, dto.getSext3());
   ...
   */
	...
	*/
    ...
    {
        HtfPaybankRefundsubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfPaybankRefundsubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfPaybankRefundsubPK pk = (HtfPaybankRefundsubPK)_pk ;
    ...
    {
        HtfPaybankRefundsubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfPaybankRefundsubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfPaybankRefundsubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfPaybankRefundsubDto[] dtos = new HtfPaybankRefundsubDto[0];
		    dtos = (HtfPaybankRefundsubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfPaybankRefundsubDto  dto = new  HtfPaybankRefundsubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                      		throw new SQLException("数据库表：HTF_PAYBANK_REFUNDSUB没有检查修改的字段");
    }
