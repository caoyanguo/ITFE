...
/**
 * <p>Title: DAO of table: TS_INFOCONNORG</p>
...
public class TsInfoconnorgDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_INFOCONNORG ("
          + "S_ORGCODE,S_TRECODE,S_CONNORGCODE,S_CONNORGNAME,S_KEY"
          + ",I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_INFOCONNORG.S_ORGCODE, TS_INFOCONNORG.S_TRECODE, TS_INFOCONNORG.S_CONNORGCODE, TS_INFOCONNORG.S_CONNORGNAME, TS_INFOCONNORG.S_KEY, "
        + "TS_INFOCONNORG.I_MODICOUNT "
        + "FROM TS_INFOCONNORG "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_CONNORGCODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_INFOCONNORG "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_CONNORGCODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_INFOCONNORG SET "
        + "S_TRECODE =?,S_CONNORGNAME =?,S_KEY =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_CONNORGCODE = ?"
        ;
  ...
  {
     TsInfoconnorgDto dto = (TsInfoconnorgDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(3, dto.getSconnorgcode());

          ps.setString(4, dto.getSconnorgname());

          ps.setString(5, dto.getSkey());
   ...
   */
	...
	*/
    ...
    {
        TsInfoconnorgDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsInfoconnorgDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsInfoconnorgPK pk = (TsInfoconnorgPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getSconnorgcode());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsInfoconnorgPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*2+2, pk.getSconnorgcode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsInfoconnorgDto[] dtos = new TsInfoconnorgDto[0];
		    dtos = (TsInfoconnorgDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsInfoconnorgDto  dto = new  TsInfoconnorgDto ();
             //S_ORGCODE
             ...
             //S_CONNORGCODE
             str = rs.getString("S_CONNORGCODE");
             if (str == null)
                dto.setSconnorgcode(null);
             else
                dto.setSconnorgcode(str.trim());

             //S_CONNORGNAME
             str = rs.getString("S_CONNORGNAME");
             if (str == null)
                dto.setSconnorgname(null);
             else
                dto.setSconnorgname(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_CONNORGNAME
            ps.setString(pos, dto.getSconnorgname());
            pos++;
           ...
           pos++;
           //S_CONNORGCODE
           ps.setString(pos, dto.getSconnorgcode());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
            		throw new SQLException("数据库表：TS_INFOCONNORG没有检查修改的字段");
    }
