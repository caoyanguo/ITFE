...
import com.cfcc.itfe.persistence.dto.HtfReconcileRefundSubDto ;
import com.cfcc.itfe.persistence.pk.HtfReconcileRefundSubPK ;


/**
 * <p>Title: DAO of table: HTF_RECONCILE_REFUND_SUB</p>
...
public class HtfReconcileRefundSubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO HTF_RECONCILE_REFUND_SUB ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "HTF_RECONCILE_REFUND_SUB.I_VOUSRLNO, HTF_RECONCILE_REFUND_SUB.I_SEQNO, HTF_RECONCILE_REFUND_SUB.S_ID, HTF_RECONCILE_REFUND_SUB.S_PAYDETAILID, HTF_RECONCILE_REFUND_SUB.S_FUNDTYPECODE, "
        + "HTF_RECONCILE_REFUND_SUB.S_FUNDTYPENAME, HTF_RECONCILE_REFUND_SUB.S_PAYEEACCTNO, HTF_RECONCILE_REFUND_SUB.S_PAYEEACCTNAME, HTF_RECONCILE_REFUND_SUB.S_PAYEEACCTBANKNAME, HTF_RECONCILE_REFUND_SUB.S_PAYACCTNO, "
        + "HTF_RECONCILE_REFUND_SUB.S_PAYACCTNAME, HTF_RECONCILE_REFUND_SUB.S_PAYACCTBANKNAME, HTF_RECONCILE_REFUND_SUB.S_AGENCYCODE, HTF_RECONCILE_REFUND_SUB.S_AGENCYNAME, HTF_RECONCILE_REFUND_SUB.S_INCOMESORTCODE, "
        + "HTF_RECONCILE_REFUND_SUB.S_INCOMESORTNAME, HTF_RECONCILE_REFUND_SUB.N_PAYAMT, HTF_RECONCILE_REFUND_SUB.S_XCHECKRESULT, HTF_RECONCILE_REFUND_SUB.S_XCHECKREASON, HTF_RECONCILE_REFUND_SUB.S_HOLD1, "
        + "HTF_RECONCILE_REFUND_SUB.S_HOLD2, HTF_RECONCILE_REFUND_SUB.S_HOLD3, HTF_RECONCILE_REFUND_SUB.S_HOLD4, HTF_RECONCILE_REFUND_SUB.S_EXT1, HTF_RECONCILE_REFUND_SUB.S_EXT2, "
        + "HTF_RECONCILE_REFUND_SUB.S_EXT3, HTF_RECONCILE_REFUND_SUB.S_EXT4, HTF_RECONCILE_REFUND_SUB.S_EXT5 "
        + "FROM HTF_RECONCILE_REFUND_SUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE HTF_RECONCILE_REFUND_SUB SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM HTF_RECONCILE_REFUND_SUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     HtfReconcileRefundSubDto dto = (HtfReconcileRefundSubDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        HtfReconcileRefundSubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (HtfReconcileRefundSubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       HtfReconcileRefundSubPK pk = (HtfReconcileRefundSubPK)_pk ;
    ...
    {
        HtfReconcileRefundSubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (HtfReconcileRefundSubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (HtfReconcileRefundSubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            HtfReconcileRefundSubDto[] dtos = new HtfReconcileRefundSubDto[0];
		    dtos = (HtfReconcileRefundSubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             HtfReconcileRefundSubDto  dto = new  HtfReconcileRefundSubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                                              		throw new SQLException("数据库表：HTF_RECONCILE_REFUND_SUB没有检查修改的字段");
    }
