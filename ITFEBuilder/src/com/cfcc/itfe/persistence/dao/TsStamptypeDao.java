...
/**
 * <p>Title: DAO of table: TS_STAMPTYPE</p>
...
public class TsStamptypeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_STAMPTYPE ("
          + "S_STAMPTYPECODE,S_STAMPTYPENAME,S_STAMPTYPEID,I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_STAMPTYPE.S_STAMPTYPECODE, TS_STAMPTYPE.S_STAMPTYPENAME, TS_STAMPTYPE.S_STAMPTYPEID, TS_STAMPTYPE.I_MODICOUNT "
        + "FROM TS_STAMPTYPE "
        +" WHERE " 
        + "S_STAMPTYPECODE = ?"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_STAMPTYPE SET "
        + "S_STAMPTYPENAME =?,S_STAMPTYPEID =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_STAMPTYPECODE = ?"
        ;
  ...
  {
     TsStamptypeDto dto = (TsStamptypeDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ps.setString(1, dto.getSstamptypecode());

          ps.setString(2, dto.getSstamptypename());

          ps.setString(3, dto.getSstamptypeid());

          if (dto.getImodicount()==null)
            ps.setNull(4, java.sql.Types.INTEGER);
         else
            ps.setInt(4, dto.getImodicount().intValue());
         ps.executeUpdate();
     }
     finally
     {
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsStamptypePK pk = (TsStamptypePK)_pk ;

       PreparedStatement ps = null;
       ResultSet rs = null;
       try {
           ps = conn.prepareStatement(SQL_SELECT);
           ps.setString(1, pk.getSstamptypecode());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsStamptypePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*1+1, pk.getSstamptypecode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsStamptypeDto[] dtos = new TsStamptypeDto[0];
		    dtos = (TsStamptypeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_STAMPTYPENAME
             str = rs.getString("S_STAMPTYPENAME");
             if (str == null)
                dto.setSstamptypename(null);
             else
                dto.setSstamptypename(str.trim());

             //S_STAMPTYPEID
             str = rs.getString("S_STAMPTYPEID");
             if (str == null)
                dto.setSstamptypeid(null);
             else
                dto.setSstamptypeid(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            int pos = 1;
            //S_STAMPTYPENAME
            ps.setString(pos, dto.getSstamptypename());
            pos++;

            //S_STAMPTYPEID
            ps.setString(pos, dto.getSstamptypeid());
            pos++;
    ...
    */
    ...
    {
         		throw new SQLException("数据库表：TS_STAMPTYPE没有检查修改的字段");
    }
