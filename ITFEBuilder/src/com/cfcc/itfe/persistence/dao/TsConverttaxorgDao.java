...
/**
 * <p>Title: DAO of table: TS_CONVERTTAXORG</p>
...
public class TsConverttaxorgDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_CONVERTTAXORG ("
          + "S_ORGCODE,S_TCBSTAXORGCODE,S_NATIONTAXORGCODE,S_AREATAXORGCODE,S_TBSTAXORGCODE"
          + ",S_TRECODE,I_MODICOUNT"
        + ") VALUES ("
        + "?,?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_CONVERTTAXORG.S_ORGCODE, TS_CONVERTTAXORG.S_TCBSTAXORGCODE, TS_CONVERTTAXORG.S_NATIONTAXORGCODE, TS_CONVERTTAXORG.S_AREATAXORGCODE, TS_CONVERTTAXORG.S_TBSTAXORGCODE, "
        + "TS_CONVERTTAXORG.S_TRECODE, TS_CONVERTTAXORG.I_MODICOUNT "
        + "FROM TS_CONVERTTAXORG "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TBSTAXORGCODE = ? AND S_TRECODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_CONVERTTAXORG "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_TBSTAXORGCODE = ? AND S_TRECODE = ? FOR UPDATE"
        ;
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TS_CONVERTTAXORG SET "
        + "S_TCBSTAXORGCODE =?,S_NATIONTAXORGCODE =?,S_AREATAXORGCODE =?,I_MODICOUNT =? "
        + "WHERE "
        + "S_ORGCODE = ? AND S_TBSTAXORGCODE = ? AND S_TRECODE = ?"
        ;
  ...
  {
     TsConverttaxorgDto dto = (TsConverttaxorgDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getStcbstaxorgcode());

          ps.setString(3, dto.getSnationtaxorgcode());

          ps.setString(4, dto.getSareataxorgcode());

          ps.setString(5, dto.getStbstaxorgcode());
   ...
   */
	...
	*/
    ...
    {
        TsConverttaxorgDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsConverttaxorgDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsConverttaxorgPK pk = (TsConverttaxorgPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getStbstaxorgcode());
    ...
    {
        TsConverttaxorgPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsConverttaxorgPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsConverttaxorgPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*3+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*3+2, pk.getStbstaxorgcode());

                   ps.setString((i-iBegin)*3+3, pk.getStrecode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsConverttaxorgDto[] dtos = new TsConverttaxorgDto[0];
		    dtos = (TsConverttaxorgDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
        ...
        while (rs.next())
         {
             ...
             //S_NATIONTAXORGCODE
             str = rs.getString("S_NATIONTAXORGCODE");
             if (str == null)
                dto.setSnationtaxorgcode(null);
             else
                dto.setSnationtaxorgcode(str.trim());

             //S_AREATAXORGCODE
             str = rs.getString("S_AREATAXORGCODE");
             if (str == null)
                dto.setSareataxorgcode(null);
             else
                dto.setSareataxorgcode(str.trim());

             //S_TBSTAXORGCODE
             str = rs.getString("S_TBSTAXORGCODE");
             if (str == null)
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
            ...
            //S_NATIONTAXORGCODE
            ps.setString(pos, dto.getSnationtaxorgcode());
            pos++;

            //S_AREATAXORGCODE
            ps.setString(pos, dto.getSareataxorgcode());
            pos++;
    ...
    */
    ...
    {
            		throw new SQLException("数据库表：TS_CONVERTTAXORG没有检查修改的字段");
    }
