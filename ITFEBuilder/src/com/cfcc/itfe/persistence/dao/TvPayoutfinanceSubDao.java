...
/**
 * <p>Title: DAO of table: TV_PAYOUTFINANCE_SUB</p>
...
public class TvPayoutfinanceSubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TV_PAYOUTFINANCE_SUB ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TV_PAYOUTFINANCE_SUB.I_VOUSRLNO, TV_PAYOUTFINANCE_SUB.S_SEQNO, TV_PAYOUTFINANCE_SUB.S_TRANO, TV_PAYOUTFINANCE_SUB.S_PAYEEACCT, TV_PAYOUTFINANCE_SUB.S_PAYEENAME, "
        + "TV_PAYOUTFINANCE_SUB.S_PAYEEADDR, TV_PAYOUTFINANCE_SUB.S_PAYEEOPNBNKNO, TV_PAYOUTFINANCE_SUB.N_SUBAMT, TV_PAYOUTFINANCE_SUB.TS_SYSUPDATE "
        + "FROM TV_PAYOUTFINANCE_SUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TV_PAYOUTFINANCE_SUB SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TV_PAYOUTFINANCE_SUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND S_SEQNO = ?"
        ;
  ...
  {
     TvPayoutfinanceSubDto dto = (TvPayoutfinanceSubDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TvPayoutfinanceSubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TvPayoutfinanceSubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TvPayoutfinanceSubPK pk = (TvPayoutfinanceSubPK)_pk ;
    ...
    {
        TvPayoutfinanceSubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TvPayoutfinanceSubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TvPayoutfinanceSubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TvPayoutfinanceSubDto[] dtos = new TvPayoutfinanceSubDto[0];
		    dtos = (TvPayoutfinanceSubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TvPayoutfinanceSubDto  dto = new  TvPayoutfinanceSubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                     		throw new SQLException("数据库表：TV_PAYOUTFINANCE_SUB没有检查修改的字段");
    }
