...
/**
 * <p>Title: DAO of table: TS_ASSITFLAGTRANS</p>
...
public class TsAssitflagtransDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_ASSITFLAGTRANS ("
          + "S_ORGCODE,S_BUDGETSUBJECT,S_BUDGETLEVEL,S_TIPSASSISTSIGN,I_MODICOUNT"
          + ",S_TRECODE"
        + ") VALUES ("
        + "?,?,?,?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_ASSITFLAGTRANS.S_ORGCODE, TS_ASSITFLAGTRANS.S_BUDGETSUBJECT, TS_ASSITFLAGTRANS.S_BUDGETLEVEL, TS_ASSITFLAGTRANS.S_TIPSASSISTSIGN, TS_ASSITFLAGTRANS.I_MODICOUNT, "
        + "TS_ASSITFLAGTRANS.S_TRECODE "
        + "FROM TS_ASSITFLAGTRANS "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_BUDGETSUBJECT = ? AND S_BUDGETLEVEL = ? AND S_TRECODE = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_ASSITFLAGTRANS "
        +" WHERE " 
        + "S_ORGCODE = ? AND S_BUDGETSUBJECT = ? AND S_BUDGETLEVEL = ? AND S_TRECODE = ? FOR UPDATE"
        ;
  ...
  {
     TsAssitflagtransDto dto = (TsAssitflagtransDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ...
          ps.setString(2, dto.getSbudgetsubject());

          ps.setString(3, dto.getSbudgetlevel());

          ps.setString(4, dto.getStipsassistsign());

          if (dto.getImodicount()==null)
            ps.setNull(5, java.sql.Types.INTEGER);
         else
            ps.setInt(5, dto.getImodicount().intValue());
   ...
   */
	...
	*/
    ...
    {
        TsAssitflagtransDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TsAssitflagtransDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsAssitflagtransPK pk = (TsAssitflagtransPK)_pk ;
       ...
       try {
           ...
           ps.setString(2, pk.getSbudgetsubject());

           ps.setString(3, pk.getSbudgetlevel());

           ps.setString(4, pk.getStrecode());
    ...
    {
        TsAssitflagtransPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TsAssitflagtransPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsAssitflagtransPK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*4+1, pk.getSorgcode());

                   ps.setString((i-iBegin)*4+2, pk.getSbudgetsubject());

                   ps.setString((i-iBegin)*4+3, pk.getSbudgetlevel());

                   ps.setString((i-iBegin)*4+4, pk.getStrecode());
        ...
        if (results!=null && results.size() > 0)
        {
            TsAssitflagtransDto[] dtos = new TsAssitflagtransDto[0];
		    dtos = (TsAssitflagtransDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsAssitflagtransDto  dto = new  TsAssitflagtransDto ();
             //S_ORGCODE
             ...
             //S_BUDGETSUBJECT
             str = rs.getString("S_BUDGETSUBJECT");
             if (str == null)
                dto.setSbudgetsubject(null);
             else
                dto.setSbudgetsubject(str.trim());

             //S_BUDGETLEVEL
             str = rs.getString("S_BUDGETLEVEL");
             if (str == null)
                dto.setSbudgetlevel(null);
             else
                dto.setSbudgetlevel(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
           ...
           pos++;
           //S_BUDGETSUBJECT
           ps.setString(pos, dto.getSbudgetsubject());
           pos++;
           //S_BUDGETLEVEL
           ps.setString(pos, dto.getSbudgetlevel());
           pos++;
           //S_TRECODE
    ...
    */
    ...
    {
      		throw new SQLException("数据库表：TS_ASSITFLAGTRANS没有检查修改的字段");
    }
