...
import com.cfcc.itfe.persistence.dto.TrTaxorgStockdayrptDto ;
import com.cfcc.itfe.persistence.pk.TrTaxorgStockdayrptPK ;


/**
 * <p>Title: DAO of table: TR_TAXORG_STOCKDAYRPT</p>
...
public class TrTaxorgStockdayrptDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TR_TAXORG_STOCKDAYRPT ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TR_TAXORG_STOCKDAYRPT.S_TAXORGCODE, TR_TAXORG_STOCKDAYRPT.S_TRECODE, TR_TAXORG_STOCKDAYRPT.S_RPTDATE, TR_TAXORG_STOCKDAYRPT.S_ACCNO, TR_TAXORG_STOCKDAYRPT.S_ACCNAME, "
        + "TR_TAXORG_STOCKDAYRPT.S_ACCDATE, TR_TAXORG_STOCKDAYRPT.N_MONEYYESTERDAY, TR_TAXORG_STOCKDAYRPT.N_MONEYIN, TR_TAXORG_STOCKDAYRPT.N_MONEYOUT, TR_TAXORG_STOCKDAYRPT.N_MONEYTODAY "
        + "FROM TR_TAXORG_STOCKDAYRPT "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TR_TAXORG_STOCKDAYRPT SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TR_TAXORG_STOCKDAYRPT " 
        + " WHERE "
        + "S_TAXORGCODE = ? AND S_TRECODE = ? AND S_RPTDATE = ? AND S_ACCNO = ? AND S_ACCDATE = ?"
        ;
  ...
  {
     TrTaxorgStockdayrptDto dto = (TrTaxorgStockdayrptDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TrTaxorgStockdayrptDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TrTaxorgStockdayrptDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TrTaxorgStockdayrptPK pk = (TrTaxorgStockdayrptPK)_pk ;
    ...
    {
        TrTaxorgStockdayrptPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TrTaxorgStockdayrptPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TrTaxorgStockdayrptPK)(pks.get(i)) ; 
        ...
        if (results!=null && results.size() > 0)
        {
            TrTaxorgStockdayrptDto[] dtos = new TrTaxorgStockdayrptDto[0];
		    dtos = (TrTaxorgStockdayrptDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TrTaxorgStockdayrptDto  dto = new  TrTaxorgStockdayrptDto ();
             //S_TAXORGCODE
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
               		throw new SQLException("数据库表：TR_TAXORG_STOCKDAYRPT没有检查修改的字段");
    }
