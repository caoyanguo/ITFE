...
/**
 * <p>Title: DAO of table: TF_PAYBANK_REFUNDSUB</p>
 * <p>Description:收款银行退款通知2252子表 Data Access Object  </p>
...
public class TfPaybankRefundsubDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TF_PAYBANK_REFUNDSUB ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TF_PAYBANK_REFUNDSUB.I_VOUSRLNO, TF_PAYBANK_REFUNDSUB.I_SEQNO, TF_PAYBANK_REFUNDSUB.S_ID, TF_PAYBANK_REFUNDSUB.S_VOUCHERBILLID, TF_PAYBANK_REFUNDSUB.S_PAYACCTNO, "
        + "TF_PAYBANK_REFUNDSUB.S_PAYACCTNAME, TF_PAYBANK_REFUNDSUB.S_PAYACCTBANKNAME, TF_PAYBANK_REFUNDSUB.S_PAYEEACCTNO, TF_PAYBANK_REFUNDSUB.S_PAYEEACCTNAME, TF_PAYBANK_REFUNDSUB.S_PAYEEACCTBANKNAME, "
        + "TF_PAYBANK_REFUNDSUB.S_PAYEEACCTBANKNO, TF_PAYBANK_REFUNDSUB.N_PAYAMT, TF_PAYBANK_REFUNDSUB.S_REMARK, TF_PAYBANK_REFUNDSUB.S_HOLD1, TF_PAYBANK_REFUNDSUB.S_HOLD2, "
        + "TF_PAYBANK_REFUNDSUB.S_HOLD3, TF_PAYBANK_REFUNDSUB.S_HOLD4, TF_PAYBANK_REFUNDSUB.S_EXT1, TF_PAYBANK_REFUNDSUB.S_EXT2, TF_PAYBANK_REFUNDSUB.S_EXT3 "
        + "FROM TF_PAYBANK_REFUNDSUB "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TF_PAYBANK_REFUNDSUB SET "
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TF_PAYBANK_REFUNDSUB " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     TfPaybankRefundsubDto dto = (TfPaybankRefundsubDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TfPaybankRefundsubDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TfPaybankRefundsubDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TfPaybankRefundsubPK pk = (TfPaybankRefundsubPK)_pk ;
    ...
    {
        TfPaybankRefundsubPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TfPaybankRefundsubPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TfPaybankRefundsubPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TfPaybankRefundsubDto[] dtos = new TfPaybankRefundsubDto[0];
		    dtos = (TfPaybankRefundsubDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TfPaybankRefundsubDto  dto = new  TfPaybankRefundsubDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                                      		throw new SQLException("数据库表：TF_PAYBANK_REFUNDSUB没有检查修改的字段");
    }
