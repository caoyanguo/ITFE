...
import com.cfcc.itfe.persistence.pk.TvPayreckbanklistSxPK ;


/**
 * <p>Title: DAO of table: TV_PAYRECKBANKLIST_SX</p>
...
public class TvPayreckbanklistSxDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TV_PAYRECKBANKLIST_SX ("
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TV_PAYRECKBANKLIST_SX.I_VOUSRLNO, TV_PAYRECKBANKLIST_SX.I_SEQNO, TV_PAYRECKBANKLIST_SX.S_BDGORGCODE, TV_PAYRECKBANKLIST_SX.S_FUNCBDGSBTCODE, TV_PAYRECKBANKLIST_SX.S_ECNOMICSUBJECTCODE, "
        + "TV_PAYRECKBANKLIST_SX.F_AMT, TV_PAYRECKBANKLIST_SX.S_ACCTPROP, TV_PAYRECKBANKLIST_SX.TS_UPDATE, TV_PAYRECKBANKLIST_SX.S_SUPDEPNAME, TV_PAYRECKBANKLIST_SX.S_EXPFUNCNAME, "
        + "TV_PAYRECKBANKLIST_SX.S_PAYSUMMARYNAME, TV_PAYRECKBANKLIST_SX.S_HOLD1, TV_PAYRECKBANKLIST_SX.S_HOLD2, TV_PAYRECKBANKLIST_SX.S_HOLD3, TV_PAYRECKBANKLIST_SX.S_HOLD4, "
        + "TV_PAYRECKBANKLIST_SX.S_VOUCHERN0 "
        + "FROM TV_PAYRECKBANKLIST_SX "
        +" WHERE " 
   ...
   /* SQL to select batch data where condition  */
    ...
    private static final String SQL_UPDATE =
        "UPDATE TV_PAYRECKBANKLIST_SX SET "
        ...
        + "S_HOLD2 =?,S_HOLD3 =?,S_HOLD4 =?,S_VOUCHERN0 =? "
        + "WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
    ...
    private static final String SQL_DELETE =
        "DELETE FROM TV_PAYRECKBANKLIST_SX " 
        + " WHERE "
        + "I_VOUSRLNO = ? AND I_SEQNO = ?"
        ;
  ...
  {
     TvPayreckbanklistSxDto dto = (TvPayreckbanklistSxDto)_dto ;
   ...
   */
	...
	*/
    ...
    {
        TvPayreckbanklistSxDto dto  ;
     	for (int i= 0; i< _dtos.length ; i++)
 	    {
 		    dto  = (TvPayreckbanklistSxDto)_dtos[i] ; 
    	    String msgValid = dto.checkValid() ;
        	if (msgValid != null)
     	    	throw new SQLException("插入错误，"+msgValid) ;
     	}
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TvPayreckbanklistSxPK pk = (TvPayreckbanklistSxPK)_pk ;
    ...
    {
        TvPayreckbanklistSxPK pk ;
        
        List pks = new ArrayList();  
	  
		for (int i= 0; i< _pks.length ; i++)
 		{
 			pk  = (TvPayreckbanklistSxPK)_pks[i] ; 
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TvPayreckbanklistSxPK)(pks.get(i)) ; 
                   if (pk.getIvousrlno()==null)
        ...
        if (results!=null && results.size() > 0)
        {
            TvPayreckbanklistSxDto[] dtos = new TvPayreckbanklistSxDto[0];
		    dtos = (TvPayreckbanklistSxDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TvPayreckbanklistSxDto  dto = new  TvPayreckbanklistSxDto ();
             //I_VOUSRLNO
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    */
    ...
    {
                                          		throw new SQLException("数据库表：TV_PAYRECKBANKLIST_SX没有检查修改的字段");
    }
