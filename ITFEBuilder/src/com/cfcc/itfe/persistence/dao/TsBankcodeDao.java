...
/**
 * <p>Title: DAO of table: TS_BANKCODE</p>
...
public class TsBankcodeDao  implements IDao
{


    /* SQL to insert data */
    private static final String SQL_INSERT =
        "INSERT INTO TS_BANKCODE ("
          + "S_BNKCODE,S_ACCTSTATUS,S_BNKNAME"
        + ") VALUES ("
        + "?,?,?)";
    ...
    private static final String SQL_SELECT =
        "SELECT "
        + "TS_BANKCODE.S_BNKCODE, TS_BANKCODE.S_ACCTSTATUS, TS_BANKCODE.S_BNKNAME "
        + "FROM TS_BANKCODE "
        +" WHERE " 
        + "S_BNKCODE = ? AND S_ACCTSTATUS = ?"
        ;
    ...
    private static final String SQL_SELECT_FOR_UPDATE =
        ...
        + "FROM TS_BANKCODE "
        +" WHERE " 
        + "S_BNKCODE = ? AND S_ACCTSTATUS = ? FOR UPDATE"
        ;
  ...
  {
     TsBankcodeDto dto = (TsBankcodeDto)_dto ;
     ...
     try
     {
         ps = conn.prepareStatement(SQL_INSERT);
          ps.setString(1, dto.getSbnkcode());

          ps.setString(2, dto.getSacctstatus());
   ...
   */
	...
	*/
    ...
    public IDto find(IPK _pk, Connection conn, boolean isLobSupport) throws SQLException
    {
       ...
       TsBankcodePK pk = (TsBankcodePK)_pk ;

       PreparedStatement ps = null;
       ResultSet rs = null;
       try {
           ps = conn.prepareStatement(SQL_SELECT);
           ps.setString(1, pk.getSbnkcode());

           ps.setString(2, pk.getSacctstatus());
    ...
    public IDto[] find(IPK[] _pks, Connection conn, boolean isLobSupport) throws SQLException
    {
		...
		for (int iBegin = 0; iBegin < pks.size(); iBegin += FIND_BATCH_SIZE)   //
		{
            ...
            try {
                ...
                {
                    pk  = (TsBankcodePK)(pks.get(i)) ; 
                   ps.setString((i-iBegin)*2+1, pk.getSbnkcode());

                   ps.setString((i-iBegin)*2+2, pk.getSacctstatus());
        ...
        if (results!=null && results.size() > 0)
        {
            TsBankcodeDto[] dtos = new TsBankcodeDto[0];
		    dtos = (TsBankcodeDto[]) results.toArray(dtos) ;
		    return dtos ;
        }
        else
        {
    ...
    public List getResults(ResultSet rs, int maxSize, boolean isLobSupport) throws SQLException
    {
        ...
        while (rs.next())
         {
             TsBankcodeDto  dto = new  TsBankcodeDto ();
             //S_BNKCODE
             ...
             //S_ACCTSTATUS
             str = rs.getString("S_ACCTSTATUS");
             if (str == null)
                dto.setSacctstatus(null);
             else
                dto.setSacctstatus(str.trim());
  ...
  protected IDto[] getChildrenResults(ResultSet rs ,Class _dto) throws SQLException{
    ...
    public void update(IDto _dto, Connection conn, boolean isLobSupport) throws SQLException
    {
        ...
        try {
           ...
           pos++;
           //S_ACCTSTATUS
           ps.setString(pos, dto.getSacctstatus());
           pos++;
           ps.executeUpdate();
        }
        finally
    ...
    */
    ...
    {
   		throw new SQLException("数据库表：TS_BANKCODE没有检查修改的字段");
    }
