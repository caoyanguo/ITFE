   ...
   public String  checkValid()
  {
  	StringBuffer sb = new StringBuffer() ;
        ...
        if (this.getSorgcode()==null)
              sb.append("数据库表：TV_DIRECTPAYFILEMAIN 字段：S_ORGCODE 不能为空; ");
                
       if (this.getSorgcode()!=null)
          if (this.getSorgcode().getBytes().length > 12)
             sb.append("数据库表：TV_DIRECTPAYFILEMAIN 字段：S_ORGCODE 宽度不能超过 "+12);
                
        if (this.getScommitdate()==null)
              sb.append("数据库表：TV_DIRECTPAYFILEMAIN 字段：S_COMMITDATE 不能为空; ");
                
       if (this.getScommitdate()!=null)
          if (this.getScommitdate().getBytes().length > 8)
             sb.append("数据库表：TV_DIRECTPAYFILEMAIN 字段：S_COMMITDATE 宽度不能超过 "+8);
                
        if (this.getSfilename()==null)
              sb.append("数据库表：TV_DIRECTPAYFILEMAIN 字段：S_FILENAME 不能为空; ");
                
       if (this.getSfilename()!=null)
          if (this.getSfilename().getBytes().length > 100)
             sb.append("数据库表：TV_DIRECTPAYFILEMAIN 字段：S_FILENAME 宽度不能超过 "+100);
