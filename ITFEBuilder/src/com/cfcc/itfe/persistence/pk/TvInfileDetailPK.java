package com.cfcc.itfe.persistence.pk;

import java.sql.Date;
import java.math.BigDecimal;
import java.sql.Time;
import java.sql.Timestamp ;
import com.cfcc.jaf.persistence.jaform.parent.IPK ;

/**
 * <p>Title: PK of table: TV_INFILE_DETAIL</p>
 * <p>Description:  PK Object  </p>
 * <p>Copyright: Copyright (c) 2005</p>
 * <p>Company: CFCC </p>
 * <p>Generation Time: 2014-12-25 15:29:02 </p>
 *  Do NOT change this file by hand.
 *  This java source file is generated by JAF_ORM .
 *
 *  pk.vm version timestamp: 2007-04-06 08:30:00
 * 
 * @author win7
 */

public class TvInfileDetailPK implements IPK
{
    /**
    *   S_FILENAME VARCHAR  , PK   , NOT NULL        */
    protected String sfilename;
    /**
    *   S_ORGCODE VARCHAR  , PK   , NOT NULL        */
    protected String sorgcode;
    /**
    *   I_NUM INTEGER  , PK   , NOT NULL        */
    protected Integer inum;



    /*  Getter S_FILENAME, PK , NOT NULL   */
    public String getSfilename()
    {
        return sfilename;
    }

     /*   Setter S_FILENAME, PK , NOT NULL   */
    public void setSfilename(String _sfilename) {
        this.sfilename = _sfilename;
    }
    /*  Getter S_ORGCODE, PK , NOT NULL   */
    public String getSorgcode()
    {
        return sorgcode;
    }

     /*   Setter S_ORGCODE, PK , NOT NULL   */
    public void setSorgcode(String _sorgcode) {
        this.sorgcode = _sorgcode;
    }
    /*  Getter I_NUM, PK , NOT NULL   */
    public Integer getInum()
    {
        return inum;
    }

     /*   Setter I_NUM, PK , NOT NULL   */
    public void setInum(Integer _inum) {
        this.inum = _inum;
    }


    /* Indicates whether some other object is "equal to" this one. */
    public boolean equals(Object obj)
    {
        if (this == obj)
            return true;

        if (obj == null || !(obj instanceof TvInfileDetailPK))
            return false;

        TvInfileDetailPK bean = (TvInfileDetailPK) obj;

	      //compare field sfilename
      if((this.sfilename==null && bean.sfilename!=null) || (this.sfilename!=null && bean.sfilename==null))
          return false;
        else if(this.sfilename==null && bean.sfilename==null){
        }
        else{
          if(!bean.sfilename.equals(this.sfilename))
               return false;
     }
  	      //compare field sorgcode
      if((this.sorgcode==null && bean.sorgcode!=null) || (this.sorgcode!=null && bean.sorgcode==null))
          return false;
        else if(this.sorgcode==null && bean.sorgcode==null){
        }
        else{
          if(!bean.sorgcode.equals(this.sorgcode))
               return false;
     }
  	      //compare field inum
      if((this.inum==null && bean.inum!=null) || (this.inum!=null && bean.inum==null))
          return false;
        else if(this.inum==null && bean.inum==null){
        }
        else{
          if(!bean.inum.equals(this.inum))
               return false;
     }
          return true;
    }


   /* return hashCode ,if A.equals(B) that A.hashCode()==B.hashCode() */
	public int hashCode()
	{
		int __hash__ = 1;
		

        if(this.sfilename!=null)
          __hash__ = __hash__ * 31+ this.sfilename.hashCode() ;

        if(this.sorgcode!=null)
          __hash__ = __hash__ * 31+ this.sorgcode.hashCode() ;

        if(this.inum!=null)
          __hash__ = __hash__ * 31+ this.inum.hashCode() ;

		return __hash__;
	
	}



     /* Creates and returns a copy of this object. */
    public Object clone()
    {
        TvInfileDetailPK bean = new TvInfileDetailPK();
          if(this.sfilename!=null){
                  bean.sfilename = new String(this.sfilename);
            }
             if(this.sorgcode!=null){
                  bean.sorgcode = new String(this.sorgcode);
            }
             if(this.inum!=null){
                  bean.inum = new Integer(this.inum.toString());
            }
   
        return bean;
    }


   /* Returns a string representation of the object. */
    public String toString()
    {
        String sep = ", ";
        StringBuffer sb = new StringBuffer();
        sb.append("TvInfileDetailPK").append(":");
            sb.append("[sfilename]").append(" = ").append(sfilename).append(sep);
            sb.append("[sorgcode]").append(" = ").append(sorgcode).append(sep);
            sb.append("[inum]").append(" = ").append(inum).append(sep);
            return sb.toString();
    }


     /* Returns value valid checking String , NULL is Valid*/
   public String  checkValid()
  {
  	StringBuffer sb = new StringBuffer() ;

          
        if (this.getSfilename()==null)
              sb.append("数据库表：TV_INFILE_DETAIL 字段：S_FILENAME 不能为空; ");
                
       if (this.getSfilename()!=null)
          if (this.getSfilename().getBytes().length > 100)
             sb.append("数据库表：TV_INFILE_DETAIL 字段：S_FILENAME 宽度不能超过 "+100);
                
        if (this.getSorgcode()==null)
              sb.append("数据库表：TV_INFILE_DETAIL 字段：S_ORGCODE 不能为空; ");
                
       if (this.getSorgcode()!=null)
          if (this.getSorgcode().getBytes().length > 12)
             sb.append("数据库表：TV_INFILE_DETAIL 字段：S_ORGCODE 宽度不能超过 "+12);
                
        if (this.getInum()==null)
              sb.append("数据库表：TV_INFILE_DETAIL 字段：I_NUM 不能为空; ");
                	String msg = sb.toString() ;
	if (msg.length() == 0)
		return null ;
  	return  msg;
   }

}
