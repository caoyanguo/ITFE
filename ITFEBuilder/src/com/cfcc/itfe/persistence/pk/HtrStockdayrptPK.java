...
/**
 * <p>Title: PK of table: HTR_STOCKDAYRPT</p>
...
public class HtrStockdayrptPK implements IPK
{
    /**
    ...
    *   S_ACCDATE CHARACTER  , PK   , NOT NULL        */
    ...
    public boolean equals(Object obj)
    {
        ...
        if (obj == null || !(obj instanceof HtrStockdayrptPK))
            return false;

        HtrStockdayrptPK bean = (HtrStockdayrptPK) obj;
   ...
   /* return hashCode ,if A.equals(B) that A.hashCode()==B.hashCode() */
    ...
    public Object clone()
    {
        HtrStockdayrptPK bean = new HtrStockdayrptPK();
          if(this.sorgcode!=null){
   ...
   /* Returns a string representation of the object. */
    public String toString()
    {
        ...
        sb.append("HtrStockdayrptPK").append(":");
   ...
   public String  checkValid()
  {
  	StringBuffer sb = new StringBuffer() ;
        ...
        if (this.getSorgcode()==null)
              sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_ORGCODE 不能为空; ");
                
       if (this.getSorgcode()!=null)
          if (this.getSorgcode().getBytes().length > 12)
             sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_ORGCODE 宽度不能超过 "+12);
                
        if (this.getStrecode()==null)
              sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_TRECODE 不能为空; ");
                
       if (this.getStrecode()!=null)
          if (this.getStrecode().getBytes().length > 10)
             sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_TRECODE 宽度不能超过 "+10);
                
        if (this.getSrptdate()==null)
              sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_RPTDATE 不能为空; ");
                
       if (this.getSrptdate()!=null)
          if (this.getSrptdate().getBytes().length > 8)
             sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_RPTDATE 宽度不能超过 "+8);
                
        if (this.getSaccno()==null)
              sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_ACCNO 不能为空; ");
                
       if (this.getSaccno()!=null)
          if (this.getSaccno().getBytes().length > 32)
             sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_ACCNO 宽度不能超过 "+32);
                
        if (this.getSaccdate()==null)
              sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_ACCDATE 不能为空; ");
                
       if (this.getSaccdate()!=null)
          if (this.getSaccdate().getBytes().length > 8)
             sb.append("数据库表：HTR_STOCKDAYRPT 字段：S_ACCDATE 宽度不能超过 "+8);
