   ...
   public String  checkValid()
  {
  	StringBuffer sb = new StringBuffer() ;
        ...
        if (this.getSorgcode()==null)
              sb.append("数据库表：TS_DWBK_REASON 字段：S_ORGCODE 不能为空; ");
                
       if (this.getSorgcode()!=null)
          if (this.getSorgcode().getBytes().length > 12)
             sb.append("数据库表：TS_DWBK_REASON 字段：S_ORGCODE 宽度不能超过 "+12);
                
        if (this.getSdrawbackreacode()==null)
              sb.append("数据库表：TS_DWBK_REASON 字段：S_DRAWBACKREACODE 不能为空; ");
                
       if (this.getSdrawbackreacode()!=null)
          if (this.getSdrawbackreacode().getBytes().length > 12)
             sb.append("数据库表：TS_DWBK_REASON 字段：S_DRAWBACKREACODE 宽度不能超过 "+12);
