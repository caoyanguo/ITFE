...
public class VoucherDto2MapFor3508 implements IVoucherDto2Map{

	private static Log logger = LogFactory.getLog(VoucherDto2MapFor3508.class);
	/**
	 * 凭证生成
	 * @param vDto
	 * @return
		...
		try{
			...
			for(IDto idto:detailList)
			{
				...
				if(mdto.getSubDtoList()!=null&&mdto.getSubDtoList().size()>0&&mdto.getMainDto() instanceof TvPayoutmsgmainDto)
				{
					...
					//暂时广东特殊判断
					if (pdto.getStrecode().startsWith("19")) {
						// 根据实拨资金表中的信息获取3208的凭证编号
						String bizno = pdto.getSbizno();
						//根据bizno获取凭证索引表中对应的实拨资金的索引信息
						TvVoucherinfoDto indexDto = new TvVoucherinfoDto();
						indexDto.setSext4(bizno);
						indexDto.setSvtcode(MsgConstant.VOUCHER_NO_3208);
						List <TvVoucherinfoDto> index3128List = CommonFacade.getODB().findRsByDto(
									indexDto);
						logger.debug(index3128List);
						if (index3128List.size() > 0) {
							voucherno = index3128List.get(0).getSvoucherno();
							logger.debug(voucherno);
						} else {
							logger.info("查找3208无信息");
						}
					}
					...
					for(int i=0;i<mdto.getSubDtoList().size();i++)
					{
						...
						//暂时广东做特殊判断如果以后全国通用可以放开
						if (pdto.getStrecode().startsWith("19") ) { 
							if ("1".equals(pdto.getSbackflag())){
						       Detailmap.put("PayDetailId",getString(voucherno));
							}else{
						...
						if("1".equals(pdto.getSbackflag())){
						    ...
						    Detailmap.put("PayAcctNo","".equals(getString(subdto.getSpayeeacctno()))?getString(pdto.getSrecacct()):getString(subdto.getSpayeeacctno()));//付款人账号
						    Detailmap.put("PayAcctName","".equals(getString(subdto.getSpayeeacctname()))?getString(pdto.getSrecname()):getString(subdto.getSpayeeacctname()));//付款人名称
						    Detailmap.put("PayAcctBankName","".equals(getString(subdto.getSpayeeacctbankname()))?getString(pdto.getSrecbankname()):getString(subdto.getSpayeeacctbankname()));//款人银行
						    Detailmap.put("PayeeAcctNo",getString(pdto.getSpayeracct()));//收款账户账号
						    Detailmap.put("PayeeAcctName",getString(pdto.getSpayername()));//收款账户名称
						    Detailmap.put("PayeeAcctBankName",getString(pdto.getSpayerbankname()));//收款账户银行
					    }else{
						...
						if("1".equals(pdto.getSbackflag()))
						{
							TvVoucherinfoDto searchvdto = new TvVoucherinfoDto();
							searchvdto.setSext4(pdto.getSbizno());
							searchvdto.setSorgcode(pdto.getSorgcode());
							searchvdto.setStrecode(pdto.getStrecode());
							searchvdto.setSstatus(DealCodeConstants.VOUCHER_SUCCESS);
							List searchlist = CommonFacade.getODB().findRsByDto(searchvdto);
							if(searchlist!=null&&searchlist.size()>0)
							{
								searchvdto = (TvVoucherinfoDto) searchlist.get(0);
								Detailmap.put("PayAmt","-"+String.valueOf(searchvdto.getNmoney()));//拨款金额
								allamt = allamt.subtract(searchvdto.getNmoney());
							}
				...
				}else if(mdto.getMainDto() instanceof TvVoucherinfoAllocateIncomeDto)
				{
					...
					Detailmap.put("PayDetailId",getString(tadto.getSdealno()));//拨款明细Id
...
//					Detailmap.put("PayAmt","-"+String.valueOf(tadto.getNmoney()));//拨款金额
				...
				}else if(mdto.getSubDtoList()!=null&&mdto.getSubDtoList().size()>0&&mdto.getMainDto() instanceof TvPayoutbackmsgMainDto)
				{
					...
					for(int i=0;i<mdto.getSubDtoList().size();i++)
					{
						...
						HashMap<String, Object> Detailmap = new HashMap<String, Object>();
						Detailmap.put("Id",vDto.getSvoucherno()+id);//序号
						if (badto.getStrecode().startsWith("19")) { 
							Detailmap.put("PayDetailId",getString(badto.getSvouno()));
						}else{
							Detailmap.put("PayDetailId",subdto.getSseqno());//拨款明细Id
						}
						Detailmap.put("VoucherNo",getString(badto.getSorivouno()));//预留字段1
						Detailmap.put("BgtTypeCode",getString(subdto.getSbudgetprjcode()));//预算类型编码
						Detailmap.put("BgtTypeName",getString(""));//预算类型名称
						Detailmap.put("FundTypeCode",getString(badto.getShold3()==null ? "9" : badto.getShold3()));//资金性质编码
						Detailmap.put("FundTypeName",getString(badto.getShold4()==null ? "其它资金" :badto.getShold4()));//资金性质名称
						Detailmap.put("PayTypeCode",getString("91"));//支付方式编码
						Detailmap.put("PayTypeName",getString("实拨退款"));//支付方式名称
						...
						Detailmap.put("PayAcctBankName",getString(badto.getShold1()==null ? "" : badto.getShold1()));//付款账户银行
	...
	private List<IDto> getDataList(TvVoucherinfoDto vDto,SQLExecutor execDetail) throws ITFEBizException
	{
		...
		try {
			...
			if(detailList!=null&&detailList.size()>0)
			{
				...
				if(vDto.getSext3()!=null&&("1".equals(vDto.getSext3())||"2".equals(vDto.getSext3())))
					execDetail.addParam(vDto.getSext3());
				TsTreasuryDto tredto = SrvCacheFacade.cacheTreasuryInfo(null).get(vDto.getStrecode());
				if (StateConstant.COMMON_YES.equals(String.valueOf(tredto==null?"":tredto.getSpayunitname()))) {
					subsql = subsql + " and S_STATUS=?";
					execDetail.addParam(DealCodeConstants.DEALCODE_ITFE_SUCCESS);
				}
				execDetail.setMaxRows(500000);
