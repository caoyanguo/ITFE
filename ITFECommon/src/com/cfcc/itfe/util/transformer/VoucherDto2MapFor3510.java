...
public class VoucherDto2MapFor3510 implements IVoucherDto2Map{

	private static Log logger = LogFactory.getLog(VoucherDto2MapFor3510.class);
		...
		if(dataList!=null&&dataList.size()>0)
		{
			...
			if(sendList!=null&&sendList.size()>0)
			{
				...
				for(int i=0;i<sendList.size();i++)
				{
					...
					((Map)map.get("Voucher")).put("PreDateMoney", MtoCodeTrans.transformString(preDateMoney));
					((Map)map.get("Voucher")).put("ClearAmt", MtoCodeTrans.transformString(clearAmt));
					((Map)map.get("Voucher")).put("CurReckMoney", MtoCodeTrans.transformString(curReckMoney));
					((Map)map.get("Voucher")).put("CurDateMoney", MtoCodeTrans.transformString(curDateMoney));
...
//			TsConvertfinorgDto cDto = (TsConvertfinorgDto)mapList.get(1);
			...
			}else
			{
				...
				if(MsgConstant.grantPay.equals(vDto.getSext1())||"12".equals(vDto.getSext1()))
				{
					...
					vouchermap.put("PayTypeName","授权支付");//支付方式名称
				}else if(MsgConstant.directPay.equals(vDto.getSext1())||"11".equals(vDto.getSext1()))
				{
			...
			vouchermap.put("Hold3","");//预留字段1
			vouchermap.put("Hold4","");//预留字段2
			...
			for(Map<String,String> dataMap:detailList)
			{
				...
				Detailmap.put("Id",dataMap.get("Id")==null ? id :dataMap.get("Id"));//序号
...
//				Detailmap.put("PayTypeCode",dataMap.get("PayTypeCode"));//支付方式编码
				...
				else
				{
					...
					if(MsgConstant.grantPay.equals(dataMap.get("PayTypeCode"))||"12".equals(dataMap.get("PayTypeCode")))
						Detailmap.put("PayTypeName","授权支付");//支付方式名称
					else if(MsgConstant.directPay.equals(dataMap.get("PayTypeCode"))||"11".equals(dataMap.get("PayTypeCode")))
						Detailmap.put("PayTypeName","直接支付");//支付方式名称
					else
	...
	private TfReconcilePayquotaSubDto getSubDto(HashMap<String, Object> subMap,HashMap<String, Object> mainMap)
	{
		...
		if(subDto.getSid()!=null&&subDto.getSid().length()>38)
			subDto.setSid(subDto.getSid().substring(subDto.getSid().length()-38));
	...
	private List<IDto> getDataList(TvVoucherinfoDto vDto,SQLExecutor execDetail) throws ITFEBizException
	{
		...
		try {
			...
			{
				sql = new StringBuffer("SELECT max(I_ENROLSRLNO) AS I_ENROLSRLNO,'"+CommonUtil.strToDate(vDto.getShold3())+"' AS D_ENDDATE,'"+CommonUtil.strToDate(vDto.getShold4())+"' AS D_STARTDATE,");
				sql.append("S_BOOKORGCODE,S_TRECODE,S_BDGORGCODE,max(S_BDGORGNAME) as S_BDGORGNAME,S_BNKNO,S_FUNCSBTCODE,S_ECOSBTCODE,C_AMTKIND,"+((vDto.getShold3().contains("0101")&&vDto.getShold4().contains("1231"))?"0 AS F_LASTMONTHZEROAMT,":"sum(F_LASTMONTHZEROAMT) as F_LASTMONTHZEROAMT,"));
				sql.append(((vDto.getShold3().contains("0101")&&vDto.getShold4().contains("1231"))?"(sum(F_CURSMALLAMT)-sum(F_CURRECKZEROAMT))":"sum(F_CURZEROAMT)")+" AS F_CURZEROAMT,sum(F_CURRECKZEROAMT) AS F_CURRECKZEROAMT,sum(F_LASTMONTHSMALLAMT) AS F_LASTMONTHSMALLAMT,");
				sql.append("sum(F_CURSMALLAMT) AS F_CURSMALLAMT,sum(F_CURRECKSMALLAMT) AS F_CURRECKSMALLAMT,max(TS_SYSUPDATE) AS TS_SYSUPDATE FROM TN_CONPAYCHECKBILL");
				sql.append(" WHERE S_BOOKORGCODE=? ");
				sql.append(vDto.getSpaybankcode()==null?"":" AND S_BNKNO=? ");
				sql.append((vDto.getSext1()!=null&&("1".equals(vDto.getSext1())||"0".equals(vDto.getSext1())))?" AND C_AMTKIND=? ":"");
				sql.append(vDto.getSext3()==null?"":" AND S_ECOSBTCODE=? ");
				sql.append(vDto.getStrecode()==null?"":" and S_TRECODE=? ");
				sql.append("AND D_STARTDATE >= ? AND D_STARTDATE <= ? ");
				sql.append(" GROUP BY S_BOOKORGCODE,S_TRECODE,S_BDGORGCODE,S_BNKNO,S_FUNCSBTCODE,S_ECOSBTCODE,C_AMTKIND");
				...
				if(vDto.getSext1()!=null&&("0".equals(vDto.getSext1())||"1".equals(vDto.getSext1())))
				{
...
//						}
				...
				}else if(dto.getMainDtoList()!=null&&dto.getMainDtoList().size()>0)
				{
					...
					for(IDto bdto:templist)
					{
						...
						dataMap = new HashMap<String,String>();
						dataMap.put("Id",TimeFacade.getCurrentStringTime()+String.valueOf(Math.abs(tempdto.getIenrolsrlno())));//序号
