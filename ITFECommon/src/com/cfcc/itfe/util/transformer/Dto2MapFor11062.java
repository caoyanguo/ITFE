...
public class Dto2MapFor11062 {
	...
	public static Map tranfor(List<TvFreeDto> list, String orgcode,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// 电子凭证号码
			bill1106Map.put("ElectroTaxVouNo", maindto.getSfreevouno());
			// 免抵调增预算种类
			...
			// 免抵调减交易金额
			bill1106Map.put("FreeMiAmt", MtoCodeTrans.transformString(maindto.getFfreemiamt()));
			// 整理期标志
			bill1106Map.put("TrimSign", maindto.getCtrimsign());
			// 企业代码
			bill1106Map.put("CorpCode", maindto.getScorpcode());
			// 渠道代码
			bill1106Map.put("ChannelCode", maindto.getSchannelcode());
