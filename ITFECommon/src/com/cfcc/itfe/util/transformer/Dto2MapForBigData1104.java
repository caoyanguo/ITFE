 ...
 * 
 */
public class Dto2MapForBigData1104 {

	private static Log logger = LogFactory.getLog(Dto2MapForBigData1104.class);
	
	/**
	 * DTO转化XML报文
	 * 
	 * @param List<TipsTbDrawbackDto> list 发送报文对象集合
	 * @param String orgcode 机构代码
	 * @param String filename 文件名称
	 * @param String  packno 包流水号
	...
	public static Map tranfor(List<TipsTbDrawbackDto> list, String orgcode, String filename, String packno,boolean isRepeat) throws ITFEBizException {
		...
		if(ITFECommonConstant.SRC_NODE.equals("201053100013")){
			...
			dto.setStrecode(list.get(0).getSdwbktrecode());
		...
		try {
			...
			head1104Map.put("PackNo", SequenceGenerator
					.changePackNoForLocal(SequenceGenerator.getNextByDb2(
							...
							MsgConstant.SEQUENCE_MAX_DEF_VALUE))); // 包流水号
		...
		for (int i = 0; i < list.size(); i++) {
			...
			TipsTbDrawbackDto maindto = list.get(i);

			allamt = allamt.add(maindto.getFtraamt());

			bill1104Map.put("TraNo", maindto.getStrasrlno()); // 交易流水号
			bill1104Map.put("DrawBackTreCode", maindto.getSdwbktrecode()); // 退库国库代码
			bill1104Map.put("PayeeBankNo", maindto.getSpayeebnkno()); // 收款行行号
			...
			bill1104Map.put("TaxPayCode", maindto.getStaxercode()); // 纳税人编码
			bill1104Map.put("TaxPayName", maindto.getStaxername()); // 纳税人名称
			bill1104Map.put("DrawBackVouNo", maindto.getSdwbkvoucode()); // 退库凭证编号
			bill1104Map.put("ElectroTaxVouNo", maindto.getSelcvouno()); // 电子凭证号码
			//bill1104Map.put("OriTaxVouNo", ""); // 原税票号码(tips新版本删除原税票号码)
			bill1104Map.put("TraAmt", MtoCodeTrans.transformString(maindto.getFtraamt())); //交易金额
			bill1104Map.put("BillDate", maindto.getDbill().toString().replaceAll("-", "")); // 开票日期
			bill1104Map.put("CorpCode", maindto.getScorpcode()); // 企业代码
			bill1104Map.put("BudgetType", maindto.getSbdgkind()); // 预算种类
			bill1104Map.put("BudgetSubjectCode", maindto.getSbdgsbtcode()); // 预算科目代码
			bill1104Map.put("BudgetLevelCode", maindto.getSbdglevel()); // 预算级次代码
			bill1104Map.put("TrimSign", maindto.getStrimflag()); // 整理期标志
