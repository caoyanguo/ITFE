...
public class VoucherDto2MapFor3404 implements IVoucherDto2Map{

	private static Log logger = LogFactory.getLog(VoucherDto2MapFor3404.class);
...
//		sDto.setStrecode(vDto.getStrecode());
		...
		try {
			...
			sList= CommonFacade.getODB().findRsByDtoForWhere(sDto," order by S_TRECODE ");		
	...
	private Map tranfor(List<TrStockdayrptDto> dtoList,TvVoucherinfoDto vDto) throws ITFEBizException{
		...
		vouchermap.put("StYear", vDto.getScheckdate().subSequence(0, 4));//业务年度		
		...
		{
//			if(sdto.getSaccno().indexOf(vDto.getSorgcode()+"371")<0)//核算主体代码加371开头的账户代码不生成
			if(((!sdto.getSaccno().startsWith("371"))&&sdto.getSaccno().length()<10)||(!sdto.getSaccno().substring(12,15).equals("371")))//核算主体代码加371开头的账户代码不生成
			{
				detailMap = new HashMap();
				detailMap.put("Id", vDto.getSdealno()+(id++));//id
				detailMap.put("BankCode", sdto.getStrecode());//国库代码
				try {
					detailMap.put("TreName", SrvCacheFacade.cacheTreasuryInfo(vDto.getSorgcode()).get(sdto.getStrecode())==null?"":SrvCacheFacade.cacheTreasuryInfo(vDto.getSorgcode()).get(sdto.getStrecode()).getStrename());
