...
public class VoucherDto2MapFor3507 implements IVoucherDto2Map{
				...
				if(mdto.getMainDto() instanceof TvPayreckBankDto)
				{
					...
					if(mdto!=null&&mdto.getSubDtoList()!=null&&mdto.getSubDtoList().size()>0)
					{
						for(int i=0;i<mdto.getSubDtoList().size();i++)
						{
							...
							Detailmap.put("FundTypeCode",getString(dsdto.getSfundtypecode()));//资金性质编码
							Detailmap.put("FundTypeName",getString(dsdto.getSfundtypename()));//资金性质名称
...
//								Detailmap.put("PayAmt","-"+String.valueOf(gsdto.getNpayamt()));//支付金额
	...
	private List<IDto> getDataList(TvVoucherinfoDto vDto,SQLExecutor execDetail) throws ITFEBizException
	{
		...
		//支持代理库对账
		String payTypecode = vDto.getSext1();
		vDto.setSext1("");
		try {
			...
			sql.append((vDto.getSext1()==null||"".equals(vDto.getSext1()))?"":" AND S_EXT1=? ");
			if(vDto.getSext3()!=null&&("1".equals(vDto.getSext3())||"2".equals(vDto.getSext3())))
			...
			if(vDto.getSext1()!=null&&!"".equals(vDto.getSext1()))
			...
			//增加支付方式
			if (null!=payTypecode) {
				execDetail.addParam(payTypecode);
			}
