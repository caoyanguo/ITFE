...
public class VoucherDto2MapFor3210 implements IVoucherDto2Map {

	private static Log logger = LogFactory.getLog(VoucherDto2MapFor3210.class);	
...
//			}
	...
	public Map tranforDwbkBack(List lists) throws ITFEBizException {
		try {
			...
			vouchermap.put("ReturnReasonName", returnValue(dto.getSdemo()));// 退款原因
	...
	public Map tranforDwbkBackForNB(List lists) throws ITFEBizException {
		try {
			...
			vouchermap.put("VouDate", vDto.getScreatdate());// 凭证日期
			vouchermap.put("VoucherNo", vDto.getSvoucherno());// 凭证号
			vouchermap.put("AgentBusinessNo", returnValue(""));//原银行交易流水号
			vouchermap.put("OriBillNo", returnValue(""));// 原退付单单号
			vouchermap.put("OriVouDate", returnValue(""));// 原退付单凭证日期
			vouchermap.put("OriPayDate", returnValue(""));// 原退付日期
			vouchermap.put("TreCode", returnValue(dto.getStrecode()));//国库代码
	...
	public Map tranfor(List lists) throws ITFEBizException {
		try {
			...
			vouchermap.put("OriVouDate", returnValue(String.valueOf(dto.getDaccept()).replace("-", "")));// 原拨款单凭证日期
			vouchermap.put("OriPayDate", returnValue(String.valueOf(dto.getDacct()).replace("-", "")));// 原支付日期
			...
			vouchermap.put("BudgetLevelCode", returnValue(dto.getCbdglevel()==null?"000":dto.getCbdglevel()));
			vouchermap.put("BudgetLevelName", returnValue("未知"));
			...
			vouchermap.put("PayAcctBankName", returnValue(dto.getSrecbankname()));// 付款人银行dto.getSpayacctbankname()
			...
			Detailmap.put("VoucherDetailNo", returnValue(dto.getSbizno()));
			Detailmap.put("PayeeAcctNo", returnValue(dto.getSpayacctno()));// 收款人账号dto.getSpayacctno()
			Detailmap.put("PayeeAcctName", returnValue(dto.getSpayacctname()));// 收款人名称dto.getSpayacctname()
			Detailmap.put("PayeeAcctBankName", returnValue(dto.getSpayacctbankname()));// 收款人银行dto.getSpayacctbankname()
			Detailmap.put("PayeeAcctBankNo", returnValue(payList!=null&&payList.size()>0?payList.get(0).getSbankno():returnValue(dto.getSpayeeopnbnkno())));// 收款人银行dto.getSpayacctbankname()
