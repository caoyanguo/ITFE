...
public class VoucherDto2MapFor5507 implements IVoucherDto2Map{

	private static Log logger = LogFactory.getLog(VoucherDto2MapFor5507.class);
	...
	*/
	...
	private List getVoucher(TvVoucherinfoDto vDto) throws ITFEBizException
	{
		...
		try {
			...
			if(getMap!=null&&getMap.size()>0&&datadto!=null&&datadto.getMainDto()!=null)
			{
				...
				dto.setSvtcode(MsgConstant.VOUCHER_NO_5507);
	...
	private Map tranfor(MainAndSubDto datadto,Map<String,Object> getMap,TvVoucherinfoDto dto) throws ITFEBizException {
		try{
			...
			vouchermap.put("VtCode",MsgConstant.VOUCHER_NO_5507);//凭证类型编号
	...
	private List<IDto> getDataList(MainAndSubDto dto3507) throws ITFEBizException
	{
		...
		try {
			...
			sql.append("SELECT S_DEALNO FROM HTV_VOUCHERINFO WHERE S_VTCODE= '"+MsgConstant.VOUCHER_NO_2301+"' AND S_ORGCODE= '"+vDto.getSorgcode()+"' ");
			...
			sql.append("SELECT S_DEALNO FROM HTV_VOUCHERINFO WHERE S_VTCODE= '"+MsgConstant.VOUCHER_NO_2302+"' AND S_ORGCODE= '"+vDto.getSorgcode()+"' ");
			sql.append("AND S_TRECODE= '"+vDto.getStrecode()+"'  AND S_STATUS='"+DealCodeConstants.VOUCHER_SUCCESS+"' ");
			sql.append("AND S_CREATDATE BETWEEN '"+vDto.getSbegindate()+"' AND '"+vDto.getSenddate()+"') and S_PAYERACCT='"+vDto.getSclearaccno()+"'");
			...
			if(ITFECommonConstant.PUBLICPARAM.indexOf(",sh,")>=0)
			{
				...
				sql.append("AND S_TRECODE= '"+vDto.getStrecode()+"'  AND S_STATUS='"+DealCodeConstants.VOUCHER_SUCCESS+"' ");
				sql.append("AND S_CREATDATE BETWEEN '"+vDto.getSbegindate()+"' AND '"+vDto.getSenddate()+"') and S_CLEARBANKCODE='"+vDto.getSclearaccno()+"'");
				...
				sql.append("AND S_TRECODE= '"+vDto.getStrecode()+"'  AND S_STATUS='"+DealCodeConstants.VOUCHER_SUCCESS+"' ");
				sql.append("AND S_CREATDATE BETWEEN '"+vDto.getSbegindate()+"' AND '"+vDto.getSenddate()+"') and S_PAYSNDBNKNO='"+vDto.getSclearaccno()+"'");
	...
	private MainAndSubDto get3507Data(TvVoucherinfoDto vDto) throws ITFEBizException
	{
		...
		if(vDto!=null&&vDto.getSdealno()!=null)
		{
			try {
				if(execDetail==null)
					execDetail = DatabaseFacade.getODB().getSqlExecutorFactory().getSQLExecutor();
				String sql = " select * from TF_RECONCILE_PAYINFO_MAIN where I_VOUSRLNO="+vDto.getSdealno();
				...
				if(dataList!=null&&dataList.size()>0)
				{
					...
					sql = "select * from TF_RECONCILE_PAYINFO_SUB where I_VOUSRLNO="+vDto.getSdealno();
					dataDto.setSubDtoList((List<IDto>)execDetail.runQuery(sql,TfReconcilePayinfoSubDto.class).getDtoCollection());
				}
				
			} catch (JAFDatabaseException e) {
				logger.error(e.getMessage(),e);
				throw new ITFEBizException("回执5507时查询3507数据信息异常！",e);
			}finally
			{
				if(execDetail!=null)
					execDetail.closeConnection();
