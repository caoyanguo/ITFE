...
/**
 * 总额分成日报表（下级汇总、全辖）（3405）
 * 
 */
@SuppressWarnings("unchecked")
public class VoucherDto2MapFor3405 implements IVoucherDto2Map{

	private static Log logger = LogFactory.getLog(VoucherDto2MapFor3405.class);
...
//		vDto.setScheckdate(TimeFacade.getEndDateOfMonth(vDto.getScheckdate()));
		...
		if(MsgConstant.VOUCHER_NO_3405.equals(vDto.getSvtcode()))
			dto.setSbillkind("4");
	...
	private List tranforList(TrIncomedayrptDto dto,TvVoucherinfoDto vDto) throws ITFEBizException
	{
		...
		try {
			...
			{
				sql.append("  AND (S_BUDGETSUBCODE=? or S_BUDGETSUBCODE=? or S_BUDGETSUBCODE=?)");
				execDetail.addParam("7777777777");
				execDetail.addParam("8888888888");
				execDetail.addParam("9999999999");
			}
	...
	public Map tranfor(List lists) throws ITFEBizException{
		try{	
			...
			vouchermap.put("Hold1", vDto.getShold4());//科目代码		
