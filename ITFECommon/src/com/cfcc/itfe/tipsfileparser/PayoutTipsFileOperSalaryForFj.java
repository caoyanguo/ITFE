 ...
 * author hua 实拨资金
 */
public class PayoutTipsFileOperSalaryForFj extends AbstractTipsFileOper {
		...
		try {
			...
			for (int i = 0; i < fileContent.size(); i++) {
				...
				if (i == 0) {
					...
					//将导入的工资文件的序号存入字段S_GROUPID，用于确定汇总后主信息的科目代码
					payoutDto.setSgroupid(singDto[0].trim());
					if (singDto[1] == null || "".equals(singDto[1].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'国库代码'为空!");
					}
					payoutDto.setStrecode(singDto[1] + "00");// 国库代码
					trecode = singDto[1] + "00";
					if (!this.checkTreasury(trecode, bookorgCode)) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中国库主体代码："
										+ trecode + " 没有在'国库主体信息参数'中查找到!");
					}
					if (singDto[2] == null || "".equals(singDto[2].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'拨款机关'为空!");
					}
					payoutDto.setSbillorg(singDto[2]);// 拨款机关

					if (singDto[3] == null || "".equals(singDto[3].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'收款单位代码'为空!");
					}
					if (filekind != null
							&& filekind
									.equals(StateConstant.IFNEWINTERFACE_FALSE)) {
						if (singDto[3] != null && singDto[3].length() > 15) {
							payoutDto.setSbdgorgcode(singDto[3]
									.substring(0, 15));// 收款单位代码
						} else {
							payoutDto.setSbdgorgcode(singDto[3]);// 收款单位代码
						}
					} else if (filekind != null
							&& filekind
									.equals(StateConstant.IFNEWINTERFACE_TRUE)) {
						payoutDto.setSbdgorgcode(singDto[3]);// 收款单位代码
					}
					// 修改为按照国库代码+预算单位唯一
					if (!rpmap.containsKey(singDto[1] + "00"
							+ payoutDto.getSbdgorgcode().trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中预算单位代码 '"
										+ payoutDto.getSbdgorgcode()
										+ "' 没有在法人代码表中找到!");
					} else {
						if (!"1".equals(rpmap.get(
								singDto[1] + "00" + payoutDto.getSbdgorgcode())
								.getCmayaprtfund().trim())) {
							multi.getErrorList().add(
									"实拨资金文件[" + fi.getName() + "]中预算单位代码 '"
											+ payoutDto.getSbdgorgcode()
											+ "' 对应预算单位不能进行实拨资金!");
					...
					if (singDto[4] == null || "".equals(singDto[4].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'收款单位名称'为空!");
					}
					...
					if (singDto[5] == null || "".equals(singDto[5].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'收款单位账号'为空!");
					}
					...
					if (filekind != null
						...
						if (singDto[5].length() > 32) {
							payoutDto
									.setSpayeeacct(singDto[5].substring(0, 32));// 收款单位账号
						} else {
					...
					if (singDto[6] == null || "".equals(singDto[6].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'收款单位开户行'为空!");
					}

					TsPaybankDto bnkpaydto = paymap.get(singDto[6].trim());
					if (null == bnkpaydto) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]根据收款单位开户行：'"
										+ singDto[6].trim()
										+ "'没有在'支付行号查询参数' 中找到相应清算行行号!");
					} else {
						if (null != bnkpaydto.getSstate()
								&& "0".equals(bnkpaydto.getSstate())) {
							multi.getErrorList().add(
									"实拨资金文件[" + fi.getName() + "]中收款单位开户行：'"
											+ singDto[6].trim()
											+ "' 处于'生效前'状态!");
						} else if (null != bnkpaydto.getSstate()
								&& "2".equals(bnkpaydto.getSstate())) {
							multi.getErrorList()
									.add(
											"实拨资金文件[" + fi.getName()
													+ "]中收款单位开户行：'"
					...
					if (singDto[7] == null || "".equals(singDto[7].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'拨款凭证号'为空!");
					}
					/*
					 * 分包，按凭证编号分包
					 */
					payoutDto.setSvouno(singDto[7]);// 拨款凭证号
					vouset.add(payoutDto.getSvouno()); // 根据凭证编号分包
					if (!voulist.contains(trecode.trim() + ","
							+ singDto[7].trim())) {
					...
					if (singDto[9] == null || "".equals(singDto[9].trim())) {
						multi.getErrorList().add(
								"实拨资金文件[" + fi.getName() + "]中存在'支出功能科目代码'为空!");
					}
					String err = this.verifySubject(dmap, singDto[9],
							MsgConstant.MSG_NO_5101, "1", fi.getName(),singDto[7]);
					if (!"".equals(err)) {
					...
					err = this.verifySubject(dmap, singDto[14],
							MsgConstant.MSG_NO_5101, "2", fi.getName(),singDto[7]);
					if (!"".equals(err)) {
						multi.getErrorList().add(err);
					}
					...
					payoutDto.setIofyear(Integer.parseInt(fi.getName()
							.substring(0, 4))); // 所属年度
					...
					payoutDto.setSaddword(singDto[9]);
					recordNum++;
			...
			// 2.如果某凭证编号下的明细DTO的数量大于TIPS分包限制,则进行再分包
			for (String st : vouset) {
				int fbcount = 0; // 分包记数器
				List<IDto> detlist = bnkli.get(st);
				//某凭证编号下的明细DTO的数量
				int detlistSize=detlist.size();
				if(detlistSize!=0&&detlistSize>499){
					multi.getErrorList().add("工资拨付文件["+fi.getName()+"]中，凭证编号为["+st.toString()+"]的明细信息总笔数为["+detlistSize+"],超过499笔，请手工拆包后导入!");	
				}
				// 生成包流水号
				...
				for (IDto dto3 : detlist) {
					TbsTvPayoutDto bkdto=(TbsTvPayoutDto) dto3;
					// 设置包流水号
				...
				List conlist = CommonFacade.getODB().findRsByDtoWithUR(
						condto);
				if (conlist == null || conlist.size() == 0) {
					multi.getErrorList().add(
							"实拨资金文件[" + fi.getName() + "] 中根据核算主体代码："
									+ bookorgCode + "、 国库主体代码：" + trecode
									+ " 没有在 '财政机构信息维护参数'中找到对应财政机构代码!");
				} else {
