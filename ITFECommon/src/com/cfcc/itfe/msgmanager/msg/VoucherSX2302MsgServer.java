...
public class VoucherSX2302MsgServer extends AbstractXmlParser {

	private static Log logger = LogFactory.getLog(VoucherSX2302MsgServer.class);
	...
	public void dealMsg(HashMap map, String xmlString) throws ITFEBizException {
		...
		String SupDepCode = "";
		TvPayreckbankbackSxDto maindto = null;
		List<TvPayreckbankbacklistSxDto> subDtoList =null;
		...
		for (int i = 0; i < VoucherBodyList.size(); i++) {
			try {
				 ...
				 **/
				maindto = new TvPayreckbankbackSxDto();
				...
				if(StateConstant.COMMON_YES.equals(ITFECommonConstant.ISCHECKPAYPLAN)){
				...
				maindto.setDorivoudate(CommonUtil.strToDate(TimeFacade.getCurrentStringTimebefor()));//原凭证日期
				 ...
				 */
				subDtoList = new ArrayList<TvPayreckbankbacklistSxDto>();
				...
				String returnmsg = voucherVerifySX.checkValid(verifydto, MsgConstant.VOUCHER_NO_2302);
				 ...
				 * 业务主表，子表入库失败即签收失败
				 */
				TvPayreckbankbacklistSxDto[] subDtos = new TvPayreckbankbacklistSxDto[subDtoList
						.size()];
				subDtos = (TvPayreckbankbacklistSxDto[]) subDtoList.toArray(subDtos);
