...
public class Voucher5513MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher5513MsgServer.class);
...
//					mainXCheckResult = elementVoucher.elementText("XCheckResult");//对帐结果
					...
					{
						sql = "update TF_REPORT_DEPOSIT_MAIN set S_XCHECKRESULT=?,S_XDIFFNUM=? where I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_VTCODE=? and S_STATUS=?)";
						execDetail.addParam(mainXCheckResult);
						execDetail.addParam(mainXDiffNum);
						execDetail.addParam(voucherCheckNo);
						execDetail.addParam(MsgConstant.VOUCHER_NO_3513);
					...
					+" where S_DEALNO=? and S_VTCODE=? and S_STATUS=?";//+"',S_RECVTIME='"+new Timestamp(new java.util.Date().getTime())
					...
					if(ITFECommonConstant.PUBLICPARAM.contains(",bussreport=insert,"))
					{
						if(listDetail!=null&&listDetail.size()>0)
						{
							for (int j = 0; j < listDetail.size(); j++) {
								...
								sql = "update TF_REPORT_DEPOSIT_SUB set S_XCHECKRESULT=?,S_XCHECKREASON=? where I_SEQNO=? and I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_VTCODE=? and S_STATUS=?)";
			...
			logger.error("解析库款账户对帐回执5513报文出现错误！", e);
			throw new ITFEBizException("解析库款账户对帐回执5513报文出现错误！", e);
