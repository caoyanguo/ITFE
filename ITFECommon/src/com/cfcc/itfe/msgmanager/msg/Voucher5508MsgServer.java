...
public class Voucher5508MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher5508MsgServer.class);
		...
		try {
			...
			for (int i = 0; i < VoucherBodyList.size(); i++) {
				try {
					...
					{
						sql = "update TF_RECONCILE_REALDIAL_MAIN set S_XCHECKRESULT=?,S_XDIFFNUM=? where I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_HOLD1=? and S_HOLD2=? and S_VTCODE=? and S_STATUS=?)";
						...
						execDetail.addParam(MsgConstant.VOUCHER_NO_3508);
					...
					if(ITFECommonConstant.PUBLICPARAM.contains(",bussreport=insert,"))
					{
						if(listDetail!=null&&listDetail.size()>0)
						{
							for (int j = 0; j < listDetail.size(); j++) {
								...
								sql = "update TF_RECONCILE_REALDIAL_SUB set S_XCHECKRESULT=?,S_XCHECKREASON=? where I_SEQNO=? and I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_HOLD1=? and S_HOLD2=? and S_VTCODE=? and S_STATUS=?)";
			...
			logger.error("解析实拨信息对帐回执5508报文出现错误！", e);
			throw new ITFEBizException("解析实拨信息对帐回执5508报文出现错误！", e);
