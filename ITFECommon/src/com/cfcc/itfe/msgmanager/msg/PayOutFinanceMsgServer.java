...
public class PayOutFinanceMsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(PayOutFinanceMsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			...
			// DTO -> MAP
			Map xmlMap = Dto2MapFor5112.tranfor(list, orgcode, filename, packno,isRepeat);
			...
			try {
				...
				String updateSql ="update "+TvFilepackagerefDto.tableName()+" set  S_MSGID = ? , T_SENDTIME = CURRENT TIMESTAMP where  S_PACKAGENO = ? and  S_ORGCODE = ?  and S_COMMITDATE = ?";
					...
					updateExce.addParam(list.get(0).getSentrustdate().replaceAll("-", ""));////委托日期
			...
			logger.error("查询代发财政性款项请求业务出现数据库异常!", e);
			throw new ITFEBizException("查询代发财政性款项请求业务出现数据库异常!", e);
