...
public class CorrhandBookMsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(CorrhandBookMsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			List<TvInCorrhandbookDto> list = DatabaseFacade.getDb().find(TvInCorrhandbookDto.class, orderbysql, params);
			Map xmlMap = new HashMap();
			// DTO -> MAP
			if(ITFECommonConstant.IFNEWINTERFACE.equals("1")){
				xmlMap = Dto2MapFor11052.tranfor(list, orgcode, filename, packno,isRepeat);
			}else{
				xmlMap = Dto2MapFor1105.tranfor(list, orgcode, filename, packno,isRepeat);
			}
			
			// 写发送日志
			String ls_MsgId  = (String)((Map)((Map)xmlMap.get("cfx")).get("HEAD")).get("MsgID");
			Map msg  = (Map) xmlMap.get("cfx");
			Map batchHead1105 = (Map)((Map)msg.get("MSG")).get("BatchHead1105");
			
			BigDecimal amt = new BigDecimal(batchHead1105.get("AllAmt").toString());
			int num = Integer.parseInt(batchHead1105.get("AllNum").toString());
			eventContext.getMessage().setProperty(MessagePropertyKeys.MSG_SEND_LOG_DTO,
					MsgLogFacade.writeSendLogWithResult(StampFacade.getStampSendSeq("FS"), null,
							...
							MsgConstant.MSG_NO_1105, (String) eventContext
							.getMessage().getProperty("XML_MSG_FILE_PATH"), num, amt, list.get(0).getSpackageno(), list.get(0).getScuraimtrecode(),
							null, list.get(0).getSoritaxorgcode(), null, ls_MsgId,
			...
			logger.error("查询更正请求业务出现数据库异常!", e);
			throw new ITFEBizException("查询收更正请求业务出现数据库异常!", e);
