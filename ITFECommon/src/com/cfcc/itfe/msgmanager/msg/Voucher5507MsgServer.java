...
public class Voucher5507MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher5507MsgServer.class);
		...
		try {
			...
			for (int i = 0; i < VoucherBodyList.size(); i++) {
				try {
					...
					execDetail.addParam(("0".equals(mainXCheckResult)?"财政对账成功,":"财政对账失败,"));
					execDetail.addParam(voucherCheckNo);
					execDetail.addParam(childPackNum);
					execDetail.addParam(curPackNo);
					execDetail.addParam(MsgConstant.VOUCHER_NO_3507);
			...
			logger.error("解析财政清算信息对帐回执5507报文出现错误！", e);
			throw new ITFEBizException("解析财政清算信息对帐回执5507报文出现错误！", e);
