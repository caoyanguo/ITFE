...
public class Send2202MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Send2202MsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			List<TvPayreckBankBackDto> list = DatabaseFacade.getDb().find(TvPayreckBankBackDto.class, orderbysql, params);

			// DTO -> MAP
			Map xmlMap = Dto2MapFor2202.tranfor(list, orgcode, filename, packno,isRepeat);
			
			String ls_MsgId  = (String)((Map)((Map)xmlMap.get("cfx")).get("HEAD")).get("MsgID");
			Map msg  = (Map) xmlMap.get("cfx");
			Map Head2202 = (Map)((Map)msg.get("MSG")).get("Head2202");
			BigDecimal amt = new BigDecimal(Head2202.get("AllAmt").toString());
			int num = Integer.parseInt(Head2202.get("AllNum").toString());
			// 设置消息体
			...
			eventContext.getMessage().setProperty(MessagePropertyKeys.MSG_SEND_LOG_DTO,
					MsgLogFacade.writeSendLogWithResult(StampFacade.getStampSendSeq("FS"), null,
							...
							MsgConstant.APPLYPAY_BACK_DAORU, (String) eventContext
							.getMessage().getProperty("XML_MSG_FILE_PATH"), num, amt, list.get(0).getSpackno(), list.get(0).getStrecode(),
							null, list.get(0).getSpaysndbnkno(), null, ls_MsgId,
