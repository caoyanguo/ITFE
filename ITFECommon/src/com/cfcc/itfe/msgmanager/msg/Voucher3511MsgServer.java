...
public class Voucher3511MsgServer extends AbstractMsgManagerServer {
	private static Log logger = LogFactory.getLog(Voucher3511MsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			...
			logger.error("解析收入报表对账请求信息3511报文出现错误！",e);
			throw new ITFEBizException("解析收入报表对账请求信息3511报文出现错误！",e);
		...
		TfReportIncomeMainDto mainDto = null;
		...
		for(int i=0;i<VoucherBodyList.size();i++){
			...
			mainDto = new TfReportIncomeMainDto();
			...
			//辖属标志
			String belongFlag = elementVoucher.elementText("BelongFlag");
			mainDto.setSbelongflag(belongFlag);
			//预算级次
			String budgetLevelCode = elementVoucher.elementText("BudgetLevelCode");
			mainDto.setSbudgetlevelcode(budgetLevelCode);
			//报表种类
			...
			subDtoList = new ArrayList<TfReportIncomeSubDto>();
			//封装明细信息
			for(int j=0;j<listDetail.size();j++){
				...
				subdto.setSid(id);
				//行政区划代码
				String adc = elementDetail.elementText("AdmDivCode");
				subdto.setSadmdivcode(adc);
				//业务年度
				String sy = elementDetail.elementText("StYear");
				subdto.setSstyear(sy);
				//征收机关代码
				String taxOrgCode = elementDetail.elementText("TaxOrgCode");
				subdto.setStaxorgcode(taxOrgCode);
				//征收机关名称
				String taxOrgName = elementDetail.elementText("TaxOrgName");
				subdto.setStaxorgname(taxOrgName);
				//资金性质
				String budgetType = elementDetail.elementText("BudgetType");
				subdto.setSbudgettype(budgetType);
				//预算级次
				String blc = elementDetail.elementText("BudgetLevelCode");
				subdto.setSbudgetlevelcode(blc);
				//收入分类科目编码
				String budgetSubjectCode = elementDetail.elementText("BudgetSubjectCode");
				subdto.setSbudgetsubjectcode(budgetSubjectCode);
				//收入分类科目名称
				String budgetSubjectName = elementDetail.elementText("BudgetSubjectName");
				subdto.setSbudgetsubjectname(budgetSubjectName);
				//本期金额
				String curIncomeAmt = elementDetail.elementText("CurIncomeAmt");
				subdto.setNcurincomeamt(new BigDecimal(curIncomeAmt));
				//累计金额
				String sumIncomeAmt = elementDetail.elementText("SumIncomeAmt");
				subdto.setNsumincomeamt(new BigDecimal(sumIncomeAmt));
				//对帐结果
			...
			try {
				...
				String returnmsg = voucherVerify.checkValid(verifydto, MsgConstant.VOUCHER_NO_3511);
				//返回错误信息签收失败
				if(returnmsg != null){
				...
				logger.error("校验收入报表对账请求信息3511报文出现错误！", e1);
				throw new ITFEBizException("校验收入报表对账请求信息3511报文出现错误！");
			...
			try {
				...
				TfReportIncomeSubDto[] subDtos = (TfReportIncomeSubDto[])(subDtoList.toArray(new TfReportIncomeSubDto[subDtoList.size()]));
				DatabaseFacade.getODB().create(subDtos);
			} catch (JAFDatabaseException e1) {
				logger.error("保存收入报表对账请求信息数据库异常", e1);
				throw new ITFEBizException("保存收入报表对账请求信息数据库异常");
			}
