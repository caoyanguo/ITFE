...
public class Voucher2305MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher2305MsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			...
			logger.error("解析大数据划款2305报文出现错误！", e);
			throw new ITFEBizException("解析大数据划款2305报文出现错误！", e);
...
//		}
		...
		TvPayreckBigdataDto maindto = null;
		// 解析报文
...
//				String S_BGTTYPECODE= elementVoucher.elementText("BgtTypeCode");//'预算类型编码';
				...
				maindto = new TvPayreckBigdataDto();
				 ...
				 */
				List<TvPayreckBigdataListDto> subDtoList = new ArrayList<TvPayreckBigdataListDto>();
				for (int j = 0; j < listDetail.size(); j++) {
					...
					if(sN_PAYAMT==null||"".equals(N_XPAYAMT))
						N_XPAYAMT="0.00";
					...
					TvPayreckBigdataListDto subdto = new TvPayreckBigdataListDto();
					// 此处的设值待确认
				...
				{
					List<List<TvPayreckBigdataListDto>> gelist = getSubLists(subDtoList,1000);
					for(int k=0;k<gelist.size();k++)
					{
						TvPayreckBigdataListDto[] subDtos = new TvPayreckBigdataListDto[gelist.get(k).size()];
						subDtos = (TvPayreckBigdataListDto[]) gelist.get(k).toArray(subDtos);
						DatabaseFacade.getODB().create(subDtos);
					}
				}else
				{
					TvPayreckBigdataListDto[] subDtos = new TvPayreckBigdataListDto[subDtoList.size()];
					subDtos = (TvPayreckBigdataListDto[]) subDtoList.toArray(subDtos);
					DatabaseFacade.getODB().create(subDtos);
				}
				voucher.voucherConfirmSuccess(vDto.getSdealno());
			}catch(Exception e){
				logger.error("2305解析发生异常:",e);
