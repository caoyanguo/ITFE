...
public class Voucher5512MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher5512MsgServer.class);
...
//					curPackNo = elementVoucher.elementText("CurPackNo");//本包序号
					...
					{
						sql = "update TF_REPORT_DEFRAY_MAIN set S_XCHECKRESULT=?,S_XDIFFNUM=? where I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_VTCODE=? and S_STATUS=?)";
						execDetail.addParam(mainXCheckResult);
						execDetail.addParam(mainXDiffNum);
						execDetail.addParam(voucherCheckNo);
						execDetail.addParam(MsgConstant.VOUCHER_NO_3512);
					...
					if(ITFECommonConstant.PUBLICPARAM.contains(",bussreport=insert,"))
					{
						if(listDetail!=null&&listDetail.size()>0)
						{
							for (int j = 0; j < listDetail.size(); j++) {
								...
								sql = "update TF_REPORT_DEFRAY_SUB set S_XCHECKRESULT=?,S_XCHECKREASON=? where I_SEQNO=? and I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_VTCODE=? and S_STATUS=?)";
			...
			logger.error("解析支出报表对帐回执5512报文出现错误！", e);
			throw new ITFEBizException("解析支出报表对帐回执5512报文出现错误！", e);
