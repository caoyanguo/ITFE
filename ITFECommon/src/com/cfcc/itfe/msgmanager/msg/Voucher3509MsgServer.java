...
public class Voucher3509MsgServer extends AbstractMsgManagerServer {
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			...
			logger.error("解析收入退付对账请求信息3509报文出现错误！",e);
			throw new ITFEBizException("解析收入退付对账请求信息3509报文出现错误！",e);
		...
		for(int i=0;i<VoucherBodyList.size();i++){
			...
			try {
				...
				logger.error("校验收入退付对账请求信息失败！", e1);
				throw new ITFEBizException("校验收入退付对账请求信息失败！");
			...
			try {
				...
				logger.error("保存收入退付对账请求信息数据库异常", e1);
				throw new ITFEBizException("保存收入退付对账请求信息数据库异常");
			}
