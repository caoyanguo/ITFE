...
public class Voucher5509MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher5509MsgServer.class);
		...
		try {
			...
			for (int i = 0; i < VoucherBodyList.size(); i++) {
				try {
					...
					{
						sql = "update TF_RECONCILE_REFUND_MAIN set S_XCHECKRESULT=?,S_XDIFFNUM=? where I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_HOLD1=? and S_HOLD2=?)";
					...
					execDetail.addParam(childPackNum);
					execDetail.addParam(curPackNo);
					execDetail.addParam(MsgConstant.VOUCHER_NO_3509);
					...
					if(ITFECommonConstant.PUBLICPARAM.contains(",bussreport=insert,"))
					{
						if(listDetail!=null&&listDetail.size()>0)
						{
							for (int j = 0; j < listDetail.size(); j++) {
								...
								sql = "update TF_RECONCILE_REFUND_SUB set S_XCHECKRESULT=?,S_XCHECKREASON=? where I_SEQNO=? and I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_HOLD1=? and S_HOLD2=?)";
			...
			logger.error("解析收入退付对帐回执5509报文出现错误！", e);
			throw new ITFEBizException("解析收入退付对帐回执5509报文出现错误！", e);
