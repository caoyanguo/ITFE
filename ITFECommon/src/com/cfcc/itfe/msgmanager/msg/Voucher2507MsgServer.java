...
public class Voucher2507MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher2507MsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			...
			String voucherNo = null;//主单凭证号//TfReconcilePayinfoMainDto maindto = null;//List<TfReconcilePayinfoSubDto> subDtoList =null;
			...
			for (int i = 0; i < VoucherBodyList.size(); i++) {
				try {
					...
					{
						sql = "update TF_RECONCILE_PAYINFO_MAIN set S_XCHECKRESULT=?,S_XDIFFNUM=? where I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_HOLD1=? and S_HOLD2=? and S_VTCODE=? and S_STATUS=?)";
						...
						execDetail.addParam(MsgConstant.VOUCHER_NO_3507);
						execDetail.addParam(DealCodeConstants.VOUCHER_SUCCESS);
						execDetail.runQuery(sql);
					}
					sql = "update TV_VOUCHERINFO set S_STATUS=?,S_DEMO=S_DEMO||?"
					+" where S_ATTACH=? and S_HOLD1=? and S_HOLD2=? and S_VTCODE=? and S_STATUS=?";
					execDetail.addParam(DealCodeConstants.VOUCHER_READBANK);
					execDetail.addParam(("0".equals(mainXCheckResult)?"商行对账成功,":"商行对账失败,"));
					execDetail.addParam(voucherCheckNo);
					execDetail.addParam(childPackNum);
					execDetail.addParam(curPackNo);
					execDetail.addParam(MsgConstant.VOUCHER_NO_3507);
					...
					execDetail.addParam(curPackNo);
					execDetail.addParam(MsgConstant.VOUCHER_NO_3507);
					execDetail.addParam(DealCodeConstants.VOUCHER_READFIN);
					execDetail.runQuery(sql);
					...
					if(ITFECommonConstant.PUBLICPARAM.contains(",bussreport=insert,"))
					{
						if(listDetail!=null&&listDetail.size()>0)
						{
							for (int j = 0; j < listDetail.size(); j++) {
								...
								subXCheckReason = elementDetail.elementText("XCheckReason");// 不符原因//TfReconcilePayinfoSubDto subdto = new TfReconcilePayinfoSubDto();subdto.setSid(subId);subdto.setSxcheckreason(subXCheckReason);subdto.setSxcheckresult(subXCheckResult);
								sql = "update TF_RECONCILE_PAYINFO_SUB set S_XCHECKRESULT=?,S_XCHECKREASON=? where I_SEQNO=? and I_VOUSRLNO=(SELECT S_DEALNO FROM TV_VOUCHERINFO WHERE S_ATTACH=? and S_HOLD1=? and S_HOLD2=? and S_VTCODE=? and S_STATUS=?)";
			...
			logger.error("解析商行清算信息对帐回执2507报文出现错误！", e);
			throw new ITFEBizException("解析商行清算信息对帐回执2507报文出现错误！", e);
