...
public class GrantMsgServer extends AbstractMsgManagerServer {
	
	private static Log logger = LogFactory.getLog(GrantMsgServer.class);
	...
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			List<TvGrantpaymsgmainDto> list = DatabaseFacade.getDb().find(TvGrantpaymsgmainDto.class, orderbysql, params);

			// DTO -> MAP
			Map xmlMap = Dto2MapFor5103.tranfor(list, orgcode, filename, packno,isRepeat);
			
			
			String ls_MsgId  = (String)((Map)((Map)xmlMap.get("cfx")).get("HEAD")).get("MsgID");
			Map msg  = (Map) xmlMap.get("cfx");
			Map batchHead5103 = (Map)((Map)msg.get("MSG")).get("BatchHead5103");
			BigDecimal amt = new BigDecimal(batchHead5103.get("AllAmt").toString());
			int num = Integer.parseInt(batchHead5103.get("AllNum").toString());
			// 设置消息体
			...
			eventContext.getMessage().setProperty(MessagePropertyKeys.MSG_SEND_LOG_DTO,
					MsgLogFacade.writeSendLogWithResult(StampFacade.getStampSendSeq("FS"), null,
							...
							MsgConstant.MSG_NO_5103, (String) eventContext
...
//					updateExce.addParam(new Timestamp(TimeFacade.getCurrentDateTime().getTime()));
					...
					updateExce.addParam((list.get(0).getScommitdate()).toString().replace("-", ""));////委托日期
			...
			logger.error("查询授权支付额度业务出现数据库异常!", e);
			throw new ITFEBizException("查询授权支付额度业务出现数据库异常!", e);
