...
public class Voucher5253MsgServer extends AbstractMsgManagerServer {

	private static Log logger = LogFactory.getLog(Voucher8207MsgServer.class);
	private Voucher voucher;
	/**
	 * 财政发往人行的直接支付调整凭证5253，此类只保存业务数据
	 */
	public void dealMsg(MuleEventContext eventContext) throws ITFEBizException {
		...
		try {
			...
			logger.error("解析财政直接支付调整凭证5253报文出现错误！",e);
			throw new ITFEBizException("解析财政直接支付调整凭证5253报文出现错误！",e);
		...
		TfDirectpayAdjustmainDto maindto  = null;
		...
		for(int i=0;i<VoucherBodyList.size();i++){
			try{
				...
				//业务子表集合
				subDtoList=new ArrayList<TfDirectpayAdjustsubDto>();
				//子表明细Id集合
				...
				for(int j=0;j<listDetail.size();j++){
					...
					String sdetailXCheckNo=elementVoucher.elementText("XCheckNo");//支票号(结算号)
				...
				String returnmsg = voucherVerify.checkValid(verifydto, MsgConstant.VOUCHER_NO_5253);
				 ...
				 *
				 */
				if(subDtoIdList.size()==1){
					String errMsg="明细条数必须大于1条!";
					voucher.voucherComfail(vDto.getSdealno(), errMsg);
					continue;
				}
